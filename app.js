import { useState, useCallback } from "react";

// ══════════════════════════════════════════════════════════════
// PERIOD TIMES — 50 min each, no breaks
// P1-P2 متواصلتان، فاصل 5 دقائق قبل P3 (تبدأ 09:15)، ثم متواصلات
// ══════════════════════════════════════════════════════════════
const DEFAULT_PERIOD_CONFIG = [
  { id:1, label:"1", start:"07:30", end:"08:20", breakAfter:null },
  { id:2, label:"2", start:"08:20", end:"09:10", breakAfter:null },
  { id:3, label:"3", start:"09:15", end:"10:05", breakAfter:null },
  { id:4, label:"4", start:"10:05", end:"10:55", breakAfter:null },
  { id:5, label:"5", start:"11:00", end:"11:50", breakAfter:null },
  { id:6, label:"6", start:"11:50", end:"12:40", breakAfter:null },
  { id:7, label:"7", start:"12:40", end:"13:30", breakAfter:null },
  { id:8, label:"8", start:"13:30", end:"14:20", breakAfter:null },
];

// ══════════════════════════════════════════════════════════════
// SPECIALIZATIONS
// ══════════════════════════════════════════════════════════════
const SPECIALIZATIONS = [
  { id:"net",  name:"تقنية شبكات الحاسب",      dept:"الحاسب وتقنية المعلومات", color:"#0ea5e9" },
  { id:"auto", name:"تقنية المحركات والمركبات", dept:"التقنية الميكانيكية",      color:"#f97316" },
  { id:"gen",  name:"الدراسات العامة",           dept:"مشترك",                  color:"#8b5cf6", shared:true },
];

// L=محاضرة(نظري), P=عملي/ورش, T=تمارين(تُحسب نظري), CTH=ساعات اتصال
const GENERAL_COURSES = [
  {id:"g01",code:"ENGL 101",ar:"انجل 101",name:"لغة إنجليزية (1)",                 spec:"gen",level:1,L:3,P:0,T:1,CTH:4},
  {id:"g02",code:"ICMT 101",ar:"حاسب 101",name:"مقدمة تطبيقات الحاسب",             spec:"gen",level:1,L:0,P:4,T:0,CTH:4},
  {id:"g03",code:"MATH 101",ar:"رياض 101",name:"الرياضيات",                         spec:"gen",level:1,L:3,P:0,T:1,CTH:4},
  {id:"g04",code:"PHYS 101",ar:"فيزي 101",name:"الفيزياء",                          spec:"gen",level:1,L:2,P:2,T:0,CTH:4},
  {id:"g05",code:"ENGL 102",ar:"انجل 102",name:"لغة إنجليزية (2)",                 spec:"gen",level:2,L:3,P:0,T:1,CTH:4},
  {id:"g06",code:"ENTR 101",ar:"رياد 101",name:"أساسيات ريادة الأعمال",            spec:"gen",level:2,L:2,P:0,T:0,CTH:2},
  {id:"g07",code:"ARAB 101",ar:"عربي 101",name:"الكتابة الفنية",                   spec:"gen",level:2,L:2,P:0,T:0,CTH:2},
  {id:"g08",code:"ENGL 103",ar:"انجل 103",name:"لغة إنجليزية (3)",                 spec:"gen",level:3,L:3,P:0,T:1,CTH:4},
  {id:"g09",code:"ENTR 212",ar:"رياد 212",name:"ريادة الأعمال التخصصية",           spec:"gen",level:3,L:1,P:2,T:0,CTH:3},
  {id:"g10",code:"ENGL 204",ar:"انجل 204",name:"لغة إنجليزية (4)",                 spec:"gen",level:4,L:3,P:0,T:1,CTH:4},
  {id:"g11",code:"ISLM 101",ar:"اسلم 101",name:"الدراسات الإسلامية",               spec:"gen",level:4,L:2,P:0,T:0,CTH:2},
  {id:"g12",code:"ETHS 201",ar:"اسلك 201",name:"السلوك الوظيفي والمستقبل المهني", spec:"gen",level:4,L:2,P:0,T:0,CTH:2},
];
const NET_COURSES = [
  {id:"n01",code:"ICMT 121",ar:"حاسب 121",name:"تجميع الحاسب وتشغيله",         spec:"net",level:1,L:1,P:4,T:0,CTH:5},
  {id:"n02",code:"INET 111",ar:"شبكا 111",name:"مبادئ شبكات الحاسب",           spec:"net",level:1,L:3,P:2,T:0,CTH:5},
  {id:"n03",code:"ICMT 102",ar:"حاسب 102",name:"تطبيقات الحاسب المتقدمة",      spec:"net",level:2,L:0,P:4,T:0,CTH:4},
  {id:"n04",code:"ISUP 151",ar:"داعم 151",name:"نظم التشغيل (1)",              spec:"net",level:2,L:2,P:4,T:0,CTH:6},
  {id:"n05",code:"INET 121",ar:"شبكا 121",name:"التوجيه والتحويل (1)",         spec:"net",level:2,L:2,P:4,T:0,CTH:6},
  {id:"n06",code:"INET 231",ar:"شبكا 231",name:"مقدمة في أمن الشبكات",        spec:"net",level:3,L:2,P:2,T:0,CTH:4},
  {id:"n07",code:"ISUP 252",ar:"داعم 252",name:"نظم التشغيل (2)",              spec:"net",level:3,L:2,P:4,T:0,CTH:6},
  {id:"n08",code:"INET 222",ar:"شبكا 222",name:"التوجيه والتحويل (2)",         spec:"net",level:3,L:2,P:4,T:0,CTH:6},
  {id:"n09",code:"INET 285",ar:"شبكا 285",name:"موضوعات مختارة",              spec:"net",level:3,L:1,P:2,T:0,CTH:3},
  {id:"n10",code:"ICMT 280",ar:"حاسب 280",name:"التأهيل للشهادات الاحترافية", spec:"net",level:4,L:1,P:0,T:0,CTH:1},
  {id:"n11",code:"INET 232",ar:"شبكا 232",name:"أمن الشبكات المتقدم",         spec:"net",level:4,L:2,P:2,T:0,CTH:4},
  {id:"n12",code:"INET 241",ar:"شبكا 241",name:"ربط الشبكات",                 spec:"net",level:4,L:1,P:4,T:1,CTH:6},
  {id:"n13",code:"INET 295",ar:"شبكا 295",name:"مشروع",                       spec:"net",level:4,L:1,P:2,T:0,CTH:3},
  {id:"n14",code:"INET 299",ar:"شبكا 299",name:"التدريب التعاوني",             spec:"net",level:5,L:0,P:0,T:0,CTH:0,coop:true},
];
const AUTO_COURSES = [
  {id:"a01",code:"MMEV 101",ar:"متمر 101",name:"السلامة والصحة المهنية",                     spec:"auto",level:1,L:1,P:0,T:0,CTH:1},
  {id:"a02",code:"MMEV 111",ar:"متمر 111",name:"أساسيات ورش",                               spec:"auto",level:1,L:0,P:2,T:0,CTH:2},
  {id:"a03",code:"MELV 141",ar:"مكمر 141",name:"مبادئ الكهرباء والإلكترونيات",              spec:"auto",level:1,L:1,P:2,T:0,CTH:3},
  {id:"a04",code:"MMEV 121",ar:"متمر 121",name:"محركات السيارات",                            spec:"auto",level:2,L:1,P:4,T:0,CTH:5},
  {id:"a05",code:"MMEV 115",ar:"متمر 115",name:"الرسم الهندسي والفني",                      spec:"auto",level:2,L:0,P:2,T:0,CTH:2},
  {id:"a06",code:"MMEV 262",ar:"متمر 262",name:"نظام الإشعال",                              spec:"auto",level:2,L:1,P:2,T:0,CTH:3},
  {id:"a07",code:"MMEV 251",ar:"متمر 251",name:"نظام نقل القدرة",                           spec:"auto",level:2,L:2,P:4,T:0,CTH:6},
  {id:"a08",code:"MMEV 131",ar:"متمر 131",name:"نظام الفرامل",                              spec:"auto",level:2,L:1,P:2,T:0,CTH:3},
  {id:"a09",code:"MMEV 116",ar:"متمر 116",name:"رسم بالحاسب",                              spec:"auto",level:3,L:0,P:2,T:0,CTH:2},
  {id:"a10",code:"MMEV 242",ar:"متمر 242",name:"الأنظمة الكهربائية والإلكترونية بالسيارات",spec:"auto",level:3,L:1,P:4,T:0,CTH:5},
  {id:"a11",code:"MMEV 222",ar:"متمر 222",name:"صيانة وإصلاح المحركات",                    spec:"auto",level:3,L:0,P:4,T:0,CTH:4},
  {id:"a12",code:"MMEV 261",ar:"متمر 261",name:"نظام الوقود (بنزين)",                      spec:"auto",level:3,L:1,P:4,T:0,CTH:5},
  {id:"a13",code:"MMEV 223",ar:"متمر 223",name:"محركات الطاقات البديلة",                   spec:"auto",level:3,L:0,P:2,T:0,CTH:2},
  {id:"a14",code:"MMEV 271",ar:"متمر 271",name:"تخطيط وإدارة ورش السيارات",                spec:"auto",level:4,L:2,P:0,T:0,CTH:2},
  {id:"a15",code:"MMEV 252",ar:"متمر 252",name:"نظام التعليق والتوجيه",                    spec:"auto",level:4,L:1,P:4,T:0,CTH:5},
  {id:"a16",code:"MMEV 264",ar:"متمر 264",name:"نظام الوقود (ديزل)",                      spec:"auto",level:4,L:2,P:2,T:0,CTH:4},
  {id:"a17",code:"MMEV 263",ar:"متمر 263",name:"منظومات التحكم في ملوثات السيارات",        spec:"auto",level:4,L:0,P:2,T:0,CTH:2},
  {id:"a18",code:"MMEV 281",ar:"متمر 281",name:"تشخيص الأعطال",                            spec:"auto",level:4,L:2,P:4,T:0,CTH:6},
  {id:"a19",code:"MMEV 295",ar:"متمر 295",name:"مشروع",                                    spec:"auto",level:4,L:1,P:2,T:0,CTH:3},
  {id:"a20",code:"MMEV 299",ar:"متمر 299",name:"التدريب التعاوني",                         spec:"auto",level:5,L:0,P:0,T:0,CTH:0,coop:true},
];
const ALL_COURSES = [...GENERAL_COURSES,...NET_COURSES,...AUTO_COURSES];

const INIT_TRAINERS = [
  {id:"t1",name:"أ. محمد العمري",   specs:["net","gen"],maxH:18},
  {id:"t2",name:"أ. سارة الزهراني",specs:["auto","gen"],maxH:16},
  {id:"t3",name:"أ. خالد الغامدي", specs:["net"],       maxH:20},
  {id:"t4",name:"أ. نورة السهلي",  specs:["gen"],       maxH:14},
  {id:"t5",name:"أ. فيصل الدوسري",specs:["auto"],      maxH:18},
];
const INIT_HALLS = [
  {id:"h1",name:"قاعة 101",      cap:30,type:"theory",   specs:["all"]},
  {id:"h2",name:"قاعة 102",      cap:30,type:"theory",   specs:["all"]},
  {id:"h3",name:"مختبر شبكات 1",cap:20,type:"practical",specs:["net"]},
  {id:"h4",name:"مختبر شبكات 2",cap:20,type:"practical",specs:["net"]},
  {id:"h5",name:"ورشة محركات 1",cap:20,type:"practical",specs:["auto"]},
  {id:"h6",name:"ورشة محركات 2",cap:20,type:"practical",specs:["auto"]},
];

const DAYS = ["الأحد","الاثنين","الثلاثاء","الأربعاء","الخميس"];
const uid  = () => Math.random().toString(36).slice(2,9);

// ══════════════════════════════════════════════════════════════
// HELPERS
// ══════════════════════════════════════════════════════════════
// Check if adding a theory slot at (day,period) would create 4 consecutive theory slots
function wouldExceedConsecutiveTheory(sched, courses, day, newPeriod, newSessionHours=1, excludeId=null) {
  // Build set of theory periods (each session occupies sessionHours consecutive periods)
  const theoryPeriods = new Set();
  sched
    .filter(s => s.day === day && s.id !== excludeId)
    .filter(s => {
      const c = courses.find(x => x.id === s.courseId);
      const st = s.sessionType || (c?.P===0?"theory":"practical");
      return c && st === "theory";
    })
    .forEach(s => {
      for (let i=0; i<(s.sessionHours||1); i++) theoryPeriods.add(s.period+i);
    });
  // Add new session periods
  for (let i=0; i<newSessionHours; i++) theoryPeriods.add(newPeriod+i);
  const sorted = [...theoryPeriods].sort((a,b)=>a-b);
  let maxRun=1,run=1;
  for(let i=1;i<sorted.length;i++){
    if(sorted[i]===sorted[i-1]+1){run++;maxRun=Math.max(maxRun,run);}
    else run=1;
  }
  return maxRun>3;
}

// Check if a course placed in this cell is purely theory (no practical hours)
function isPurelyTheory(course) {
  return course && course.P === 0;
}

// ══════════════════════════════════════════════════════════════
// MINI COMPONENTS
// ══════════════════════════════════════════════════════════════
const Badge = ({color,children,sm}) => (
  <span style={{background:color+"22",color,border:`1px solid ${color}44`,
    borderRadius:5,padding:sm?"1px 6px":"2px 8px",fontSize:sm?9:11,fontWeight:700,whiteSpace:"nowrap"}}>
    {children}
  </span>
);
const Btn = ({onClick,bg="#3b82f6",outline,children,sm,full}) => (
  <button onClick={onClick} style={{
    background:outline?"transparent":bg,color:outline?bg:"#fff",
    border:`1px solid ${bg}`,borderRadius:9,padding:sm?"5px 12px":"8px 18px",
    cursor:"pointer",fontSize:sm?11:13,fontWeight:700,width:full?"100%":"auto"}}>
    {children}
  </button>
);
const Modal = ({title,onClose,wide,children}) => (
  <div style={{position:"fixed",inset:0,background:"#000000bb",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center"}}>
    <div style={{background:"#FFFFFF",border:"1px solid #1e2d45",borderRadius:16,padding:26,
      width:wide?860:480,maxHeight:"88vh",overflow:"auto",direction:"rtl"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <h3 style={{color:"#042F2E",margin:0,fontSize:16}}>{title}</h3>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#0F766E",cursor:"pointer",fontSize:22}}>×</button>
      </div>
      {children}
    </div>
  </div>
);
const Fld = ({label,value,onChange,type="text",min,max,note}) => (
  <div style={{marginBottom:13}}>
    <div style={{color:"#0F766E",fontSize:11,marginBottom:5}}>{label}</div>
    <input type={type} value={value} onChange={e=>onChange(e.target.value)} min={min} max={max}
      style={{width:"100%",background:"#FFFFFF",border:"1px solid #1e3a5f",borderRadius:8,
        padding:"8px 12px",color:"#134E4A",fontSize:13,boxSizing:"border-box",direction:"rtl"}}/>
    {note&&<div style={{color:"#0F766E",fontSize:10,marginTop:3}}>{note}</div>}
  </div>
);
const Sel = ({label,value,onChange,opts,note}) => (
  <div style={{marginBottom:13}}>
    {label&&<div style={{color:"#0F766E",fontSize:11,marginBottom:5}}>{label}</div>}
    <select value={value} onChange={e=>onChange(e.target.value)}
      style={{width:"100%",background:"#FFFFFF",border:"1px solid #1e3a5f",borderRadius:8,
        padding:"8px 12px",color:"#134E4A",fontSize:13,direction:"rtl"}}>
      {opts.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}
    </select>
    {note&&<div style={{color:"#0F766E",fontSize:10,marginTop:3}}>{note}</div>}
  </div>
);

// ══════════════════════════════════════════════════════════════
// SCHEDULE CELL COMPONENT
// ══════════════════════════════════════════════════════════════
function SchedCell({cell,courses,trainers,halls,spColor,mainSpec,onRemove,onUpdate,sched,periods,conflicts,sharedWith,allGroups,onAddShared}) {
  const c = courses.find(x=>x.id===cell.courseId);
  if(!c) return null;
  const color    = c.spec==="gen" ? spColor("gen") : spColor(mainSpec);
  const isTheory = c.P === 0;
  const hasBoth  = c.L > 0 && c.P > 0;
  const sessionH = cell.sessionHours || 1;
  const sessionType = cell.sessionType || (isTheory ? "theory" : "practical");

  // Filter trainers by spec eligibility AND no time conflict at this cell's day/period
  const eligibleTr = trainers.filter(t=>
    t.specs.includes(c.spec)||t.specs.includes(mainSpec)||t.specs.includes("gen")
  );
  // Further filter: remove those who have another session at same day+period (excluding this cell and its shared family)
  const getAllCellsLocal = () => {
    if(!sched) return [];
    return sched; // sched here is the current group's cells
  };
  const avTr = eligibleTr.filter(t=>{
    // Check off-days
    if((t.offDays||[]).includes(cell.day)) return false;
    const slot=(t.offSlots||[]).find(s=>s.day===cell.day);
    if(slot&&slot.periods.includes(cell.period)) return false;
    return true;
    // Note: cross-group conflict check done at save time (in handleCellUpdate)
  });
  // Separate lists: available vs busy
  const busyTr = eligibleTr.filter(t=>!avTr.find(x=>x.id===t.id));

  const hasConflict = conflicts && conflicts.length > 0;
  const isShared = sharedWith && sharedWith.length > 0;

  // Max consecutive hours allowed for this session type
  const maxH = sessionType === "practical" ? 4 : 3;

  return (
    <div
      draggable
      onDragStart={e=>e.dataTransfer.setData("moveCell",JSON.stringify({id:cell.id,day:cell.day,period:cell.period}))}
      style={{background:hasConflict?"#FEF2F2":color+"11",
        border:`2px solid ${hasConflict?"#EF4444":color+"66"}`,
        borderRadius:8,padding:"5px 7px",cursor:"grab",position:"relative",userSelect:"none"}}
    >
      {hasConflict&&(
        <div title={conflicts.join("\n")}
          style={{position:"absolute",top:-6,right:-6,background:"#EF4444",color:"#fff",
            borderRadius:"50%",width:16,height:16,fontSize:10,display:"flex",alignItems:"center",
            justifyContent:"center",fontWeight:900,zIndex:10,cursor:"help"}}>!</div>
      )}
      <button onClick={onRemove}
        style={{position:"absolute",top:2,left:4,background:"none",border:"none",
          color:"#EF4444",cursor:"pointer",fontSize:12,lineHeight:1,padding:0}}>×</button>

      {/* Course code + session hours badge */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingLeft:12,marginBottom:2}}>
        <div style={{color:hasConflict?"#991B1B":color,fontWeight:700,fontSize:9,fontFamily:"monospace"}}>{c.code}</div>
        <div style={{background:color,color:"#fff",borderRadius:4,padding:"0 5px",fontSize:9,fontWeight:700,lineHeight:"16px"}}>
          {sessionH}س
        </div>
      </div>
      <div style={{color:"#134E4A",fontSize:9,marginBottom:isShared?2:4,lineHeight:1.3}}>{c.name}</div>

      {/* Shared with other groups badge */}
      {isShared&&(
        <div style={{marginBottom:4}}>
          <div style={{background:"#FEF3C7",border:"1px solid #F59E0B",borderRadius:5,
            padding:"3px 6px",fontSize:8,color:"#92400E",lineHeight:1.5}}>
            🔗 مشترك مع:
            {sharedWith.map((sw,i)=>(
              <span key={i} style={{display:"inline-block",background:"#F59E0B",color:"#fff",
                borderRadius:3,padding:"0 5px",marginRight:3,fontSize:8,fontWeight:700}}>
                {sw}
              </span>
            ))}
          </div>
        </div>
      )}
      <div style={{marginBottom:4}}>
        <input
          value={cell.refNumber||""}
          onChange={e=>onUpdate({refNumber:e.target.value})}
          placeholder="الرقم المرجعي..."
          onClick={e=>e.stopPropagation()}
          onMouseDown={e=>e.stopPropagation()}
          style={{width:"100%",background:"#F0FDFA",border:"1px solid #CBD5E1",borderRadius:5,
            padding:"3px 6px",fontSize:9,color:"#134E4A",boxSizing:"border-box",direction:"rtl",
            fontFamily:"monospace",fontWeight:600}}
        />
      </div>

      {/* Session type (for mixed courses) */}
      {hasBoth && (
        <div style={{display:"flex",gap:3,marginBottom:4}}>
          {["theory","practical"].map(t=>(
            <button key={t} onClick={()=>onUpdate({sessionType:t})}
              style={{flex:1,
                background:sessionType===t?(t==="theory"?"#ECFEFF":"#ECFDF5"):"#F0FDFA",
                border:`1px solid ${t==="theory"?"#0891B2":"#10B981"}`,
                color:t==="theory"?"#0E7490":"#065F46",
                borderRadius:4,padding:"2px 0",fontSize:9,cursor:"pointer",
                fontWeight:sessionType===t?700:400}}>
              {t==="theory"?"📘 نظري":"🔧 عملي"}
            </button>
          ))}
        </div>
      )}

      {/* Hours per session selector */}
      <div style={{marginBottom:4}}>
        <div style={{color:"#0F766E",fontSize:9,marginBottom:2}}>
          عدد ساعات الجلسة:
          <span style={{color:sessionType==="practical"?"#10B981":"#0891B2",marginRight:4,fontWeight:700}}>
            (max {maxH})
          </span>
        </div>
        <div style={{display:"flex",gap:3}}>
          {[1,2,3,4].map(h=>(
            <button key={h} onClick={()=>onUpdate({sessionHours:h})}
              disabled={h>maxH}
              style={{flex:1,
                background:sessionH===h?color:"#F0FDFA",
                color:sessionH===h?"#fff":h>maxH?"#0D9488":"#0F766E",
                border:`1px solid ${sessionH===h?color:h>maxH?"#99F6E4":"#0D9488"}`,
                borderRadius:4,padding:"2px 0",fontSize:10,cursor:h>maxH?"not-allowed":"pointer",
                fontWeight:sessionH===h?700:400,opacity:h>maxH?0.4:1}}>
              {h}
            </button>
          ))}
        </div>
      </div>

      {/* Theory trainer */}
      {(isTheory || (hasBoth && sessionType==="theory")) && (
        <div style={{marginBottom:3}}>
          <div style={{color:"#0E7490",fontSize:9,marginBottom:1}}>👨‍🏫 مدرب النظري:</div>
          <select value={cell.theoryTrainerId||""}
            onChange={e=>onUpdate({theoryTrainerId:e.target.value||null})}
            style={{width:"100%",background:"#E0FDF9",border:"1px solid #A5F3FC",borderRadius:4,
              color:"#1E40AF",fontSize:9,padding:"2px 4px"}}>
            <option value="">-- اختر مدرباً --</option>
            {avTr.map(t=><option key={t.id} value={t.id}>✅ {t.name}</option>)}
            {busyTr.length>0&&<option disabled>─── في إجازة ───</option>}
            {busyTr.map(t=><option key={t.id} value={t.id}>🏖️ {t.name}</option>)}
          </select>
        </div>
      )}

      {/* Practical trainer */}
      {(!isTheory && (sessionType==="practical" || !hasBoth)) && (
        <div style={{marginBottom:3}}>
          <div style={{color:"#065F46",fontSize:9,marginBottom:1}}>🔧 مدرب العملي:</div>
          <select value={cell.practicalTrainerId||""}
            onChange={e=>onUpdate({practicalTrainerId:e.target.value||null})}
            style={{width:"100%",background:"#ECFDF5",border:"1px solid #6EE7B7",borderRadius:4,
              color:"#065F46",fontSize:9,padding:"2px 4px"}}>
            <option value="">-- اختر مدرباً --</option>
            {avTr.map(t=><option key={t.id} value={t.id}>✅ {t.name}</option>)}
            {busyTr.length>0&&<option disabled>─── في إجازة ───</option>}
            {busyTr.map(t=><option key={t.id} value={t.id}>🏖️ {t.name}</option>)}
          </select>
        </div>
      )}

      {/* Hall */}
      <div>
        <div style={{color:"#0F766E",fontSize:9,marginBottom:1}}>🏛 القاعة:</div>
        <select value={cell.hallId||""}
          onChange={e=>onUpdate({hallId:e.target.value||null})}
          style={{width:"100%",background:"#F0FDFA",border:"1px solid #CBD5E1",borderRadius:4,
            color:"#0F766E",fontSize:9,padding:"2px 4px"}}>
          <option value="">-- اختر قاعة --</option>
          {halls.filter(h=>h.specs.includes("all")||h.specs.includes(mainSpec)).map(h=>(
            <option key={h.id} value={h.id}>{h.name} ({h.cap})</option>
          ))}
        </select>
      </div>

      {/* ── Sharing section ── */}
      {!cell.isSharedCopy && onAddShared && (
        <div style={{marginTop:5,borderTop:"1px solid #E2E8F0",paddingTop:5}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
            <span style={{color:"#0F766E",fontSize:9,fontWeight:700}}>🔗 مشترك مع رزمة؟</span>
            <div style={{display:"flex",gap:4}}>
              <button
                onClick={()=>onUpdate({showSharing:true})}
                style={{padding:"2px 8px",borderRadius:4,fontSize:8,fontWeight:700,cursor:"pointer",
                  background:cell.showSharing?"#0C4A6E":"#F0FDFA",
                  color:cell.showSharing?"#fff":"#0F766E",border:"1px solid #CBD5E1"}}>
                نعم
              </button>
              <button
                onClick={()=>onUpdate({showSharing:false,sharedGroupKeys:[]})}
                style={{padding:"2px 8px",borderRadius:4,fontSize:8,fontWeight:700,cursor:"pointer",
                  background:cell.showSharing===false?"#F0FDFA":"transparent",
                  color:"#0F766E",border:"1px solid #CBD5E1"}}>
                لا
              </button>
            </div>
          </div>

          {cell.showSharing && allGroups && allGroups.length > 0 && (
            <div style={{background:"#FFFBEB",border:"1px solid #F59E0B",borderRadius:6,padding:6}}>
              <div style={{color:"#92400E",fontSize:8,fontWeight:700,marginBottom:4}}>
                اختر الرزم المشتركة:
              </div>
              <div style={{maxHeight:120,overflowY:"auto"}}>
                {allGroups.map(g=>{
                  const selected=(cell.sharedGroupKeys||[]).includes(g.key);
                  return (
                    <label key={g.key} style={{display:"flex",alignItems:"center",gap:6,
                      marginBottom:4,cursor:g.exists?"not-allowed":"pointer",opacity:g.exists?0.5:1}}>
                      <input type="checkbox"
                        checked={selected}
                        disabled={!!g.exists}
                        onChange={()=>{
                          const keys=cell.sharedGroupKeys||[];
                          const newKeys=selected?keys.filter(k=>k!==g.key):[...keys,g.key];
                          onUpdate({sharedGroupKeys:newKeys});
                        }}/>
                      <span style={{fontSize:8,color:"#0F766E"}}>{g.label}</span>
                      {g.exists&&<span style={{fontSize:7,color:"#F59E0B",fontWeight:700}}>موجود</span>}
                    </label>
                  );
                })}
              </div>
              {(cell.sharedGroupKeys||[]).length>0&&(
                <button
                  onClick={()=>onAddShared(cell.sharedGroupKeys||[])}
                  style={{width:"100%",marginTop:5,padding:"4px",background:"#10B981",color:"#fff",
                    border:"none",borderRadius:5,fontSize:9,fontWeight:700,cursor:"pointer"}}>
                  ✅ نسخ للرزم المختارة ({(cell.sharedGroupKeys||[]).length})
                </button>
              )}
            </div>
          )}

          {/* Show existing shared copies */}
          {isShared&&(
            <div style={{marginTop:4,background:"#E0FDF9",border:"1px solid #BFDBFE",
              borderRadius:5,padding:"3px 6px"}}>
              <div style={{color:"#0E7490",fontSize:8,fontWeight:700}}>🔗 منسوخ لـ:</div>
              {sharedWith.map((sw,i)=>(
                <div key={i} style={{color:"#0891B2",fontSize:8}}>{sw}</div>
              ))}
            </div>
          )}
        </div>
      )}
    </div>
  );
}

// ══════════════════════════════════════════════════════════════
// PERIOD SETTINGS MODAL
// ══════════════════════════════════════════════════════════════
function PeriodSettingsModal({periods,setPeriods,onClose}) {
  const [local,setLocal] = useState(periods.map(p=>({...p,breakAfter:p.breakAfter?{...p.breakAfter}:null})));

  const update = (idx,field,val) => setLocal(p=>p.map((x,i)=>i===idx?{...x,[field]:val}:x));
  const updateBreak = (idx,field,val) => setLocal(p=>p.map((x,i)=>i===idx?{...x,breakAfter:x.breakAfter?{...x.breakAfter,[field]:val}:null}:x));
  const toggleBreak = (idx) => setLocal(p=>p.map((x,i)=>i===idx?{...x,breakAfter:x.breakAfter?null:{label:"استراحة",dur:10,from:x.end,to:x.end}}:x));

  return (
    <Modal title="⚙️ إعدادات أوقات الحصص" onClose={onClose} wide>
      <div style={{background:"#FFFFFF",borderRadius:10,padding:12,marginBottom:14}}>
        <div style={{color:"#0F766E",fontSize:11,marginBottom:8}}>
          🕐 كل حصة = <strong style={{color:"#f59e0b"}}>50 دقيقة</strong> &nbsp;|&nbsp;
          بداية اليوم: <strong style={{color:"#0ea5e9"}}>07:30</strong> &nbsp;|&nbsp;
          نهاية اليوم: <strong style={{color:"#0ea5e9"}}>14:20</strong> &nbsp;|&nbsp;
          الحصة الثالثة تبدأ: <strong style={{color:"#0ea5e9"}}>09:15</strong>
        </div>
        <div style={{color:"#0F766E",fontSize:10}}>
          ⚠️ النظري: لا يمكن تجاوز 3 حصص متتالية &nbsp;|&nbsp; العملي: يُسمح بـ 4 حصص متتالية
        </div>
      </div>

      <div style={{overflowX:"auto"}}>
        <table style={{borderCollapse:"collapse",width:"100%",fontSize:12}}>
          <thead>
            <tr style={{background:"#F0FDFA"}}>
              {["الحصة","من","إلى","استراحة بعدها","من","إلى","المدة"].map(h=>(
                <th key={h} style={{color:"#0F766E",padding:"8px 10px",border:"1px solid #1e2d45",textAlign:"center"}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {local.map((p,i)=>(
              <>
                <tr key={p.id} style={{background:i%2===0?"#0C4A6E":"#FFFFFF"}}>
                  <td style={{padding:"8px 10px",border:"1px solid #1e2d45",textAlign:"center",color:"#0ea5e9",fontWeight:700}}>
                    الحصة {p.label}
                  </td>
                  <td style={{padding:"6px 8px",border:"1px solid #1e2d45"}}>
                    <input type="time" value={p.start} onChange={e=>update(i,"start",e.target.value)}
                      style={{background:"#FFFFFF",border:"1px solid #1e3a5f",borderRadius:6,padding:"4px 8px",color:"#134E4A",fontSize:12,direction:"ltr"}}/>
                  </td>
                  <td style={{padding:"6px 8px",border:"1px solid #1e2d45"}}>
                    <input type="time" value={p.end} onChange={e=>update(i,"end",e.target.value)}
                      style={{background:"#FFFFFF",border:"1px solid #1e3a5f",borderRadius:6,padding:"4px 8px",color:"#134E4A",fontSize:12,direction:"ltr"}}/>
                  </td>
                  <td style={{padding:"6px 8px",border:"1px solid #1e2d45",textAlign:"center"}}>
                    <button onClick={()=>toggleBreak(i)}
                      style={{background:p.breakAfter?"#065F46":"#FFFFFF",border:`1px solid ${p.breakAfter?"#10b981":"#99F6E4"}`,
                        color:p.breakAfter?"#6ee7b7":"#0F766E",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:11}}>
                      {p.breakAfter?"✓ يوجد":"+ أضف"}
                    </button>
                  </td>
                  {p.breakAfter ? (<>
                    <td style={{padding:"6px 8px",border:"1px solid #1e2d45"}}>
                      <input type="time" value={p.breakAfter.from} onChange={e=>updateBreak(i,"from",e.target.value)}
                        style={{background:"#FFFFFF",border:"1px solid #064e3b",borderRadius:6,padding:"4px 8px",color:"#6ee7b7",fontSize:12,direction:"ltr"}}/>
                    </td>
                    <td style={{padding:"6px 8px",border:"1px solid #1e2d45"}}>
                      <input type="time" value={p.breakAfter.to} onChange={e=>updateBreak(i,"to",e.target.value)}
                        style={{background:"#FFFFFF",border:"1px solid #064e3b",borderRadius:6,padding:"4px 8px",color:"#6ee7b7",fontSize:12,direction:"ltr"}}/>
                    </td>
                    <td style={{padding:"6px 8px",border:"1px solid #1e2d45",textAlign:"center"}}>
                      <input type="number" value={p.breakAfter.dur} min={5} max={120}
                        onChange={e=>updateBreak(i,"dur",+e.target.value)}
                        style={{width:55,background:"#FFFFFF",border:"1px solid #064e3b",borderRadius:6,padding:"4px 6px",color:"#6ee7b7",fontSize:12,textAlign:"center"}}/>
                      <span style={{color:"#0F766E",fontSize:10,marginRight:3}}>د</span>
                    </td>
                  </>) : (<>
                    <td style={{border:"1px solid #1e2d45",background:"#F0FDFA"}} colSpan={3}/>
                  </>)}
                </tr>
                {p.breakAfter && (
                  <tr key={`br-${p.id}`} style={{background:"#F0FDF4"}}>
                    <td colSpan={7} style={{padding:"4px 16px",border:"1px solid #1e2d45"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <span style={{color:"#10b981",fontSize:10}}>🕌 اسم الاستراحة:</span>
                        <input value={p.breakAfter.label} onChange={e=>updateBreak(i,"label",e.target.value)}
                          style={{background:"#FFFFFF",border:"1px solid #064e3b",borderRadius:6,padding:"3px 10px",
                            color:"#6ee7b7",fontSize:11,width:200}}/>
                        <span style={{color:"#065F46",fontSize:10}}>{p.breakAfter.from} — {p.breakAfter.to} ({p.breakAfter.dur} دقيقة)</span>
                      </div>
                    </td>
                  </tr>
                )}
              </>
            ))}
          </tbody>
        </table>
      </div>

      <div style={{display:"flex",gap:10,marginTop:16}}>
        <Btn onClick={()=>{setPeriods(local);onClose();}} bg="#10b981" full>💾 حفظ الإعدادات</Btn>
        <Btn onClick={()=>{setLocal(DEFAULT_PERIOD_CONFIG.map(p=>({...p,breakAfter:p.breakAfter?{...p.breakAfter}:null})));}} bg="#99F6E4" outline>↺ إعادة الضبط</Btn>
      </div>
    </Modal>
  );
}

// ══════════════════════════════════════════════════════════════
// COURSE ASSIGNMENT MODAL (assign trainer per course)
// ══════════════════════════════════════════════════════════════
function CourseAssignModal({courses,trainers,assignments,setAssignments,onClose}) {
  const [filterSpec,setFilterSpec] = useState("all");
  const vis = courses.filter(c=>!c.coop&&(filterSpec==="all"||c.spec===filterSpec));
  const spColor = id => SPECIALIZATIONS.find(s=>s.id===id)?.color||"#0F766E";
  const spName  = id => SPECIALIZATIONS.find(s=>s.id===id)?.name||id;

  const getAssign = (cid) => assignments[cid] || {theoryId:null,practicalId:null};
  const setAssign = (cid,field,val) => setAssignments(p=>({...p,[cid]:{...getAssign(cid),[field]:val}}));

  return (
    <Modal title="📋 تخصيص المدربين للمقررات" onClose={onClose} wide>
      <div style={{background:"#FFFFFF",borderRadius:8,padding:10,marginBottom:14,fontSize:11,color:"#0F766E",lineHeight:1.8}}>
        هنا يمكنك تخصيص مدرب ثابت لكل مقرر (نظري و/أو عملي). عند وضع المقرر في الجدول، سيُعبأ المدرب تلقائياً.
        يمكن أن يكون مدرب النظري مختلفاً عن مدرب العملي.
      </div>
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
        {[{id:"all",name:"الكل",color:"#0F766E"},...SPECIALIZATIONS].map(s=>(
          <button key={s.id} onClick={()=>setFilterSpec(s.id)}
            style={{padding:"5px 12px",borderRadius:7,border:`1px solid ${s.color}55`,
              background:filterSpec===s.id?s.color:"transparent",color:filterSpec===s.id?"#fff":s.color,
              cursor:"pointer",fontSize:11,fontWeight:600}}>
            {s.name}
          </button>
        ))}
      </div>
      <div style={{overflowX:"auto"}}>
        <table style={{borderCollapse:"collapse",width:"100%",fontSize:12}}>
          <thead>
            <tr style={{background:"#F0FDFA"}}>
              {["التخصص","الفصل","الرمز","اسم المقرر","نوع الساعات","مدرب النظري","مدرب العملي"].map(h=>(
                <th key={h} style={{color:"#0F766E",padding:"8px 10px",border:"1px solid #1e2d45",textAlign:"center",whiteSpace:"nowrap"}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {vis.map(c=>{
              const color=spColor(c.spec);
              const a=getAssign(c.id);
              const avTr=trainers.filter(t=>t.specs.includes(c.spec)||t.specs.includes("gen"));
              const isTheory=c.P===0; const hasBoth=c.L>0&&c.P>0; const isPract=c.L===0&&c.T===0;
              return (
                <tr key={c.id} style={{borderBottom:"1px solid #1a2535"}}>
                  <td style={{padding:"7px 10px",border:"1px solid #1a2535"}}>
                    <Badge color={color}>{spName(c.spec)}</Badge>
                  </td>
                  <td style={{padding:"7px 10px",textAlign:"center",color:"#0F766E",border:"1px solid #1a2535"}}>{c.level}</td>
                  <td style={{padding:"7px 10px",color,fontWeight:700,fontFamily:"monospace",fontSize:11,border:"1px solid #1a2535"}}>{c.code}</td>
                  <td style={{padding:"7px 10px",color:"#134E4A",border:"1px solid #1a2535"}}>{c.name}</td>
                  <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}>
                    <div style={{display:"flex",gap:3,justifyContent:"center"}}>
                      {c.L>0&&<Badge color="#3b82f6">مح:{c.L}</Badge>}
                      {c.T>0&&<Badge color="#8b5cf6">مت:{c.T}</Badge>}
                      {c.P>0&&<Badge color="#10b981">عم:{c.P}</Badge>}
                    </div>
                  </td>
                  <td style={{padding:"6px 8px",border:"1px solid #1a2535"}}>
                    {(!isPract) ? (
                      <select value={a.theoryId||""} onChange={e=>setAssign(c.id,"theoryId",e.target.value||null)}
                        style={{width:"100%",background:"#FFFFFF",border:"1px solid #1e40af",borderRadius:6,
                          color:"#0E7490",fontSize:11,padding:"4px 6px"}}>
                        <option value="">-- غير محدد --</option>
                        {avTr.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
                      </select>
                    ) : <span style={{color:"#99F6E4",fontSize:10}}>لا يوجد نظري</span>}
                  </td>
                  <td style={{padding:"6px 8px",border:"1px solid #1a2535"}}>
                    {c.P > 0 ? (
                      <select value={a.practicalId||""} onChange={e=>setAssign(c.id,"practicalId",e.target.value||null)}
                        style={{width:"100%",background:"#FFFFFF",border:"1px solid #064e3b",borderRadius:6,
                          color:"#6ee7b7",fontSize:11,padding:"4px 6px"}}>
                        <option value="">-- غير محدد --</option>
                        {avTr.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
                      </select>
                    ) : <span style={{color:"#99F6E4",fontSize:10}}>لا يوجد عملي</span>}
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
      <div style={{marginTop:14,display:"flex",justifyContent:"flex-end"}}>
        <Btn onClick={onClose} bg="#10b981">✓ حفظ التخصيصات</Btn>
      </div>
    </Modal>
  );
}

// ══════════════════════════════════════════════════════════════
// MAIN APP
// ══════════════════════════════════════════════════════════════
// Halls Excel template base64
const HALLS_TEMPLATE_B64 = "";

// Trainers Excel template base64
const TRAINERS_TEMPLATE_B64 = "";

// Logo base64
const LOGO_B64 = "data:image/jpeg;base64,";

export default function App() {
  const [tab,setTab]             = useState("dashboard");
  const [collegeName,setCollegeName] = useState("");
  const [entered,setEntered]     = useState(false);
  const [adminView,setAdminView] = useState(false);
  const [users,setUsers]         = useState([]);
  const ADMIN_PASS = "tvtc@admin2025";

  // Log user session on entry
  const logEntry = async (college) => {
    try {
      const sessionKey = `session-${Date.now()}`;
      const session = {
        college,
        time: new Date().toISOString(),
        date: new Date().toLocaleDateString("ar-SA"),
        timeStr: new Date().toLocaleTimeString("ar-SA"),
      };
      await window.storage.set(sessionKey, JSON.stringify(session), true);
    } catch(e) {}
  };

  // Load all sessions for admin
  const loadUsers = async () => {
    try {
      const result = await window.storage.list("session-", true);
      const keys = result?.keys || [];
      const loaded = await Promise.all(
        keys.map(async k => {
          try {
            const r = await window.storage.get(k, true);
            return r ? JSON.parse(r.value) : null;
          } catch { return null; }
        })
      );
      setUsers(loaded.filter(Boolean).sort((a,b)=>b.time.localeCompare(a.time)));
    } catch(e) { setUsers([]); }
  };
  const [courses,setCourses]     = useState(ALL_COURSES);
  const [trainers,setTrainers]   = useState(INIT_TRAINERS);
  const [halls,setHalls]         = useState(INIT_HALLS);
  const [trainees,setTrainees]   = useState([]);
  // scheds: { spec: { level: { groupId: cells[] } } }
  const [scheds,setScheds]       = useState({net:{},auto:{},gen:{}});
  // groups: { spec: { level: [{id, name}] } }
  const [groups,setGroups]       = useState({net:{},auto:{},gen:{}});
  const [selSpec,setSelSpec]     = useState("net");
  const [selLv,setSelLv]         = useState(1);
  const [selGroup,setSelGroup]   = useState(null); // active group id
  const [viewSpec,setViewSpec]   = useState("net");
  const [viewMode,setViewMode]   = useState("trainees");
  const [modal,setModal]         = useState(null);
  const [importTxt,setImportTxt] = useState("");
  const [periods,setPeriods]     = useState(DEFAULT_PERIOD_CONFIG);
  const [courseAssign,setCourseAssign] = useState({});
  const [warn,setWarn]           = useState(()=>{return null;});
  const [quotaCounts,setQuotaCounts] = useState(()=>{
    try{ const s=localStorage.getItem('tvtc_quotaCounts'); return s?JSON.parse(s):{};} catch{return {};}
  });
  const [theoryCap,setTheoryCap] = useState(()=>{
    try{ return +localStorage.getItem('tvtc_theoryCap')||35;} catch{return 35;}
  });
  const [practCap,setPractCap]   = useState(()=>{
    try{ return +localStorage.getItem('tvtc_practCap')||20;} catch{return 20;}
  });

  // ── Auto-save to localStorage whenever key state changes ──
  const { useEffect } = React;

  useEffect(()=>{
    try{
      const data = {
        courses, trainers, halls, trainees,
        scheds, groups, courseAssign, periods,
        quotaCounts, theoryCap, practCap,
        collegeName, savedAt: new Date().toISOString()
      };
      localStorage.setItem('tvtc_autosave', JSON.stringify(data));
    } catch(e){}
  }, [courses, trainers, halls, scheds, groups, courseAssign,
      periods, quotaCounts, theoryCap, practCap, collegeName]);

  // ── Auto-load on first render if no data entered yet ──────
  useEffect(()=>{
    try{
      const saved = localStorage.getItem('tvtc_autosave');
      if(!saved) return;
      const data = JSON.parse(saved);
      if(data.courses)    setCourses(data.courses);
      if(data.trainers)   setTrainers(data.trainers);
      if(data.halls)      setHalls(data.halls);
      if(data.trainees)   setTrainees(data.trainees);
      if(data.scheds)     setScheds(data.scheds);
      if(data.groups)     setGroups(data.groups);
      if(data.courseAssign) setCourseAssign(data.courseAssign);
      if(data.periods)    setPeriods(data.periods);
      if(data.quotaCounts) setQuotaCounts(data.quotaCounts);
      if(data.theoryCap)  setTheoryCap(data.theoryCap);
      if(data.practCap)   setPractCap(data.practCap);
      if(data.collegeName) { setCollegeName(data.collegeName); setEntered(true); }
    } catch(e){}
  }, []);

  const spColor = id => SPECIALIZATIONS.find(s=>s.id===id)?.color||"#0F766E";
  const spName  = id => SPECIALIZATIONS.find(s=>s.id===id)?.name||id;

  // ── Group helpers ─────────────────────────────────────────
  const getGroups = (sp,lv) => (groups[sp]&&groups[sp][lv]) || [];

  const ensureGroup = (sp,lv) => {
    // If no groups exist for this spec/level, auto-create "رزمة 1"
    const existing = (groups[sp]&&groups[sp][lv]) || [];
    if (existing.length === 0) {
      const g = { id: uid(), name: "رزمة 1" };
      setGroups(p=>({...p,[sp]:{...p[sp],[lv]:[g]}}));
      setSelGroup(g.id);
      return g.id;
    }
    return existing[0].id;
  };

  const addGroup = (sp,lv,name) => {
    const g = { id: uid(), name };
    setGroups(p=>({...p,[sp]:{...p[sp],[lv]:[...(p[sp]&&p[sp][lv]||[]),g]}}));
    setSelGroup(g.id);
  };

  const removeGroup = (sp,lv,gid) => {
    setGroups(p=>({...p,[sp]:{...p[sp],[lv]:(p[sp]&&p[sp][lv]||[]).filter(g=>g.id!==gid)}}));
    setScheds(p=>({...p,[sp]:{...p[sp],[lv]:Object.fromEntries(Object.entries((p[sp]&&p[sp][lv])||{}).filter(([k])=>k!==gid))}}));
    const remaining = (groups[sp]&&groups[sp][lv]||[]).filter(g=>g.id!==gid);
    setSelGroup(remaining.length>0?remaining[0].id:null);
  };

  const renameGroup = (sp,lv,gid,name) => {
    setGroups(p=>({...p,[sp]:{...p[sp],[lv]:(p[sp]&&p[sp][lv]||[]).map(g=>g.id===gid?{...g,name}:g)}}));
  };

  // ── Schedule helpers ──────────────────────────────────────
  const getSched = (sp,lv,gid) => (scheds[sp]&&scheds[sp][lv]&&scheds[sp][lv][gid]) || [];
  const setSched = (sp,lv,gid,fn) => setScheds(p=>{
    const cur = (p[sp]&&p[sp][lv]&&p[sp][lv][gid]) || [];
    return {...p,[sp]:{...p[sp],[lv]:{...((p[sp]&&p[sp][lv])||{}),[gid]:typeof fn==="function"?fn(cur):fn}}};
  });

  // Active group for current spec/level (auto-ensure)
  const activeGroup = () => {
    const gs = getGroups(selSpec,selLv);
    if (gs.length===0) return null;
    if (selGroup && gs.find(g=>g.id===selGroup)) return selGroup;
    return gs[0].id;
  };

  const ag = activeGroup();

  const lc = (sp,lv) => [
    ...courses.filter(c=>c.spec===sp&&c.level===lv&&!c.coop),
    ...(sp!=="gen"?courses.filter(c=>c.spec==="gen"&&c.level===lv):[]),
  ];

  // Used hours per trainer
  // Used hours per trainer — shared copies NOT counted (only source cells counted)
  const usedH = tid => Object.values(scheds).reduce((a,ls)=>
    a+Object.values(ls).reduce((b,lvGroups)=>
      b+Object.values(lvGroups).reduce((c,cells)=>
        c+cells
          .filter(x=>!x.isSharedCopy&&(x.theoryTrainerId===tid||x.practicalTrainerId===tid))
          .reduce((s,x)=>s+(x.sessionHours||1),0),0),0),0);

  // ── All cells across every spec/level/group ───────────────
  const getAllCells = () =>
    Object.entries(scheds).flatMap(([sp,ls])=>
      Object.entries(ls||{}).flatMap(([lv,lvGroups])=>
        Object.entries(lvGroups||{}).flatMap(([gid,cells])=>
          cells.map(c=>({...c,spec:sp,level:+lv,gid}))
        )
      )
    );

  // Returns list of group labels sharing the same courseId at the same day+period (excluding current cell)
  const getSharedWith = (cell, currentSpec, currentLv, currentGid) => {
    const all = getAllCells();
    return all
      .filter(x =>
        x.courseId === cell.courseId &&
        x.day      === cell.day &&
        x.period   === cell.period &&
        !(x.spec===currentSpec && x.level===currentLv && x.gid===currentGid)
      )
      .map(x => {
        const sp   = SPECIALIZATIONS.find(s=>s.id===x.spec);
        const grps = (groups[x.spec]&&groups[x.spec][x.level])||[];
        const grp  = grps.find(g=>g.id===x.gid);
        return `${sp?.name||x.spec} م${x.level} ${grp?.name||""}`;
      });
  };

  // Returns ALL groups (across all specs) that have this courseId in their schedule
  const getAllGroupsForCourse = (courseId, currentSpec, currentLv, currentGid) => {
    const all = getAllCells();
    const seen = new Set();
    return all
      .filter(x => x.courseId === courseId &&
        !(x.spec===currentSpec && x.level===currentLv && x.gid===currentGid))
      .map(x => {
        const key = `${x.spec}-${x.level}-${x.gid}`;
        if(seen.has(key)) return null;
        seen.add(key);
        const sp   = SPECIALIZATIONS.find(s=>s.id===x.spec);
        const grps = (groups[x.spec]&&groups[x.spec][x.level])||[];
        const grp  = grps.find(g=>g.id===x.gid);
        const sessions = (scheds[x.spec]&&scheds[x.spec][x.level]&&scheds[x.spec][x.level][x.gid])||[];
        const courseSessions = sessions.filter(s=>s.courseId===courseId).length;
        return { label:`${sp?.name||x.spec} — م${x.level} — ${grp?.name||""}`, count:courseSessions, key };
      }).filter(Boolean);
  };

  // Build available groups list for sharing (excluding current)
  const allAvailGroups = (cell, currentSpec, currentLv, currentGid) =>
    Object.entries(groups).flatMap(([sp,levels])=>
      Object.entries(levels||{}).flatMap(([lv2,gs])=>
        (gs||[]).map(g=>{
          const key=`${sp}-${lv2}-${g.id}`;
          const existing=(scheds[sp]&&scheds[sp][+lv2]&&scheds[sp][+lv2][g.id]||[])
            .find(s=>s.courseId===cell.courseId&&s.day===cell.day&&s.period===cell.period);
          return {
            sp, lv:+lv2, gid:g.id, name:g.name,
            label:`${spName(sp)} — الفصل ${lv2} — ${g.name}`,
            key, exists:!!existing
          };
        })
      )
    ).filter(g=>!(g.sp===currentSpec&&g.lv===currentLv&&g.gid===currentGid));

  // Copy a cell to selected groups (shared copy — not counted in trainer hours)
  const addSharedCopies = (cell, fromSpec, fromLv, fromGid, keys) => {
    const course = courses.find(c=>c.id===cell.courseId);
    const hasBoth = course && course.L>0 && course.P>0;

    // Tag source cell with sourceId = its own id (marks it as head of family)
    setSched(fromSpec,fromLv,fromGid,p=>p.map(s=>s.id===cell.id
      ? {...s, sourceId:cell.id, sharedGroupKeys:[], showSharing:false}
      : s
    ));

    keys.forEach(key=>{
      const [sp,lv,gid] = key.split('-');
      const grps=(groups[sp]&&groups[sp][+lv])||[];
      const grp=grps.find(g=>g.id===gid);

      const baseCell = {
        ...cell,
        isSharedCopy: true,
        sourceId: cell.id,   // link back to source
        showSharing: false,
        sharedGroupKeys: [],
      };

      if(hasBoth) {
        // Mixed course: TWO separate copies — theory + practical
        setSched(sp,+lv,gid,p=>[...p,{
          ...baseCell,
          id: uid(),
          sessionType: "theory",
          sessionHours: course.L + course.T,
          practicalTrainerId: null,
        }]);
        setSched(sp,+lv,gid,p=>[...p,{
          ...baseCell,
          id: uid(),
          sessionType: "practical",
          sessionHours: course.P,
          theoryTrainerId: null,
        }]);
      } else {
        setSched(sp,+lv,gid,p=>[...p,{ ...baseCell, id: uid() }]);
      }
    });
  };

  // Check trainer conflict: same trainer, same day, same period (across ALL groups/specs)
  const checkTrainerConflict = (trainerId, day, period, excludeId=null) => {
    if (!trainerId) return null;
    const all = getAllCells();
    const currentCell = excludeId ? all.find(c=>c.id===excludeId) : null;
    const currentSourceId = currentCell?.sourceId || (currentCell?.isSharedCopy ? null : currentCell?.id);

    const conflict = all.find(c => {
      if(c.id === excludeId) return false;
      if(c.day !== day || c.period !== period) return false;
      if(c.theoryTrainerId !== trainerId && c.practicalTrainerId !== trainerId) return false;
      // Skip if same sharing family (same sourceId)
      const cSourceId = c.sourceId || (c.isSharedCopy ? null : c.id);
      if(currentSourceId && cSourceId && currentSourceId === cSourceId) return false;
      return true;
    });
    if (!conflict) return null;
    const trName = trainers.find(t=>t.id===trainerId)?.name || "";
    const crsName = courses.find(x=>x.id===conflict.courseId)?.code || "";
    const grpName = Object.values(groups).flatMap(ls=>Object.values(ls||{})).flat()
      .find(g=>g.id===conflict.gid)?.name || "";
    return `المدرب "${trName}" لديه حصة (${crsName}) في نفس الوقت${grpName?" — "+grpName:""}`;
  };

  const checkHallConflict = (hallId, day, period, excludeId=null) => {
    if (!hallId) return null;
    const all = getAllCells();
    const currentCell = excludeId ? all.find(c=>c.id===excludeId) : null;
    const currentSourceId = currentCell?.sourceId || (currentCell?.isSharedCopy ? null : currentCell?.id);

    const conflict = all.find(c => {
      if(c.id === excludeId) return false;
      if(c.day !== day || c.period !== period) return false;
      if(c.hallId !== hallId) return false;
      // Skip if same sharing family
      const cSourceId = c.sourceId || (c.isSharedCopy ? null : c.id);
      if(currentSourceId && cSourceId && currentSourceId === cSourceId) return false;
      return true;
    });
    if (!conflict) return null;
    const hallName = halls.find(h=>h.id===hallId)?.name || "";
    const crsName  = courses.find(x=>x.id===conflict.courseId)?.code || "";
    const grpName  = Object.values(groups).flatMap(ls=>Object.values(ls||{})).flat()
      .find(g=>g.id===conflict.gid)?.name || "";
    return `القاعة "${hallName}" محجوزة (${crsName}) في نفس الوقت${grpName?" — "+grpName:""}`;
  };

  // Drop handler with ALL validations
  const handleDrop = useCallback((e, day, period, spec, lv, gid) => {
    e.preventDefault();
    const cid = e.dataTransfer.getData("courseId");
    const mv  = e.dataTransfer.getData("moveCell");
    const sched = getSched(spec,lv,gid);

    if (mv) {
      const o = JSON.parse(mv);
      // Validate trainer/hall on move
      const cell = getAllCells().find(c=>c.id===o.id);
      if (cell) {
        const tConflict = checkTrainerConflict(cell.theoryTrainerId, day, period, o.id)
                       || checkTrainerConflict(cell.practicalTrainerId, day, period, o.id);
        const hConflict = checkHallConflict(cell.hallId, day, period, o.id);
        if (tConflict || hConflict) {
          setWarn("⚠️ تعارض عند النقل: " + (tConflict||hConflict));
          setTimeout(()=>setWarn(null),5000);
          return;
        }
      }
      setSched(spec,lv,gid,p=>p.map(s=>s.id===o.id?{...s,day,period}:s));

    } else if (cid) {
      if (sched.find(s=>s.day===day&&s.period===period)) return;
      const course = courses.find(c=>c.id===cid);

      // 1) Consecutive theory check
      if (course && isPurelyTheory(course)) {
        if (wouldExceedConsecutiveTheory(sched, courses, day, period)) {
          setWarn("⚠️ لا يمكن وضع 4 حصص نظرية متتالية. الحد الأقصى 3 حصص نظرية متتالية.");
          setTimeout(()=>setWarn(null),5000);
          return;
        }
      }

      // 2) Pre-assigned trainer conflict check
      // Check off-days for pre-assigned trainers
      const assign = courseAssign[cid] || {};
      const checkOffDay = (tid, day, period) => {
        if(!tid) return null;
        const tr = trainers.find(t=>t.id===tid);
        if(!tr) return null;
        if((tr.offDays||[]).includes(day))
          return `المدرب "${tr.name}" مفرّغ يوم ${day} بالكامل`;
        const slot = (tr.offSlots||[]).find(s=>s.day===day);
        if(slot&&slot.periods.includes(period))
          return `المدرب "${tr.name}" مفرّغ في الحصة ${period} يوم ${day}`;
        return null;
      };
      const offWarn = checkOffDay(assign.theoryId,day,period)
                   || checkOffDay(assign.practicalId,day,period);
      if(offWarn){
        setWarn("🏖️ "+offWarn);
        setTimeout(()=>setWarn(null),5000);
        // Still allow placing — just warn
      }

      const tConflict = checkTrainerConflict(assign.theoryId, day, period)
                     || checkTrainerConflict(assign.practicalId, day, period);
      if (tConflict) {
        setWarn("⚠️ تعارض مدرب: " + tConflict);
        setTimeout(()=>setWarn(null),5000);
        return;
      }

      const defaultType = course?.P>0&&course?.L>0?"theory":(course?.P>0?"practical":"theory");
      const newCell = {
        id:uid(), courseId:cid, day, period,
        theoryTrainerId: assign.theoryId||null,
        practicalTrainerId: assign.practicalId||null,
        hallId: null,
        sessionType: defaultType,
        sessionHours: 1,
        isSharedSource: false,
        sharedGroupKeys: [],
      };
      setSched(spec,lv,gid,p=>[...p,newCell]);
    }
  }, [scheds, courses, courseAssign, trainers, halls, groups]);

  const handleCellUpdate = (spec, lv, gid, cellId, upd) => {
    const cell = getSched(spec,lv,gid).find(s=>s.id===cellId);
    if (!cell) return;

    // Check if new sessionHours would exceed theory or practical limit independently
    if (upd.sessionHours !== undefined || upd.sessionType !== undefined) {
      const course = courses.find(c=>c.id===cell.courseId);
      if (course) {
        const newSessionH = upd.sessionHours ?? (cell.sessionHours||1);
        const newType     = upd.sessionType  ?? cell.sessionType ?? (course.P===0?"theory":"practical");
        const theoryNeeded = course.L + course.T;
        const practNeeded  = course.P;
        const otherSessions = getSched(spec,lv,gid).filter(s=>s.courseId===cell.courseId&&s.id!==cellId);
        const otherTh = otherSessions.filter(s=>{
          const st=s.sessionType||(course.P===0?"theory":"practical");
          return st==="theory"||course.P===0;
        }).reduce((a,s)=>a+(s.sessionHours||1),0);
        const otherPr = otherSessions.filter(s=>{
          const st=s.sessionType||(course.L===0&&course.T===0?"practical":"theory");
          return st==="practical"||(course.L===0&&course.T===0);
        }).reduce((a,s)=>a+(s.sessionHours||1),0);
        const isTheorySession = newType==="theory"||(course.P===0);
        const isPractSession  = newType==="practical"||(course.L===0&&course.T===0);
        if (isTheorySession && theoryNeeded>0 && otherTh+newSessionH>theoryNeeded) {
          setWarn(`⚠️ تجاوز النظري: "${course.name}" — النظري المتاح ${theoryNeeded}س، الموزع سيصبح ${otherTh+newSessionH}س`);
          setTimeout(()=>setWarn(null),5000);
        } else if (isPractSession && practNeeded>0 && otherPr+newSessionH>practNeeded) {
          setWarn(`⚠️ تجاوز العملي: "${course.name}" — العملي المتاح ${practNeeded}س، الموزع سيصبح ${otherPr+newSessionH}س`);
          setTimeout(()=>setWarn(null),5000);
        }
      }
    }

    // Trainer off-day check
    if (upd.theoryTrainerId !== undefined || upd.practicalTrainerId !== undefined) {
      const checkOff = (tid) => {
        if(!tid) return null;
        const tr = trainers.find(t=>t.id===tid);
        if(!tr) return null;
        if((tr.offDays||[]).includes(cell.day))
          return `🏖️ المدرب "${tr.name}" مفرّغ يوم ${cell.day} بالكامل`;
        const slot=(tr.offSlots||[]).find(s=>s.day===cell.day);
        if(slot&&slot.periods.includes(cell.period))
          return `🏖️ المدرب "${tr.name}" مفرّغ في هذه الحصة يوم ${cell.day}`;
        return null;
      };
      const ow = checkOff(upd.theoryTrainerId) || checkOff(upd.practicalTrainerId);
      if(ow){ setWarn(ow); setTimeout(()=>setWarn(null),5000); }
    }

    // Trainer conflict
    if (upd.theoryTrainerId !== undefined) {
      const c = checkTrainerConflict(upd.theoryTrainerId, cell.day, cell.period, cellId);
      if (c) { setWarn("⚠️ تعارض مدرب نظري: "+c); setTimeout(()=>setWarn(null),5000); return; }
    }
    if (upd.practicalTrainerId !== undefined) {
      const c = checkTrainerConflict(upd.practicalTrainerId, cell.day, cell.period, cellId);
      if (c) { setWarn("⚠️ تعارض مدرب عملي: "+c); setTimeout(()=>setWarn(null),5000); return; }
    }
    // Hall conflict
    if (upd.hallId !== undefined) {
      const c = checkHallConflict(upd.hallId, cell.day, cell.period, cellId);
      if (c) { setWarn("⚠️ تعارض قاعة: "+c); setTimeout(()=>setWarn(null),5000); return; }
    }

    setSched(spec,lv,gid,p=>p.map(s=>s.id===cellId?{...s,...upd}:s));

    // ── Sync shared copies ────────────────────────────────────
    // If this is a source cell and the update affects trainer/hall/refNumber,
    // propagate those changes to all shared copies (isSharedCopy=true, sourceId===cellId)
    const syncFields = ['theoryTrainerId','practicalTrainerId','hallId','refNumber'];
    const hasSyncField = syncFields.some(f=>upd[f]!==undefined);
    if(hasSyncField && !cell.isSharedCopy) {
      const syncUpd = Object.fromEntries(syncFields.filter(f=>upd[f]!==undefined).map(f=>[f,upd[f]]));
      // Find all copies across all scheds
      setScheds(prev => {
        const next = JSON.parse(JSON.stringify(prev));
        Object.entries(next).forEach(([sp,ls])=>
          Object.entries(ls||{}).forEach(([lv2,lvGroups])=>
            Object.entries(lvGroups||{}).forEach(([gid2,cells])=>{
              cells.forEach((c,i)=>{
                if(c.isSharedCopy && c.sourceId===cellId) {
                  next[sp][lv2][gid2][i] = {...c,...syncUpd};
                }
              });
            })
          )
        );
        return next;
      });
    }
  };

  const TABS = [
    {id:"dashboard",l:"لوحة التحكم",i:"📊"},
    {id:"courses",  l:"المقررات",   i:"📚"},
    {id:"trainers", l:"المدربون",  i:"👨‍🏫"},
    {id:"halls",    l:"القاعات",   i:"🏛"},
    {id:"trainees", l:"المتدربون", i:"👥"},
    {id:"schedule", l:"إعداد الجداول",i:"📅"},
    {id:"view",     l:"عرض الجداول",  i:"🗓"},
    {id:"stats",    l:"الإحصائيات",   i:"📈"},
    {id:"quota",    l:"النصاب التدريبي",i:"📋"},
  ];

  // ── Period display helper ─────────────────────────────────
  const PeriodLabel = ({pid}) => {
    const p = periods.find(x=>x.id===pid);
    if(!p) return null;
    return (
      <div style={{textAlign:"center"}}>
        <div style={{fontWeight:700,fontSize:11}}>الحصة {p.label}</div>
        <div style={{fontSize:9,color:"#0F766E",direction:"ltr"}}>{p.start}–{p.end}</div>
      </div>
    );
  };

  // ── Periods sidebar in grid ───────────────────────────────
  const PeriodSidebar = ({pid}) => {
    const p = periods.find(x=>x.id===pid);
    if(!p) return null;
    return (
      <td style={{background:"#F0FDFA",padding:"4px 6px",border:"1px solid #1e2d45",
        verticalAlign:"top",minWidth:88}}>
        <div style={{textAlign:"center"}}>
          <div style={{color:"#0ea5e9",fontWeight:700,fontSize:11}}>{p.label}</div>
          <div style={{color:"#0F766E",fontSize:9,direction:"ltr"}}>{p.start}</div>
          <div style={{color:"#0F766E",fontSize:9,direction:"ltr"}}>{p.end}</div>
        </div>
      </td>
    );
  };

  // ── Break Row ─────────────────────────────────────────────
  const BreakRow = ({breakInfo}) => (
    <tr>
      <td colSpan={6} style={{background:"#F0FDF4",border:"1px solid #1e2d45",padding:"4px 12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{color:"#10b981",fontSize:10}}>🕌 {breakInfo.label}</span>
          <span style={{color:"#065F46",fontSize:10,direction:"ltr"}}>{breakInfo.from} — {breakInfo.to}</span>
          <span style={{color:"#065F46",fontSize:10}}>({breakInfo.dur} دقيقة)</span>
        </div>
      </td>
    </tr>
  );

  // ── DASHBOARD ─────────────────────────────────────────────
  const Dashboard = () => {
    const [showEnrollImport, setShowEnrollImport] = useState(false);
    const [showPlanImport,   setShowPlanImport]   = useState(false);
    const [showLimits,       setShowLimits]       = useState(false);
    const [localTheory,      setLocalTheory]      = useState(theoryCap);
    const [localPract,       setLocalPract]       = useState(practCap);

    // Parse pasted enrolment data and fill quotaCounts
    const importEnrolment = (text, codeCol, countCol, sep) => {
      const lines2 = text.split("\n").filter(l=>l.trim());
      let matched=0; const notFound=[];
      const newCounts = {...quotaCounts};
      lines2.forEach(line=>{
        const cols = line.split(sep==="tab"?"\t":sep).map(c=>c.trim().replace(/\r/,""));
        const rawCode  = (cols[+codeCol]||"").replace(/\s/g,"").toLowerCase();
        const rawCount = (cols[+countCol]||"").replace(/[^\d]/g,"");
        const count = parseInt(rawCount)||0;
        if(!rawCode||!count) return;
        const mc = courses.find(c=>
          c.code.replace(/\s/g,"").toLowerCase()===rawCode ||
          c.code.replace(/\s/g,"").toLowerCase().includes(rawCode) ||
          rawCode.includes(c.code.replace(/\s/g,"").toLowerCase())
        );
        if(mc){ newCounts[mc.id]=count; matched++; }
        else notFound.push(cols[+codeCol]||rawCode);
      });
      setQuotaCounts(newCounts);
      setTab("quota");
      alert(`✅ تم استيراد ${matched} مقرر\n${notFound.length>0?"⚠️ لم يُعثر على:\n"+notFound.slice(0,8).join(", "):""}`);
    };

    const totalCoursesWithCount = Object.values(quotaCounts).filter(v=>v>0).length;
    const totalEnrolled = Object.values(quotaCounts).reduce((a,v)=>a+v,0);

    return (
      <div>
        <h2 style={{color:"#042F2E",fontSize:19,marginBottom:20}}>الرئيسية</h2>

        {/* ── Quick Stats ── */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:20}}>
          {[
            {l:"مقررات الشبكات",  v:courses.filter(c=>c.spec==="net").length,  i:"🌐",c:"#0ea5e9",s:courses.filter(c=>c.spec==="net").reduce((a,c)=>a+c.CTH,0)+"س"},
            {l:"مقررات المركبات", v:courses.filter(c=>c.spec==="auto").length, i:"🔧",c:"#f97316",s:courses.filter(c=>c.spec==="auto").reduce((a,c)=>a+c.CTH,0)+"س"},
            {l:"مقررات مشتركة",  v:courses.filter(c=>c.spec==="gen").length,   i:"📖",c:"#8b5cf6",s:courses.filter(c=>c.spec==="gen").reduce((a,c)=>a+c.CTH,0)+"س"},
            {l:"المدربون",        v:trainers.length,                            i:"👨‍🏫",c:"#10b981",s:"الحد الأقصى "+Math.max(...trainers.map(t=>t.maxH),0)+"س"},
          ].map(s=>(
            <div key={s.l} style={{background:"#FFFFFF",border:`1px solid ${s.c}33`,borderRadius:13,padding:"14px 16px"}}>
              <div style={{fontSize:22,marginBottom:4}}>{s.i}</div>
              <div style={{color:s.c,fontSize:24,fontWeight:800}}>{s.v}</div>
              <div style={{color:"#0C4A6E",fontSize:11,fontWeight:600}}>{s.l}</div>
              <div style={{color:"#0F766E",fontSize:10,marginTop:2}}>{s.s}</div>
            </div>
          ))}
        </div>

        {/* ── Action Cards ── */}
        <div style={{display:"grid",gridTemplateColumns:"1fr",gap:12,marginBottom:20}}>

          {/* 1. Import Training Plan */}
          <div style={{background:"#FFFFFF",border:"2px solid #0E7490",borderRadius:14,padding:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:40,height:40,background:"#E0FDF9",borderRadius:10,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>📥</div>
                <div>
                  <div style={{color:"#042F2E",fontWeight:800,fontSize:14}}>استيراد الخطة التدريبية</div>
                  <div style={{color:"#0F766E",fontSize:11}}>استيراد المقررات من ملف Excel</div>
                </div>
              </div>
              <button onClick={()=>setShowPlanImport(p=>!p)}
                style={{padding:"7px 14px",borderRadius:9,border:"1px solid #0E7490",
                  background:showPlanImport?"#0E7490":"#E0FDF9",
                  color:showPlanImport?"#fff":"#0C4A6E",cursor:"pointer",fontSize:12,fontWeight:700}}>
                {showPlanImport?"▲ إخفاء":"▼ فتح"}
              </button>
            </div>
            {showPlanImport&&(
              <div style={{marginTop:14,padding:"14px",background:"#F0FDFA",borderRadius:10}}>
                <div style={{color:"#0C4A6E",fontSize:11,lineHeight:1.9,marginBottom:10}}>
                  الصق بيانات الخطة التدريبية من Excel (رمز المقرر، الاسم، الساعات...)
                </div>
                <textarea rows={5} placeholder={"ICMT 121\tتجميع الحاسب وتشغيله\t1\t0\t4\t5\nINET 111\tمبادئ شبكات الحاسب\t3\t0\t2\t5"}
                  id="plan-paste"
                  style={{width:"100%",border:"1px solid #A5F3FC",borderRadius:8,padding:10,
                    fontSize:10,fontFamily:"monospace",direction:"ltr",background:"#FFFFFF",
                    boxSizing:"border-box",resize:"vertical"}}/>
                <div style={{color:"#0F766E",fontSize:10,marginTop:6}}>
                  💡 الأعمدة المتوقعة: رمز المقرر | الاسم | محاضرة | تمارين | عملي | (إجمالي)
                </div>
                <button onClick={()=>{
                  const text=document.getElementById("plan-paste")?.value||"";
                  const lines2=text.split("\n").filter(l=>l.trim());
                  let added=0;
                  const newCourses=[...courses];
                  lines2.forEach(line=>{
                    const cols=line.split("\t").map(c=>c.trim().replace(/\r/,""));
                    if(!cols[0]) return;
                    const exists=newCourses.find(c=>c.code.replace(/\s/g,"")===cols[0].replace(/\s/g,""));
                    if(exists) return;
                    const L=parseInt(cols[2])||0, T=parseInt(cols[3])||0, P=parseInt(cols[4])||0;
                    newCourses.push({
                      id:"c_"+Date.now()+"_"+added,
                      code:cols[0], name:cols[1]||cols[0],
                      spec:"gen", level:1, L, T, P, CTH:L+T+P, coop:false
                    });
                    added++;
                  });
                  setCourses(newCourses);
                  setShowPlanImport(false);
                  alert(`✅ تمت إضافة ${added} مقرر جديد`);
                }} style={{marginTop:10,padding:"9px 18px",background:"#0E7490",color:"#fff",
                  border:"none",borderRadius:9,cursor:"pointer",fontSize:12,fontWeight:700}}>
                  ✅ استيراد المقررات
                </button>
              </div>
            )}
          </div>

          {/* 2. Import Expected Enrolment */}
          <div style={{background:"#FFFFFF",border:"2px solid #8B5CF6",borderRadius:14,padding:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:40,height:40,background:"#EDE9FE",borderRadius:10,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>👥</div>
                <div>
                  <div style={{color:"#042F2E",fontWeight:800,fontSize:14}}>استيراد التسجيل المتوقع</div>
                  <div style={{color:"#6D28D9",fontSize:11}}>
                    {totalCoursesWithCount>0
                      ?`${totalCoursesWithCount} مقرر — إجمالي ${totalEnrolled} متدرب`
                      :"استيراد أعداد المتدربين من Excel"}
                  </div>
                </div>
              </div>
              <button onClick={()=>setShowEnrollImport(p=>!p)}
                style={{padding:"7px 14px",borderRadius:9,border:"1px solid #8B5CF6",
                  background:showEnrollImport?"#8B5CF6":"#EDE9FE",
                  color:showEnrollImport?"#fff":"#6D28D9",cursor:"pointer",fontSize:12,fontWeight:700}}>
                {showEnrollImport?"▲ إخفاء":"▼ فتح"}
              </button>
            </div>
            {showEnrollImport&&(
              <div style={{marginTop:14,padding:14,background:"#FAF5FF",borderRadius:10}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                  <div>
                    <label style={{display:"block",color:"#6D28D9",fontSize:11,fontWeight:700,marginBottom:3}}>عمود رمز المقرر</label>
                    <select id="enr-code-col" defaultValue="0"
                      style={{width:"100%",border:"1px solid #DDD6FE",borderRadius:7,padding:"6px 8px",fontSize:11,background:"#FFFFFF"}}>
                      {["العمود 1","العمود 2","العمود 3","العمود 4","العمود 5"].map((l,i)=><option key={i} value={i}>{l}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{display:"block",color:"#6D28D9",fontSize:11,fontWeight:700,marginBottom:3}}>عمود عدد المتدربين</label>
                    <select id="enr-count-col" defaultValue="1"
                      style={{width:"100%",border:"1px solid #DDD6FE",borderRadius:7,padding:"6px 8px",fontSize:11,background:"#FFFFFF"}}>
                      {["العمود 1","العمود 2","العمود 3","العمود 4","العمود 5"].map((l,i)=><option key={i} value={i}>{l}</option>)}
                    </select>
                  </div>
                </div>
                <div style={{display:"flex",gap:12,marginBottom:8}}>
                  {[{v:"tab",l:"Tab"},{v:",",l:"فاصلة"},{v:";",l:"فاصلة منقوطة"}].map(s=>(
                    <label key={s.v} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:"#6D28D9",cursor:"pointer"}}>
                      <input type="radio" name="enr-sep" value={s.v} defaultChecked={s.v==="tab"}/>{s.l}
                    </label>
                  ))}
                </div>
                <textarea rows={6} id="enr-paste"
                  placeholder={"ICMT 121\t45\nINET 111\t38\nENGL 101\t72\n..."}
                  style={{width:"100%",border:"1px solid #DDD6FE",borderRadius:8,padding:10,
                    fontSize:11,fontFamily:"monospace",direction:"ltr",background:"#FFFFFF",
                    boxSizing:"border-box",resize:"vertical"}}/>
                <div style={{display:"flex",gap:8,marginTop:10}}>
                  <button onClick={()=>{
                    const text=document.getElementById("enr-paste")?.value||"";
                    const codeCol=document.getElementById("enr-code-col")?.value||"0";
                    const countCol=document.getElementById("enr-count-col")?.value||"1";
                    const sep=document.querySelector('input[name="enr-sep"]:checked')?.value||"tab";
                    importEnrolment(text,codeCol,countCol,sep);
                    setShowEnrollImport(false);
                  }} style={{flex:1,background:"#8B5CF6",color:"#fff",border:"none",borderRadius:9,
                    padding:"10px",cursor:"pointer",fontSize:12,fontWeight:700}}>
                    ✅ استيراد وتعبئة النصاب
                  </button>
                  <button onClick={()=>setShowEnrollImport(false)}
                    style={{padding:"10px 14px",background:"#EDE9FE",color:"#6D28D9",border:"none",
                      borderRadius:9,cursor:"pointer",fontSize:12}}>إلغاء</button>
                </div>
              </div>
            )}
          </div>

          {/* 3. Capacity Limits */}
          <div style={{background:"#FFFFFF",border:"2px solid #F97316",borderRadius:14,padding:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:40,height:40,background:"#FFF7ED",borderRadius:10,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>⚙️</div>
                <div>
                  <div style={{color:"#042F2E",fontWeight:800,fontSize:14}}>قيود الشعب التدريبية</div>
                  <div style={{color:"#C2410C",fontSize:11}}>نظري: {theoryCap} متدرب/شعبة | عملي: {practCap} متدرب/شعبة</div>
                </div>
              </div>
              <button onClick={()=>setShowLimits(p=>!p)}
                style={{padding:"7px 14px",borderRadius:9,border:"1px solid #F97316",
                  background:showLimits?"#F97316":"#FFF7ED",
                  color:showLimits?"#fff":"#C2410C",cursor:"pointer",fontSize:12,fontWeight:700}}>
                {showLimits?"▲ إخفاء":"✏️ تعديل"}
              </button>
            </div>
            {showLimits&&(
              <div style={{marginTop:14,padding:14,background:"#FFF7ED",borderRadius:10}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                  <div>
                    <label style={{display:"block",color:"#C2410C",fontSize:12,fontWeight:700,marginBottom:6}}>
                      📘 الحد الأقصى للشعبة النظرية
                    </label>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <input type="number" min={1} max={100}
                        value={localTheory}
                        onChange={e=>setLocalTheory(+e.target.value||35)}
                        style={{width:70,border:"2px solid #F97316",borderRadius:8,padding:"8px",
                          fontSize:16,fontWeight:800,textAlign:"center",color:"#C2410C"}}/>
                      <span style={{color:"#C2410C",fontSize:12}}>متدرب</span>
                    </div>
                  </div>
                  <div>
                    <label style={{display:"block",color:"#065F46",fontSize:12,fontWeight:700,marginBottom:6}}>
                      🔧 الحد الأقصى للشعبة العملية
                    </label>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <input type="number" min={1} max={100}
                        value={localPract}
                        onChange={e=>setLocalPract(+e.target.value||20)}
                        style={{width:70,border:"2px solid #10B981",borderRadius:8,padding:"8px",
                          fontSize:16,fontWeight:800,textAlign:"center",color:"#065F46"}}/>
                      <span style={{color:"#065F46",fontSize:12}}>متدرب</span>
                    </div>
                  </div>
                </div>
                <button onClick={()=>{
                  setTheoryCap(localTheory);
                  setPractCap(localPract);
                  setShowLimits(false);
                }} style={{padding:"10px 24px",background:"#F97316",color:"#fff",border:"none",
                  borderRadius:9,cursor:"pointer",fontSize:13,fontWeight:700}}>
                  ✅ حفظ القيود
                </button>
              </div>
            )}
          </div>
        </div>

        {/* ── Period schedule ── */}
        <div style={{background:"#FFFFFF",border:"1px solid #A5F3FC",borderRadius:13,padding:16,marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{color:"#042F2E",fontWeight:700,fontSize:13}}>⏰ جدول الحصص اليومي</div>
            <Btn onClick={()=>setModal("periodSettings")} bg="#E0FDF9" outline sm>⚙️ تعديل</Btn>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {periods.map(p=>(
              <div key={p.id} style={{background:"#E0FDF9",border:"1px solid #A5F3FC",borderRadius:8,padding:"6px 12px",textAlign:"center",minWidth:90}}>
                <div style={{color:"#0E7490",fontWeight:700,fontSize:11}}>الحصة {p.label}</div>
                <div style={{color:"#0C4A6E",fontSize:10,direction:"ltr"}}>{p.start}–{p.end}</div>
              </div>
            ))}
          </div>
        </div>
      </div>
    );
  };

  // ── COURSES TAB ───────────────────────────────────────────
  const CoursesTab = () => {
    const [fs,setFs]=useState("all");
    const [fl,setFl]=useState(0);
    const [addF,setAddF]=useState({code:"",ar:"",name:"",spec:"net",level:1,L:2,P:2,T:0});
    const vis=courses.filter(c=>!c.coop&&(fs==="all"||c.spec===fs)&&(fl===0||c.level===fl));
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>المقررات التدريبية</h2>
          <div style={{display:"flex",gap:8}}>
            <Btn onClick={()=>setModal("assignCourses")} bg="#8b5cf6" sm>📋 تخصيص المدربين للمقررات</Btn>
            <Btn onClick={()=>setModal("addCourse")}>+ إضافة مقرر</Btn>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          {[{id:"all",name:"الكل",color:"#0F766E"},...SPECIALIZATIONS].map(s=>(
            <button key={s.id} onClick={()=>setFs(s.id)}
              style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${s.color}55`,
                background:fs===s.id?s.color:"transparent",color:fs===s.id?"#fff":s.color,cursor:"pointer",fontSize:12,fontWeight:600}}>
              {s.name}
            </button>
          ))}
          <div style={{marginRight:"auto",display:"flex",gap:5}}>
            {[{v:0,l:"الكل"},...[1,2,3,4,5].map(x=>({v:x,l:"فصل "+x}))].map(o=>(
              <button key={o.v} onClick={()=>setFl(o.v)}
                style={{padding:"5px 11px",borderRadius:8,border:"1px solid #2a3a55",
                  background:fl===o.v?"#99F6E4":"transparent",color:fl===o.v?"#134E4A":"#0F766E",cursor:"pointer",fontSize:12}}>
                {o.l}
              </button>
            ))}
          </div>
        </div>
        <div style={{overflowX:"auto",background:"#FFFFFF",border:"1px solid #1e2d45",borderRadius:13}}>
          <table style={{borderCollapse:"collapse",width:"100%",fontSize:12}}>
            <thead>
              <tr style={{background:"#F0FDFA"}}>
                {["الرمز","الرمز العربي","اسم المقرر","التخصص","الفصل","مح","عم","مت","س.أ","مدرب النظري","مدرب العملي",""].map(h=>(
                  <th key={h} style={{color:"#0F766E",padding:"8px 10px",border:"1px solid #1e2d45",whiteSpace:"nowrap",textAlign:"center"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {vis.map(c=>{
                const col=spColor(c.spec);
                const a=courseAssign[c.id]||{};
                const tTr=trainers.find(x=>x.id===a.theoryId);
                const pTr=trainers.find(x=>x.id===a.practicalId);
                return (
                  <tr key={c.id}>
                    <td style={{padding:"7px 10px",color:col,fontWeight:700,fontFamily:"monospace",fontSize:11,border:"1px solid #1a2535"}}>{c.code}</td>
                    <td style={{padding:"7px 10px",color:"#0F766E",fontFamily:"monospace",fontSize:11,border:"1px solid #1a2535"}}>{c.ar}</td>
                    <td style={{padding:"7px 10px",color:"#134E4A",border:"1px solid #1a2535"}}>{c.name}</td>
                    <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}><Badge color={col}>{spName(c.spec)}</Badge></td>
                    <td style={{padding:"7px 10px",textAlign:"center",color:"#0F766E",border:"1px solid #1a2535"}}>{c.level}</td>
                    <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}><Badge color="#3b82f6">{c.L}</Badge></td>
                    <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}><Badge color="#10b981">{c.P}</Badge></td>
                    <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}><Badge color="#8b5cf6">{c.T}</Badge></td>
                    <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}><Badge color="#f59e0b">{c.CTH}</Badge></td>
                    <td style={{padding:"7px 10px",border:"1px solid #1a2535"}}>
                      {tTr?<Badge color="#3b82f6">{tTr.name}</Badge>:<span style={{color:"#99F6E4",fontSize:10}}>—</span>}
                    </td>
                    <td style={{padding:"7px 10px",border:"1px solid #1a2535"}}>
                      {pTr?<Badge color="#10b981">{pTr.name}</Badge>:<span style={{color:"#99F6E4",fontSize:10}}>—</span>}
                    </td>
                    <td style={{padding:"7px 10px",textAlign:"center",border:"1px solid #1a2535"}}>
                      <button onClick={()=>setCourses(p=>p.filter(x=>x.id!==c.id))}
                        style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:14}}>🗑</button>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
          <div style={{padding:"8px 14px",color:"#99F6E4",fontSize:11}}>{vis.length} مقرر</div>
        </div>
        {modal==="addCourse"&&(
          <Modal title="إضافة مقرر جديد" onClose={()=>setModal(null)}>
            <Fld label="رمز المقرر" value={addF.code} onChange={v=>setAddF(p=>({...p,code:v}))}/>
            <Fld label="الرمز العربي" value={addF.ar} onChange={v=>setAddF(p=>({...p,ar:v}))}/>
            <Fld label="اسم المقرر" value={addF.name} onChange={v=>setAddF(p=>({...p,name:v}))}/>
            <Sel label="التخصص" value={addF.spec} onChange={v=>setAddF(p=>({...p,spec:v}))} opts={SPECIALIZATIONS.map(s=>({v:s.id,l:s.name}))}/>
            <Sel label="الفصل التدريبي" value={addF.level} onChange={v=>setAddF(p=>({...p,level:+v}))} opts={[1,2,3,4,5].map(l=>({v:l,l:"الفصل "+l}))}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
              <Fld label="مح (محاضرة نظري)" value={addF.L} onChange={v=>setAddF(p=>({...p,L:+v}))} type="number" min={0}/>
              <Fld label="عم (عملي/ورش)" value={addF.P} onChange={v=>setAddF(p=>({...p,P:+v}))} type="number" min={0}/>
              <Fld label="مت (تمارين)" value={addF.T} onChange={v=>setAddF(p=>({...p,T:+v}))} type="number" min={0}/>
            </div>
            <div style={{background:"#FFFFFF",borderRadius:8,padding:10,marginBottom:14,fontSize:12,color:"#0F766E"}}>
              ساعات الاتصال الأسبوعية (س.أ): <strong style={{color:"#f59e0b",fontSize:15}}>{addF.L+addF.P+addF.T}</strong>
              <br/>النظري (مح+مت): <Badge color="#3b82f6">{addF.L+addF.T}</Badge> &nbsp; العملي: <Badge color="#10b981">{addF.P}</Badge>
            </div>
            <Btn onClick={()=>{
              if(!addF.code||!addF.name)return;
              setCourses(p=>[...p,{...addF,id:uid(),CTH:addF.L+addF.P+addF.T}]);
              setModal(null);
            }} full>إضافة المقرر</Btn>
          </Modal>
        )}
        {modal==="assignCourses"&&(
          <CourseAssignModal courses={courses} trainers={trainers}
            assignments={courseAssign} setAssignments={setCourseAssign}
            onClose={()=>setModal(null)}/>
        )}
      </div>
    );
  };

  // ── TRAINERS TAB ──────────────────────────────────────────
  const TrainersTab = () => {
    const [f,setF]=useState({name:"",specs:[],maxH:18});
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>المدربون</h2>
          <Btn onClick={()=>{
            const a=document.createElement("a");
            a.href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,"+TRAINERS_TEMPLATE_B64;
            a.download="قالب_المدربين.xlsx";
            a.click();
          }} bg="#0E7490" outline sm>⬇️ تحميل القالب</Btn>
          <Btn onClick={()=>setModal("importTrainersExcel")} bg="#10b981" outline sm>📊 استيراد Excel</Btn>
          <Btn onClick={()=>setModal("addTrainer")} bg="#8b5cf6">+ إضافة مدرب</Btn>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
          {trainers.map(t=>{
            const u=usedH(t.id),pct=Math.min(100,u/t.maxH*100);
            // Courses assigned to this trainer
            const myTh=Object.entries(courseAssign).filter(([,a])=>a.theoryId===t.id).map(([cid])=>courses.find(c=>c.id===cid)).filter(Boolean);
            const myPr=Object.entries(courseAssign).filter(([,a])=>a.practicalId===t.id).map(([cid])=>courses.find(c=>c.id===cid)).filter(Boolean);
            return (
              <div key={t.id} style={{background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:13,padding:18}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                  <div>
                    <div style={{color:"#042F2E",fontWeight:700}}>{t.name}</div>
                    <div style={{color:"#0F766E",fontSize:11}}>الحد الأقصى: {t.maxH} ساعة/أسبوع</div>
                  </div>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    <button
                      onClick={()=>setModal("offDays-"+t.id)}
                      title="تحديد أيام/ساعات الإجازة"
                      style={{background:"#FEF3C7",border:"1px solid #F59E0B",color:"#92400E",
                        borderRadius:7,padding:"4px 8px",cursor:"pointer",fontSize:11,fontWeight:700}}>
                      🏖️ إجازة
                    </button>
                    <button onClick={()=>setTrainers(p=>p.filter(x=>x.id!==t.id))}
                      style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:16}}>×</button>
                  </div>
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
                  {t.specs.map(s=><Badge key={s} color={spColor(s)}>{spName(s)}</Badge>)}
                </div>
                {/* Off days display */}
                {((t.offDays||[]).length>0||(t.offSlots||[]).length>0)&&(
                  <div style={{background:"#FEF3C7",border:"1px solid #F59E0B",borderRadius:8,
                    padding:"6px 10px",marginBottom:8,fontSize:10}}>
                    <div style={{color:"#92400E",fontWeight:700,marginBottom:3}}>🏖️ أوقات الإجازة:</div>
                    {(t.offDays||[]).map(d=>(
                      <span key={d} style={{background:"#F59E0B",color:"#fff",borderRadius:4,
                        padding:"1px 7px",marginLeft:4,fontSize:9,fontWeight:700}}>{d} (يوم كامل)</span>
                    ))}
                    {(t.offSlots||[]).map((s,i)=>(
                      <div key={i} style={{color:"#92400E",fontSize:9,marginTop:2}}>
                        {s.day}: الحصص {s.periods.join("، ")}
                      </div>
                    ))}
                  </div>
                )}
                <div style={{background:"#F0FDFA",borderRadius:5,height:6,overflow:"hidden",marginBottom:5}}>
                  <div style={{height:"100%",width:pct+"%",background:pct>85?"#ef4444":pct>65?"#f59e0b":"#10b981",borderRadius:5}}/>
                </div>
                <div style={{color:"#0F766E",fontSize:11,marginBottom:10}}>{u} / {t.maxH} ساعة موزعة</div>
                {myTh.length>0&&(
                  <div style={{marginBottom:6}}>
                    <div style={{color:"#3b82f6",fontSize:10,marginBottom:4}}>📘 يدرّس النظري في:</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                      {myTh.map(c=><Badge key={c.id} color="#3b82f6">{c.code}</Badge>)}
                    </div>
                  </div>
                )}
                {myPr.length>0&&(
                  <div>
                    <div style={{color:"#10b981",fontSize:10,marginBottom:4}}>🔧 يدرّب العملي في:</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                      {myPr.map(c=><Badge key={c.id} color="#10b981">{c.code}</Badge>)}
                    </div>
                  </div>
                )}
              </div>
            );
          })}
        </div>
        {/* Off Days Modal */}
        {modal&&modal.startsWith("offDays-")&&(()=>{
          const tid = modal.replace("offDays-","");
          const t = trainers.find(x=>x.id===tid);
          if(!t) return null;
          const offDays  = t.offDays  || [];
          const offSlots = t.offSlots || [];

          const toggleDay = (day) => {
            const has = offDays.includes(day);
            const newDays = has ? offDays.filter(d=>d!==day) : [...offDays, day];
            const newSlots = has ? offSlots : offSlots.filter(s=>s.day!==day);
            setTrainers(p=>p.map(x=>x.id===tid?{...x,offDays:newDays,offSlots:newSlots}:x));
          };

          const toggleSlot = (day, period) => {
            const newDays = offDays.filter(d=>d!==day);
            const existing = offSlots.find(s=>s.day===day);
            let newSlots;
            if(existing) {
              const hasPeriod = existing.periods.includes(period);
              const newPeriods = hasPeriod
                ? existing.periods.filter(p=>p!==period)
                : [...existing.periods, period];
              newSlots = newPeriods.length===0
                ? offSlots.filter(s=>s.day!==day)
                : offSlots.map(s=>s.day===day?{...s,periods:newPeriods}:s);
            } else {
              newSlots = [...offSlots, {day, periods:[period]}];
            }
            setTrainers(p=>p.map(x=>x.id===tid?{...x,offDays:newDays,offSlots:newSlots}:x));
          };

          return (
            <div style={{position:"fixed",inset:0,background:"#00000099",zIndex:3000,
              display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{background:"#FFFFFF",borderRadius:18,padding:24,width:480,
                maxHeight:"85vh",overflow:"auto",direction:"rtl",boxShadow:"0 20px 60px #00000033"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                  <div>
                    <h3 style={{color:"#042F2E",margin:0,fontSize:16,fontWeight:800}}>"🏖️" {"أوقات إجازة المدرب"}</h3>
                    <div style={{color:"#0F766E",fontSize:11,marginTop:3}}>{t.name}</div>
                  </div>
                  <button onClick={()=>setModal(null)}
                    style={{background:"none",border:"none",color:"#0C4A6E",cursor:"pointer",fontSize:22}}>{"×"}</button>
                </div>

                <div style={{color:"#0C4A6E",fontSize:12,marginBottom:16,background:"#FEF3C7",
                  border:"1px solid #F59E0B",borderRadius:8,padding:"8px 12px"}}>
                  {"💡"} {"اختر يوماً كاملاً للإجازة، أو اختر حصصاً محددة من أي يوم"}
                </div>

                {DAYS.map(day=>{
                  const isFullDay = offDays.includes(day);
                  const slotEntry = offSlots.find(s=>s.day===day);
                  const selPeriods = slotEntry?.periods||[];
                  return (
                    <div key={day} style={{marginBottom:12,border:`2px solid ${isFullDay?"#F59E0B":"#99F6E4"}`,
                      borderRadius:12,padding:12,background:isFullDay?"#FEF9E7":"#F0FDFA"}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:isFullDay?0:8}}>
                        <div style={{fontWeight:700,fontSize:13,color:isFullDay?"#92400E":"#134E4A"}}>{day}</div>
                        <label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}>
                          <input type="checkbox" checked={isFullDay} onChange={()=>toggleDay(day)}
                            style={{width:16,height:16,cursor:"pointer",accentColor:"#F59E0B"}}/>
                          <span style={{fontSize:11,fontWeight:700,color:isFullDay?"#92400E":"#0F766E"}}>{"يوم كامل"}</span>
                        </label>
                      </div>
                      {!isFullDay&&(
                        <div>
                          <div style={{color:"#0F766E",fontSize:10,marginBottom:6}}>{"أو اختر حصصاً:"}</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {periods.map(pr=>{
                              const sel = selPeriods.includes(pr.id);
                              return (
                                <button key={pr.id} onClick={()=>toggleSlot(day,pr.id)}
                                  style={{padding:"5px 10px",borderRadius:7,fontSize:10,fontWeight:700,
                                    cursor:"pointer",border:"none",
                                    background:sel?"#F59E0B":"#99F6E4",
                                    color:sel?"#fff":"#0D9488"}}>
                                  {pr.label}
                                  <span style={{fontSize:8,display:"block",fontWeight:400,
                                    color:sel?"#FDE68A":"#0D9488"}}>{pr.start}</span>
                                </button>
                              );
                            })}
                          </div>
                        </div>
                      )}
                    </div>
                  );
                })}

                {(offDays.length>0||offSlots.some(s=>s.periods.length>0))&&(
                  <div style={{background:"#FEF3C7",border:"1px solid #F59E0B",borderRadius:10,
                    padding:"10px 14px",marginTop:12,fontSize:11,color:"#92400E"}}>
                    <div style={{fontWeight:700,marginBottom:6}}>{"📋 ملخص الإجازات:"}</div>
                    {offDays.map(d=>(
                      <div key={d}>{"•"} {d}: {"إجازة يوم كامل"}</div>
                    ))}
                    {offSlots.filter(s=>s.periods.length>0).map((s,i)=>(
                      <div key={i}>{"•"} {s.day}: {"حصص"} {s.periods.map(p=>periods.find(x=>x.id===p)?.label).join("، ")}</div>
                    ))}
                  </div>
                )}

                <div style={{display:"flex",gap:10,marginTop:16}}>
                  <button onClick={()=>setModal(null)}
                    style={{flex:1,background:"#0C4A6E",color:"#fff",border:"none",
                      borderRadius:10,padding:"11px",cursor:"pointer",fontSize:13,fontWeight:700}}>
                    {"✅ حفظ"}
                  </button>
                  <button onClick={()=>setTrainers(p=>p.map(x=>x.id===tid?{...x,offDays:[],offSlots:[]}:x))}
                    style={{padding:"11px 14px",background:"#FEE2E2",color:"#991B1B",
                      border:"none",borderRadius:10,cursor:"pointer",fontSize:12,fontWeight:700}}>
                    {"مسح الكل"}
                  </button>
                </div>
              </div>
            </div>
          );
        })()}

        {modal==="addTrainer"&&(
          <Modal title="إضافة مدرب" onClose={()=>setModal(null)}>
            <Fld label="اسم المدرب" value={f.name} onChange={v=>setF(p=>({...p,name:v}))}/>
            <Fld label="الحد الأقصى للساعات الأسبوعية" value={f.maxH} onChange={v=>setF(p=>({...p,maxH:+v}))} type="number" min={1} max={40}/>
            <div style={{marginBottom:14}}>
              <div style={{color:"#0F766E",fontSize:11,marginBottom:8}}>التخصصات المؤهل لها</div>
              {SPECIALIZATIONS.map(s=>(
                <label key={s.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7,cursor:"pointer"}}>
                  <input type="checkbox" checked={f.specs.includes(s.id)}
                    onChange={e=>setF(p=>({...p,specs:e.target.checked?[...p.specs,s.id]:p.specs.filter(x=>x!==s.id)}))}/>
                  <span style={{color:s.color,fontWeight:600}}>{s.name}</span>
                </label>
              ))}
            </div>
            <Btn bg="#8b5cf6" full onClick={()=>{
              if(!f.name)return;
              setTrainers(p=>[...p,{...f,id:uid()}]);
              setF({name:"",specs:[],maxH:18});setModal(null);
            }}>إضافة المدرب</Btn>
          </Modal>
        )}

        {modal==="importTrainersExcel"&&(
          <Modal title="📊 استيراد المدربين من Excel" onClose={()=>setModal(null)} wide>
            <div style={{background:"#FFFFFF",borderRadius:10,padding:14,marginBottom:14}}>
              <div style={{color:"#0F766E",fontSize:12,marginBottom:10,fontWeight:700}}>📋 تعليمات الاستيراد</div>
              <div style={{color:"#0F766E",fontSize:11,lineHeight:2}}>
                1. حمّل نموذج Excel من الزر أدناه<br/>
                2. عبّئ بيانات المدربين ابتداءً من <strong style={{color:"#f59e0b"}}>الصف 7</strong><br/>
                3. احفظ الملف ثم ارفعه هنا<br/>
                4. سيتم استيراد المدربين تلقائياً مع التخصصات والساعات
              </div>
            </div>

            {/* Column reference */}
            <div style={{overflowX:"auto",marginBottom:14}}>
              <table style={{borderCollapse:"collapse",fontSize:11,width:"100%"}}>
                <thead>
                  <tr style={{background:"#A5F3FC"}}>
                    {["العمود","الحقل","مثال","إلزامي؟"].map(h=>(
                      <th key={h} style={{color:"#0E7490",padding:"6px 10px",border:"1px solid #1e2d45",textAlign:"center"}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    ["A","اسم المدرب","أ. محمد العمري","✅ إلزامي"],
                    ["B","رقم الموظف","1001","اختياري"],
                    ["C","الدرجة العلمية","بكالوريوس","اختياري"],
                    ["D","التخصص 1","net / auto / gen","✅ إلزامي"],
                    ["E","التخصص 2","net / auto / gen","اختياري"],
                    ["F","التخصص 3","net / auto / gen","اختياري"],
                    ["G","الحد الأقصى للساعات","18","✅ إلزامي"],
                    ["H","البريد الإلكتروني","name@edu.sa","اختياري"],
                    ["I","ملاحظات","—","اختياري"],
                  ].map(([col,field,ex,req])=>(
                    <tr key={col} style={{background:req.includes("✅")?"#1a2e1a":"#FFFFFF"}}>
                      <td style={{padding:"5px 10px",border:"1px solid #1e2d45",textAlign:"center",color:"#0ea5e9",fontWeight:700,fontFamily:"monospace"}}>{col}</td>
                      <td style={{padding:"5px 10px",border:"1px solid #1e2d45",color:"#134E4A"}}>{field}</td>
                      <td style={{padding:"5px 10px",border:"1px solid #1e2d45",color:"#0F766E",fontFamily:"monospace"}}>{ex}</td>
                      <td style={{padding:"5px 10px",border:"1px solid #1e2d45",textAlign:"center",color:req.includes("✅")?"#10b981":"#0F766E"}}>{req}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            {/* File upload */}
            <div style={{marginBottom:14}}>
              <div style={{color:"#0F766E",fontSize:11,marginBottom:8}}>📁 رفع ملف Excel أو CSV:</div>
              <input type="file" accept=".xlsx,.xls,.csv"
                onChange={async e=>{
                  const file=e.target.files[0];
                  if(!file)return;
                  const name=file.name.toLowerCase();
                  const reader=new FileReader();
                  const parseTrainers=(lines,sep)=>{
                    // Skip header rows (first 5 rows of template)
                    const data=lines.slice(5).filter(l=>l.trim());
                    return data.map(line=>{
                      const cols=line.split(sep).map(c=>c.replace(/"/g,"").trim());
                      if(!cols[0]||!cols[3]) return null; // name + spec1 required
                      const specs=[cols[3],cols[4],cols[5]].filter(s=>["net","auto","gen"].includes(s));
                      if(!specs.length) return null;
                      const offDays=(cols[8]||"").split(/[,،\/]/).map(d=>d.trim()).filter(d=>
                        ["الأحد","الاثنين","الثلاثاء","الأربعاء","الخميس"].includes(d)
                      );
                      return{
                        id:uid(), name:cols[0], specs,
                        maxH:+cols[6]||18,
                        offDays, offSlots:[],
                      };
                    }).filter(Boolean);
                  };

                  if(name.endsWith(".csv")){
                    reader.onload=ev=>{
                      const txt=ev.target.result;
                      // Detect separator
                      const sep=txt.includes("\t")?"\t":",";
                      const lines=txt.split("\n");
                      const imported=parseTrainers(lines,sep);
                      if(imported.length){
                        setTrainers(p=>[...p,...imported]);
                        setModal(null);
                        alert(`✅ تم استيراد ${imported.length} مدرب بنجاح!`);
                      } else alert("❌ لم يتم العثور على بيانات.\nتأكد من حفظ الملف بصيغة CSV وتعبئة البيانات من الصف 6.");
                    };
                    reader.readAsText(file,"UTF-8");
                  } else {
                    alert("💡 احفظ ملف Excel بصيغة CSV:\nملف ← حفظ باسم ← CSV UTF-8\nثم ارفع ملف الـ CSV هنا.");
                  }
                }}
                style={{display:"block",width:"100%",background:"#F0FDFA",border:"2px dashed #0E7490",
                  borderRadius:10,padding:"20px",color:"#0F766E",fontSize:12,cursor:"pointer",boxSizing:"border-box"}}
              />
            </div>

            {/* Paste area */}
            <div style={{marginBottom:14}}>
              <div style={{color:"#0F766E",fontSize:11,marginBottom:6,fontWeight:700}}>أو الصق البيانات من Excel مباشرة (Ctrl+C ثم Ctrl+V):</div>
              <textarea id="trainer-csv-input" rows={5}
                placeholder={"أ. محمد العمري\t1001\tبكالوريوس\tnet\tgen\t\t18\t\tالأحد\t\nأ. سارة الزهراني\t1002\tماجستير\tauto\tgen\t\t16"}
                style={{width:"100%",background:"#F0FDFA",border:"1px solid #A5F3FC",borderRadius:8,
                  padding:10,color:"#134E4A",fontSize:11,fontFamily:"monospace",boxSizing:"border-box",direction:"ltr"}}/>
            </div>

            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{
                const txt=document.getElementById("trainer-csv-input")?.value||"";
                const lines=txt.split("\n").filter(l=>l.trim());
                // Auto-detect separator
                const sep=txt.includes("\t")?"\t":",";
                const imported=lines.map(line=>{
                  const cols=line.split(sep).map(c=>c.replace(/"/g,"").trim());
                  if(!cols[0]) return null;
                  const specs=[cols[3],cols[4],cols[5]].filter(s=>["net","auto","gen"].includes(s));
                  if(!specs.length) return null;
                  const offDays=(cols[8]||"").split(/[,،\/]/).map(d=>d.trim()).filter(d=>
                    ["الأحد","الاثنين","الثلاثاء","الأربعاء","الخميس"].includes(d)
                  );
                  return{id:uid(),name:cols[0],specs,maxH:+cols[6]||18,offDays,offSlots:[]};
                }).filter(Boolean);
                if(!imported.length){alert("❌ لا توجد بيانات.\nتأكد من نسخ البيانات من القالب مباشرة.");return;}
                setTrainers(p=>[...p,...imported]);
                setModal(null);
                alert(`✅ تم استيراد ${imported.length} مدرب!`);
              }} style={{flex:1,background:"#8b5cf6",color:"#fff",border:"none",borderRadius:9,
                padding:"10px",cursor:"pointer",fontSize:13,fontWeight:700}}>
                ✅ استيراد من النص
              </button>
              <button onClick={()=>setModal(null)}
                style={{flex:1,background:"transparent",color:"#0F766E",border:"1px solid #334155",
                  borderRadius:9,padding:"10px",cursor:"pointer",fontSize:13}}>إلغاء</button>
            </div>
          </Modal>
        )}
      </div>
    );
  };

  // ── HALLS TAB ─────────────────────────────────────────────
  const HallsTab = () => {
    const [f,setF]=useState({name:"",cap:30,type:"theory",specs:["all"]});
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>القاعات والمختبرات والورش</h2>
          <Btn onClick={()=>{
            const a=document.createElement("a");
            a.href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,"+HALLS_TEMPLATE_B64;
            a.download="قالب_القاعات.xlsx";
            a.click();
          }} bg="#0E7490" outline sm>⬇️ تحميل القالب</Btn>
          <Btn onClick={()=>setModal("importHalls")} bg="#8b5cf6" outline sm>📊 استيراد Excel</Btn>
          <Btn onClick={()=>setModal("addHall")} bg="#10b981">+ إضافة</Btn>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:13}}>
          {halls.map(h=>(
            <div key={h.id} style={{background:"#FFFFFF",border:"1px solid #1e2d45",borderRadius:13,padding:17}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <div>
                  <div style={{color:"#042F2E",fontWeight:700}}>{h.name}</div>
                  <div style={{color:"#0F766E",fontSize:11}}>السعة: {h.cap} متدرب</div>
                </div>
                <button onClick={()=>setHalls(p=>p.filter(x=>x.id!==h.id))}
                  style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer"}}>×</button>
              </div>
              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                <Badge color={h.type==="theory"?"#3b82f6":"#10b981"}>{h.type==="theory"?"🏫 نظري":"🔧 عملي/ورشة"}</Badge>
                {h.specs.map(s=>s==="all"?<Badge key="a" color="#f59e0b">جميع التخصصات</Badge>:<Badge key={s} color={spColor(s)}>{spName(s)}</Badge>)}
              </div>
            </div>
          ))}
        </div>
        {modal==="importHalls"&&(
          <Modal title="📊 استيراد القاعات من Excel" onClose={()=>setModal(null)}>
            <div style={{color:"#0C4A6E",fontSize:11,lineHeight:1.9,marginBottom:12,
              background:"#E0FDF9",border:"1px solid #A5F3FC",borderRadius:8,padding:"10px 14px"}}>
              <strong>خطوات الاستيراد:</strong><br/>
              ١. حمّل القالب وعبّئه<br/>
              ٢. في Excel: حدد البيانات ← نسخ (Ctrl+C)<br/>
              ٣. الصق في المربع أدناه (Ctrl+V)<br/>
              الأعمدة: <strong>الاسم | الرقم | النوع | السعة | التخصص</strong>
            </div>
            <div style={{marginBottom:10}}>
              <div style={{color:"#0C4A6E",fontSize:11,fontWeight:700,marginBottom:4}}>الصق البيانات هنا:</div>
              <textarea id="halls-paste" rows={7}
                placeholder={"قاعة 101\t101\ttheory\t30\tall\nمختبر شبكات 1\tN101\tpractical\t20\tnet\nورشة محركات 1\tA101\tpractical\t20\tauto"}
                style={{width:"100%",border:"1px solid #A5F3FC",borderRadius:8,padding:10,
                  fontSize:11,fontFamily:"monospace",direction:"ltr",background:"#F0FDFA",
                  boxSizing:"border-box",resize:"vertical"}}/>
            </div>
            <div style={{display:"flex",gap:10}}>
              <Btn bg="#8b5cf6" onClick={()=>{
                const txt=document.getElementById("halls-paste")?.value||"";
                const lines=txt.split("\n").filter(l=>l.trim());
                const sep=txt.includes("\t")?"\t":",";
                let imported=0;
                const newHalls=[];
                lines.forEach(line=>{
                  const cols=line.split(sep).map(c=>c.replace(/"/g,"").trim());
                  if(!cols[0]) return;
                  const type=cols[2]==="practical"?"practical":"theory";
                  const cap=parseInt(cols[3])||30;
                  const spec=cols[4]||"all";
                  newHalls.push({
                    id:uid(), name:cols[0],
                    type, cap,
                    specs:[["all","net","auto"].includes(spec)?spec:"all"]
                  });
                  imported++;
                });
                if(!imported){alert("❌ لا توجد بيانات.");return;}
                setHalls(p=>[...p,...newHalls]);
                setModal(null);
                alert(`✅ تم استيراد ${imported} قاعة/مختبر`);
              }}>✅ استيراد</Btn>
              <Btn bg="#64748B" outline onClick={()=>setModal(null)}>إلغاء</Btn>
            </div>
          </Modal>
        )}

        {modal==="addHall"&&(
          <Modal title="إضافة قاعة / مختبر / ورشة" onClose={()=>setModal(null)}>
            <Fld label="الاسم" value={f.name} onChange={v=>setF(p=>({...p,name:v}))}/>
            <Fld label="الطاقة الاستيعابية" value={f.cap} onChange={v=>setF(p=>({...p,cap:+v}))} type="number" min={1}/>
            <Sel label="النوع" value={f.type} onChange={v=>setF(p=>({...p,type:v}))}
              opts={[{v:"theory",l:"🏫 نظري/محاضرة"},{v:"practical",l:"🔧 عملي/مختبر/ورشة"}]}/>
            <div style={{marginBottom:14}}>
              <div style={{color:"#0F766E",fontSize:11,marginBottom:8}}>تخصيص التخصص</div>
              <label style={{display:"flex",alignItems:"center",gap:8,marginBottom:7,cursor:"pointer"}}>
                <input type="checkbox" checked={f.specs.includes("all")}
                  onChange={e=>setF(p=>({...p,specs:e.target.checked?["all"]:[]}))}/>
                <span style={{color:"#f59e0b"}}>جميع التخصصات</span>
              </label>
              {!f.specs.includes("all")&&SPECIALIZATIONS.map(s=>(
                <label key={s.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7,cursor:"pointer"}}>
                  <input type="checkbox" checked={f.specs.includes(s.id)}
                    onChange={e=>setF(p=>({...p,specs:e.target.checked?[...p.specs.filter(x=>x!=="all"),s.id]:p.specs.filter(x=>x!==s.id)}))}/>
                  <span style={{color:s.color}}>{s.name}</span>
                </label>
              ))}
            </div>
            <Btn bg="#10b981" full onClick={()=>{
              if(!f.name)return;
              setHalls(p=>[...p,{...f,id:uid()}]);
              setF({name:"",cap:30,type:"theory",specs:["all"]});setModal(null);
            }}>إضافة</Btn>
          </Modal>
        )}
      </div>
    );
  };

  // ── TRAINEES TAB ──────────────────────────────────────────
  const TraineesTab = () => {
    const [ls,setLs]=useState("net");
    const [ll,setLl]=useState(1);
    const [f,setF]=useState({name:"",sid:"",spec:"net",level:1,rem:[]});
    const filt=trainees.filter(t=>t.spec===ls&&t.level===ll);
    const av=courses.filter(c=>!c.coop&&(c.spec===ls||c.spec==="gen"));
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>المتدربون</h2>
          <div style={{display:"flex",gap:8}}>
            <Btn onClick={()=>setModal("import")} bg="#99F6E4" outline sm>📥 استيراد CSV</Btn>
            <Btn onClick={()=>setModal("addTrainee")} bg="#f59e0b">+ إضافة</Btn>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
          {SPECIALIZATIONS.filter(s=>!s.shared).map(s=>(
            <button key={s.id} onClick={()=>setLs(s.id)}
              style={{padding:"6px 14px",borderRadius:8,border:`1px solid ${s.color}55`,
                background:ls===s.id?s.color:"transparent",color:ls===s.id?"#fff":s.color,cursor:"pointer",fontSize:12,fontWeight:600}}>
              {s.name}
            </button>
          ))}
          <div style={{marginRight:"auto",display:"flex",gap:5}}>
            {[1,2,3,4,5].map(l=>(
              <button key={l} onClick={()=>setLl(l)}
                style={{padding:"6px 12px",borderRadius:8,border:"1px solid #2a3a55",
                  background:ll===l?"#99F6E4":"transparent",color:ll===l?"#134E4A":"#0F766E",cursor:"pointer",fontSize:12}}>م{l}</button>
            ))}
          </div>
        </div>
        <div style={{color:"#0F766E",fontSize:12,marginBottom:12}}>العدد: <strong style={{color:"#134E4A"}}>{filt.length}</strong></div>
        {filt.length===0
          ?<div style={{textAlign:"center",padding:40,border:"1px dashed #1e2d45",borderRadius:12,color:"#99F6E4"}}>لا يوجد متدربون</div>
          :(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:12}}>
              {filt.map(t=>(
                <div key={t._id} style={{background:"#FFFFFF",border:"1px solid #1e2d45",borderRadius:12,padding:15}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
                    <div>
                      <div style={{color:"#042F2E",fontWeight:700}}>{t.name}</div>
                      <div style={{color:"#0F766E",fontSize:11}}>{t.sid}</div>
                    </div>
                    <button onClick={()=>setTrainees(p=>p.filter(x=>x._id!==t._id))}
                      style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer"}}>×</button>
                  </div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                    {t.rem.map(cid=>{const c=courses.find(x=>x.id===cid);return c?<Badge key={cid} color={spColor(c.spec)}>{c.code}</Badge>:null;})}
                  </div>
                </div>
              ))}
            </div>
          )
        }
        {modal==="addTrainee"&&(
          <Modal title="إضافة متدرب" onClose={()=>setModal(null)}>
            <Fld label="الاسم" value={f.name} onChange={v=>setF(p=>({...p,name:v}))}/>
            <Fld label="رقم الملف" value={f.sid} onChange={v=>setF(p=>({...p,sid:v}))}/>
            <Sel label="التخصص" value={f.spec} onChange={v=>setF(p=>({...p,spec:v}))} opts={SPECIALIZATIONS.filter(s=>!s.shared).map(s=>({v:s.id,l:s.name}))}/>
            <Sel label="الفصل" value={f.level} onChange={v=>setF(p=>({...p,level:+v}))} opts={[1,2,3,4,5].map(l=>({v:l,l:"الفصل "+l}))}/>
            <div style={{marginBottom:14}}>
              <div style={{color:"#0F766E",fontSize:11,marginBottom:7}}>المقررات المتبقية</div>
              <div style={{maxHeight:190,overflow:"auto",background:"#FFFFFF",borderRadius:8,padding:10}}>
                {av.map(c=>(
                  <label key={c.id} style={{display:"flex",alignItems:"center",gap:7,marginBottom:7,cursor:"pointer"}}>
                    <input type="checkbox" checked={f.rem.includes(c.id)}
                      onChange={e=>setF(p=>({...p,rem:e.target.checked?[...p.rem,c.id]:p.rem.filter(x=>x!==c.id)}))}/>
                    <Badge color={spColor(c.spec)}>{c.code}</Badge>
                    <span style={{color:"#134E4A",fontSize:11}}>{c.name} — م{c.level}</span>
                  </label>
                ))}
              </div>
            </div>
            <Btn bg="#f59e0b" full onClick={()=>{
              if(!f.name)return;
              setTrainees(p=>[...p,{name:f.name,sid:f.sid,spec:f.spec,level:f.level,rem:f.rem,_id:uid()}]);
              setF({name:"",sid:"",spec:"net",level:1,rem:[]});setModal(null);
            }}>إضافة المتدرب</Btn>
          </Modal>
        )}
        {modal==="import"&&(
          <Modal title="استيراد بيانات المتدربين" onClose={()=>setModal(null)} wide>
            <div style={{background:"#FFFFFF",borderRadius:8,padding:11,marginBottom:11,fontSize:10,color:"#0F766E",fontFamily:"monospace",lineHeight:1.9}}>
              الصيغة: الاسم,رقم الملف,التخصص(net/auto),الفصل,أكواد المقررات(مفصولة بـ ;)<br/>
              مثال:<br/>محمد العمري,1001,net,2,ICMT 102;ISUP 151;ENGL 102
            </div>
            <textarea value={importTxt} onChange={e=>setImportTxt(e.target.value)} rows={7}
              style={{width:"100%",background:"#FFFFFF",border:"1px solid #1e3a5f",borderRadius:8,
                padding:11,color:"#134E4A",fontSize:12,fontFamily:"monospace",boxSizing:"border-box",direction:"ltr"}}/>
            <div style={{color:"#0F766E",fontSize:12,margin:"8px 0"}}>
              سيتم استيراد <strong style={{color:"#10b981"}}>{importTxt.split("\n").filter(l=>l.trim()).length}</strong> متدرب
            </div>
            <Btn bg="#10b981" full onClick={()=>{
              const lines=importTxt.split("\n").filter(l=>l.trim());
              const imp=lines.map(line=>{
                const p=line.split(",");if(p.length<4)return null;
                const[name,sid,spec,level,cs]=p;
                const codes=cs?cs.split(";").map(c=>c.trim()):[];
                const rem=courses.filter(c=>codes.includes(c.code)).map(c=>c.id);
                return{name:name.trim(),sid:sid.trim(),spec:spec.trim(),level:+level.trim(),rem,_id:uid()};
              }).filter(Boolean);
              setTrainees(prev=>[...prev,...imp]);
              setImportTxt("");setModal(null);
            }}>استيراد</Btn>
          </Modal>
        )}
      </div>
    );
  };

  // ── SCHEDULE BUILDER ──────────────────────────────────────
  const ScheduleTab = () => {
    const col = spColor(selSpec);
    const coursesInLevel = lc(selSpec, selLv);
    const gs = getGroups(selSpec, selLv);
    const curGid = (selGroup && gs.find(g=>g.id===selGroup)) ? selGroup : (gs[0]?.id||null);
    const sched = curGid ? getSched(selSpec, selLv, curGid) : [];
    const [newGroupName, setNewGroupName] = useState("");
    const [editingName, setEditingName] = useState(null); // gid being renamed

    return (
      <div>
        {warn&&(
          <div style={{background:"#FEF2F2",border:"1px solid #EF4444",borderRadius:10,padding:"10px 16px",
            marginBottom:14,color:"#991B1B",fontSize:13,display:"flex",alignItems:"flex-start",gap:10,lineHeight:1.6}}>
            <span style={{fontSize:18,flexShrink:0}}>🚫</span>
            <div>
              <div style={{fontWeight:700,marginBottom:2}}>تعارض في الجدول</div>
              <div style={{fontSize:12}}>{warn}</div>
            </div>
            <button onClick={()=>setWarn(null)}
              style={{marginRight:"auto",background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:18,flexShrink:0}}>×</button>
          </div>
        )}

        {/* ── Top bar ── */}
        <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center",flexWrap:"wrap"}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>إعداد الجداول</h2>
          <Btn onClick={()=>setModal("periodSettings")} bg="#99F6E4" outline sm>⚙️ أوقات الحصص</Btn>
          <Btn onClick={()=>setModal("assignCourses")} bg="#8b5cf6" outline sm>📋 تخصيص المدربين</Btn>
          <div style={{display:"flex",gap:8,marginRight:"auto"}}>
            {SPECIALIZATIONS.filter(s=>!s.shared).map(s=>(
              <button key={s.id} onClick={()=>{ setSelSpec(s.id); setSelGroup(null); }}
                style={{padding:"6px 14px",borderRadius:8,border:`1px solid ${s.color}55`,
                  background:selSpec===s.id?s.color:"transparent",color:selSpec===s.id?"#fff":s.color,cursor:"pointer",fontSize:12,fontWeight:600}}>
                {s.name}
              </button>
            ))}
          </div>
          <div style={{display:"flex",gap:5}}>
            {[1,2,3,4,5].map(l=>(
              <button key={l} onClick={()=>{ setSelLv(l); setSelGroup(null); }}
                style={{padding:"6px 12px",borderRadius:8,border:"1px solid #2a3a55",
                  background:selLv===l?"#99F6E4":"transparent",color:selLv===l?"#134E4A":"#0F766E",cursor:"pointer",fontSize:12}}>م{l}</button>
            ))}
          </div>
        </div>

        {/* ── Group / Shu'ba bar ── */}
        <div style={{background:"#FFFFFF",border:"1px solid #1e2d45",borderRadius:12,padding:"10px 14px",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            <span style={{color:"#0F766E",fontSize:12,fontWeight:700,whiteSpace:"nowrap"}}>📋 الرزم:</span>

            {/* Existing groups */}
            {gs.map(g=>(
              <div key={g.id} style={{display:"flex",alignItems:"center",gap:0}}>
                {editingName===g.id ? (
                  <input
                    autoFocus
                    value={g.name}
                    onChange={e=>renameGroup(selSpec,selLv,g.id,e.target.value)}
                    onBlur={()=>setEditingName(null)}
                    onKeyDown={e=>e.key==="Enter"&&setEditingName(null)}
                    style={{background:"#FFFFFF",border:`1px solid ${col}`,borderRadius:"7px 0 0 7px",
                      padding:"5px 10px",color:"#042F2E",fontSize:12,width:100}}
                  />
                ) : (
                  <button
                    onClick={()=>{ setSelGroup(g.id); }}
                    onDoubleClick={()=>setEditingName(g.id)}
                    title="اضغط مرتين للتعديل"
                    style={{padding:"5px 12px",borderRadius:gs.length>1?"7px 0 0 7px":8,
                      border:`1px solid ${curGid===g.id?col:"#0F766E"}`,
                      background:curGid===g.id?col+"22":"transparent",
                      color:curGid===g.id?col:"#0F766E",cursor:"pointer",fontSize:12,fontWeight:curGid===g.id?700:400}}>
                    {g.name}
                    <span style={{color:"#0F766E",fontSize:10,marginRight:4}}>
                      ({getSched(selSpec,selLv,g.id).length}حصة)
                    </span>
                  </button>
                )}
                {gs.length>1&&(
                  <button
                    onClick={()=>{ if(window.confirm(`حذف "${g.name}"؟`)) removeGroup(selSpec,selLv,g.id); }}
                    style={{padding:"5px 7px",borderRadius:"0 7px 7px 0",border:`1px solid #2a3a55`,
                      borderRight:"none",background:"#F0FDFA",color:"#ef4444",cursor:"pointer",fontSize:12,lineHeight:1}}>×</button>
                )}
              </div>
            ))}

            {/* Add new group */}
            <div style={{display:"flex",alignItems:"center",gap:0,marginRight:4}}>
              <input
                value={newGroupName}
                onChange={e=>setNewGroupName(e.target.value)}
                placeholder="اسم الرزمة..."
                onKeyDown={e=>{
                  if(e.key==="Enter"&&newGroupName.trim()){
                    addGroup(selSpec,selLv,newGroupName.trim());
                    setNewGroupName("");
                  }
                }}
                style={{background:"#FFFFFF",border:"1px solid #334155",borderRadius:"7px 0 0 7px",
                  padding:"5px 10px",color:"#0F766E",fontSize:12,width:110}}
              />
              <button
                onClick={()=>{
                  if(newGroupName.trim()){ addGroup(selSpec,selLv,newGroupName.trim()); setNewGroupName(""); }
                  else {
                    // Auto-name: رزمة 1، ب، ج...
                    const names=["أ","ب","ج","د","هـ","و","ز","ح","ط","ي"];
                    const name="رزمة "+(gs.length+1);
                    addGroup(selSpec,selLv,name);
                  }
                }}
                style={{padding:"5px 10px",borderRadius:"0 7px 7px 0",border:"1px solid #334155",
                  background:"#A5F3FC",color:"#0E7490",cursor:"pointer",fontSize:12,fontWeight:700}}>
                + رزمة
              </button>
            </div>

            {gs.length===0&&(
              <span style={{color:"#0F766E",fontSize:11}}>لا توجد رزم — أضف رزمة لتبدأ</span>
            )}

            {curGid&&sched.length>0&&(
              <button onClick={()=>setSched(selSpec,selLv,curGid,[])}
                style={{marginRight:"auto",padding:"5px 12px",borderRadius:8,border:"1px solid #ef444455",
                  background:"none",color:"#ef4444",cursor:"pointer",fontSize:11}}>
                مسح جدول هذه الرزمة
              </button>
            )}
          </div>

          {/* Hint */}
          {gs.length>0&&(
            <div style={{color:"#0C4A6E",fontSize:10,marginTop:6}}>
              💡 اضغط مرتين على اسم الرزمة لتعديله
            </div>
          )}

          {/* ── Hours summary for current group ── */}
          {curGid&&coursesInLevel.length>0&&(()=>{
            const issues = coursesInLevel.map(c=>{
              const sessions = sched.filter(s=>s.courseId===c.id);
              const theoryNeeded = c.L + c.T;
              const practNeeded  = c.P;
              const scheduledTh = sessions.filter(s=>{
                const st=s.sessionType||(c.P===0?"theory":"practical");
                return st==="theory"||c.P===0;
              }).reduce((a,s)=>a+(s.sessionHours||1),0);
              const scheduledPr = sessions.filter(s=>{
                const st=s.sessionType||(c.L===0&&c.T===0?"practical":"theory");
                return st==="practical"||(c.L===0&&c.T===0);
              }).reduce((a,s)=>a+(s.sessionHours||1),0);
              const theoryOver = theoryNeeded>0&&scheduledTh>theoryNeeded;
              const practOver  = practNeeded>0&&scheduledPr>practNeeded;
              const theoryDone = theoryNeeded===0||scheduledTh===theoryNeeded;
              const practDone  = practNeeded===0||scheduledPr===practNeeded;
              return {c, scheduledTh, scheduledPr, theoryNeeded, practNeeded,
                theoryOver, practOver, theoryDone, practDone,
                allDone: theoryDone&&practDone, anyOver: theoryOver||practOver,
                hasAny: sessions.length>0};
            });
            const overCourses  = issues.filter(x=>x.anyOver);
            const underCourses = issues.filter(x=>!x.allDone&&!x.anyOver&&x.hasAny);
            const zeroCourses  = issues.filter(x=>!x.hasAny);
            const doneCourses  = issues.filter(x=>x.allDone&&!x.anyOver&&x.hasAny);
            if(issues.every(x=>!x.hasAny)) return null;
            return (
              <div style={{marginTop:8,background:"#F0FDFA",border:"1px solid #E2E8F0",borderRadius:10,padding:"10px 14px"}}>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-start"}}>
                  <span style={{color:"#0F766E",fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>📊 ملخص الساعات:</span>
                  {doneCourses.length>0&&(
                    <span style={{color:"#065F46",fontSize:11}}>
                      ✅ مكتمل ({doneCourses.length}):
                      {doneCourses.map(x=>(
                        <span key={x.c.id} style={{background:"#D1FAE5",color:"#065F46",borderRadius:4,padding:"1px 6px",margin:"0 3px",fontSize:10,fontWeight:700}}>{x.c.code}</span>
                      ))}
                    </span>
                  )}
                  {underCourses.length>0&&(
                    <span style={{color:"#92400E",fontSize:11}}>
                      ⚠️ ناقص ({underCourses.length}):
                      {underCourses.map(x=>(
                        <span key={x.c.id}
                          title={`نظري: ${x.scheduledTh}/${x.theoryNeeded}س | عملي: ${x.scheduledPr}/${x.practNeeded}س`}
                          style={{background:"#FEF3C7",color:"#92400E",borderRadius:4,padding:"1px 6px",margin:"0 3px",fontSize:10,fontWeight:700,cursor:"help"}}>
                          {x.c.code}
                          {x.theoryNeeded>0&&!x.theoryDone&&` ن:${x.scheduledTh}/${x.theoryNeeded}`}
                          {x.practNeeded>0&&!x.practDone&&` ع:${x.scheduledPr}/${x.practNeeded}`}
                        </span>
                      ))}
                    </span>
                  )}
                  {overCourses.length>0&&(
                    <span style={{color:"#991B1B",fontSize:11}}>
                      🚫 تجاوز ({overCourses.length}):
                      {overCourses.map(x=>(
                        <span key={x.c.id}
                          title={`نظري: ${x.scheduledTh}/${x.theoryNeeded}س | عملي: ${x.scheduledPr}/${x.practNeeded}س`}
                          style={{background:"#FEE2E2",color:"#991B1B",borderRadius:4,padding:"1px 6px",margin:"0 3px",fontSize:10,fontWeight:700,cursor:"help"}}>
                          {x.c.code}
                          {x.theoryOver&&` ن+${x.scheduledTh-x.theoryNeeded}س`}
                          {x.practOver&&` ع+${x.scheduledPr-x.practNeeded}س`}
                        </span>
                      ))}
                    </span>
                  )}
                  {zeroCourses.length>0&&(
                    <span style={{color:"#0F766E",fontSize:11}}>
                      ⬜ لم يُوزَّع ({zeroCourses.length}):
                      {zeroCourses.map(x=>(
                        <span key={x.c.id} style={{background:"#F0FDFA",color:"#0F766E",borderRadius:4,padding:"1px 6px",margin:"0 3px",fontSize:10}}>{x.c.code}</span>
                      ))}
                    </span>
                  )}
                </div>
              </div>
            );
          })()}
        </div>

        {/* ── No groups prompt ── */}
        {gs.length===0&&(
          <div style={{textAlign:"center",padding:"40px 20px",border:"1px dashed #1e2d45",borderRadius:12,color:"#99F6E4",marginBottom:14}}>
            <div style={{fontSize:32,marginBottom:10}}>📋</div>
            <div style={{fontSize:14,marginBottom:14}}>لا توجد رزم للمستوى {selLv} في هذا التخصص</div>
            <Btn onClick={()=>{ addGroup(selSpec,selLv,"رزمة 1"); }} bg={col}>+ إضافة رزمة 1</Btn>
          </div>
        )}

        {curGid&&(
          <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:14}}>
            {/* Course Bank */}
            <div style={{background:"#F0FDFA",border:"1px solid #E2E8F0",borderRadius:13,padding:10,maxHeight:720,overflowY:"auto"}}>
              <div style={{color:"#0F766E",fontSize:11,fontWeight:700,marginBottom:8}}>🗂 مقررات الفصل {selLv}</div>
              {coursesInLevel.length===0
                ?<div style={{color:"#0F766E",fontSize:11,textAlign:"center"}}>لا توجد مقررات</div>
                :coursesInLevel.map(c=>{
                  const cc=c.spec==="gen"?spColor("gen"):col;
                  const a=courseAssign[c.id]||{};
                  const tTr=trainers.find(x=>x.id===a.theoryId);
                  const pTr=trainers.find(x=>x.id===a.practicalId);

                  const theoryNeeded = c.L + c.T;  // محاضرة + تمارين
                  const practNeeded  = c.P;          // عملي

                  // Sessions for this course in current group
                  const sessions = sched.filter(s=>s.courseId===c.id);

                  // Theory sessions: sessionType==="theory" OR course has no practical
                  const theorySessions = sessions.filter(s=>{
                    const st = s.sessionType || (c.P===0?"theory":"practical");
                    return st==="theory" || c.P===0;
                  });
                  // Practical sessions: sessionType==="practical" OR course has no theory
                  const practSessions = sessions.filter(s=>{
                    const st = s.sessionType || (c.L===0&&c.T===0?"practical":"theory");
                    return st==="practical" || (c.L===0&&c.T===0);
                  });

                  const scheduledTheoryH = theorySessions.reduce((a,s)=>a+(s.sessionHours||1),0);
                  const scheduledPractH  = practSessions.reduce((a,s)=>a+(s.sessionHours||1),0);

                  // Status per type
                  const theoryOver  = theoryNeeded>0 && scheduledTheoryH>theoryNeeded;
                  const practOver   = practNeeded>0  && scheduledPractH>practNeeded;
                  const theoryDone  = theoryNeeded===0 || scheduledTheoryH===theoryNeeded;
                  const practDone   = practNeeded===0  || scheduledPractH===practNeeded;
                  const allDone     = theoryDone && practDone;
                  const anyOver     = theoryOver || practOver;

                  return (
                    <div key={c.id} draggable onDragStart={e=>e.dataTransfer.setData("courseId",c.id)}
                      style={{background:anyOver?"#FEF2F2":allDone?"#F0FDF4":cc+"12",
                        border:`1px solid ${anyOver?"#EF4444":allDone?"#10B981":cc+"44"}`,
                        borderRadius:8,padding:"7px 9px",cursor:"grab",marginBottom:6,userSelect:"none"}}>

                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span style={{color:anyOver?"#991B1B":allDone?"#065F46":cc,fontSize:10,fontWeight:700,fontFamily:"monospace"}}>{c.code}</span>
                        <span style={{background:anyOver?"#EF4444":allDone?"#10B981":"#99F6E4",
                          color:anyOver||allDone?"#fff":"#0F766E",
                          borderRadius:5,padding:"1px 6px",fontSize:9,fontWeight:700}}>
                          {scheduledTheoryH+scheduledPractH}/{c.CTH}س
                        </span>
                      </div>

                      <div style={{color:"#134E4A",fontSize:10,marginTop:2,lineHeight:1.3,marginBottom:4}}>{c.name}</div>

                      {/* Theory progress bar */}
                      {theoryNeeded>0&&(
                        <div style={{marginBottom:3}}>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:1}}>
                            <span style={{fontSize:8,color:"#0891B2",fontWeight:600}}>نظري</span>
                            <span style={{fontSize:8,color:theoryOver?"#EF4444":theoryDone?"#10B981":"#0F766E",fontWeight:700}}>
                              {scheduledTheoryH}/{theoryNeeded}س
                            </span>
                          </div>
                          <div style={{background:"#CFFAFE",borderRadius:3,height:5,overflow:"hidden"}}>
                            <div style={{height:"100%",borderRadius:3,transition:"width 0.3s",
                              width:Math.min(100,scheduledTheoryH/theoryNeeded*100)+"%",
                              background:theoryOver?"#EF4444":theoryDone?"#10B981":"#0891B2"}}/>
                          </div>
                        </div>
                      )}

                      {/* Practical progress bar */}
                      {practNeeded>0&&(
                        <div style={{marginBottom:4}}>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:1}}>
                            <span style={{fontSize:8,color:"#10B981",fontWeight:600}}>عملي</span>
                            <span style={{fontSize:8,color:practOver?"#EF4444":practDone?"#10B981":"#0F766E",fontWeight:700}}>
                              {scheduledPractH}/{practNeeded}س
                            </span>
                          </div>
                          <div style={{background:"#D1FAE5",borderRadius:3,height:5,overflow:"hidden"}}>
                            <div style={{height:"100%",borderRadius:3,transition:"width 0.3s",
                              width:Math.min(100,scheduledPractH/practNeeded*100)+"%",
                              background:practOver?"#EF4444":practDone?"#10B981":"#10B981"}}/>
                          </div>
                        </div>
                      )}

                      {/* Status messages */}
                      {anyOver&&(
                        <div style={{color:"#991B1B",fontSize:9,fontWeight:700}}>
                          {theoryOver&&`⚠️ نظري تجاوز: ${scheduledTheoryH}/${theoryNeeded}س  `}
                          {practOver&&`⚠️ عملي تجاوز: ${scheduledPractH}/${practNeeded}س`}
                        </div>
                      )}
                      {allDone&&!anyOver&&sessions.length>0&&(
                        <div style={{color:"#065F46",fontSize:9,fontWeight:700}}>✅ مكتمل</div>
                      )}
                      {!allDone&&!anyOver&&sessions.length>0&&(
                        <div style={{color:"#0F766E",fontSize:9}}>
                          {theoryNeeded>0&&!theoryDone&&`نظري متبقي: ${theoryNeeded-scheduledTheoryH}س  `}
                          {practNeeded>0&&!practDone&&`عملي متبقي: ${practNeeded-scheduledPractH}س`}
                        </div>
                      )}

                      {(tTr||pTr)&&(
                        <div style={{fontSize:9,color:"#0C4A6E",lineHeight:1.7,marginTop:3,borderTop:"1px solid #E2E8F0",paddingTop:3}}>
                          {tTr&&<div>📘 {tTr.name}</div>}
                          {pTr&&<div>🔧 {pTr.name}</div>}
                        </div>
                      )}

                      {/* Other groups that also have this course */}
                      {(()=>{
                        const otherGroups = getAllGroupsForCourse(c.id, selSpec, selLv, curGid);
                        if(!otherGroups.length) return null;
                        return (
                          <div style={{marginTop:4,background:"#FFFBEB",border:"1px solid #F59E0B",
                            borderRadius:5,padding:"4px 6px"}}>
                            <div style={{color:"#92400E",fontSize:8,fontWeight:700,marginBottom:3}}>
                              🔗 هذا المقرر في رزم أخرى:
                            </div>
                            {otherGroups.map((g,i)=>(
                              <div key={i} style={{display:"flex",justifyContent:"space-between",
                                alignItems:"center",marginBottom:2}}>
                                <span style={{color:"#92400E",fontSize:8}}>{g.label}</span>
                                <span style={{background:"#F59E0B",color:"#fff",borderRadius:3,
                                  padding:"0 4px",fontSize:8,fontWeight:700}}>{g.count}حصة</span>
                              </div>
                            ))}
                          </div>
                        );
                      })()}
                    </div>
                  );
                })}
              <div style={{marginTop:10,padding:8,background:"#E0FDF9",border:"1px solid #BFDBFE",borderRadius:8,fontSize:10,color:"#1E40AF",lineHeight:2}}>
                <strong>اسحب المقرر</strong> وأفلته في الجدول<br/>
                ثم حدد عدد ساعات الجلسة (1-4)<br/>
                📘 نظري: max 3 متتالية<br/>
                🔧 عملي: max 4 متتالية<br/>
                🟩 أخضر = مكتمل | 🟥 أحمر = تجاوز
              </div>
            </div>

            {/* Grid */}
            <div style={{overflowX:"auto"}}>
              <div style={{background:"#FFFFFF",border:`1px solid ${col}55`,borderRadius:13,padding:10}}>
                <div style={{color:"#0F766E",fontSize:12,fontWeight:700,marginBottom:8}}>
                  📅 {spName(selSpec)} — الفصل {selLv} —
                  <span style={{color:col,marginRight:4}}>{gs.find(g=>g.id===curGid)?.name}</span>
                  <span style={{color:"#99F6E4",marginRight:8}}>({sched.length} حصة)</span>
                </div>
                <table style={{borderCollapse:"collapse",width:"100%"}}>
                  <thead>
                    <tr>
                      <th style={{background:"#FFFFFF",color:"#0F766E",padding:"7px 8px",
                        border:"1px solid #1e2d45",minWidth:88,fontSize:11}}>الحصة / الوقت</th>
                      {DAYS.map(d=>(
                        <th key={d} style={{background:col+"1a",color:col,padding:"7px 8px",
                          border:"1px solid #1e2d45",minWidth:155,textAlign:"center",fontSize:12}}>{d}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {(()=>{
                      const builderCovered = new Set();
                      sched.forEach(cell=>{
                        const h=cell.sessionHours||1;
                        if(h>1) for(let i=1;i<h;i++) builderCovered.add(`${cell.day}-${cell.period+i}`);
                      });
                      return periods.map(pr=>{
                        const rowCells = DAYS.map(day=>{
                          if(builderCovered.has(`${day}-${pr.id}`)) return null;
                          const cell=sched.find(s=>s.day===day&&s.period===pr.id);
                          const sessionH = cell?.sessionHours||1;
                          const cellConflicts = cell ? (() => {
                            const msgs=[];
                            const all=getAllCells();
                            // Get this cell's source family id
                            const familyId = cell.sourceId || (cell.isSharedCopy ? null : cell.id);
                            [cell.theoryTrainerId,cell.practicalTrainerId].filter(Boolean).forEach(tid=>{
                              const clash=all.find(x=>{
                                if(x.id===cell.id) return false;
                                if(x.day!==day||x.period!==pr.id) return false;
                                if(x.theoryTrainerId!==tid&&x.practicalTrainerId!==tid) return false;
                                // Skip same sharing family
                                const xFam = x.sourceId || (x.isSharedCopy ? null : x.id);
                                if(familyId && xFam && familyId===xFam) return false;
                                return true;
                              });
                              if(clash){msgs.push("تعارض مدرب: "+trainers.find(t=>t.id===tid)?.name);}
                            });
                            if(cell.hallId){
                              const clash=all.find(x=>{
                                if(x.id===cell.id) return false;
                                if(x.day!==day||x.period!==pr.id) return false;
                                if(x.hallId!==cell.hallId) return false;
                                const xFam = x.sourceId || (x.isSharedCopy ? null : x.id);
                                if(familyId && xFam && familyId===xFam) return false;
                                return true;
                              });
                              if(clash){msgs.push("تعارض قاعة: "+halls.find(h=>h.id===cell.hallId)?.name);}
                            }
                            return msgs;
                          })() : [];
                          const cForStyle = cell?courses.find(x=>x.id===cell.courseId):null;
                          const stForStyle = cell?(cell.sessionType||(cForStyle?.P===0?"theory":"practical")):null;
                          const isTheoryCell = stForStyle==="theory"||(cForStyle&&cForStyle.P===0);
                          const borderCol = cellConflicts.length?"#EF4444":cell?(isTheoryCell?"#A5F3FC":"#6EE7B7"):"#99F6E4";
                          const bgCol = cellConflicts.length?"#FEF2F2":cell?(isTheoryCell?"#ECFEFF":"#F0FDF4"):"#F0FDFA";
                          return (
                            <td key={day}
                              rowSpan={sessionH}
                              onDragOver={e=>e.preventDefault()}
                              onDrop={e=>{ setSelGroup(curGid); handleDrop(e,day,pr.id,selSpec,selLv,curGid); }}
                              style={{
                                border:`3px solid ${borderCol}`,
                                background:bgCol,
                                padding: cell ? 0 : 3,
                                verticalAlign:"top",
                                minWidth:148,
                              }}>
                              {cell&&(
                                <div style={{height:"100%",padding:3}}>
                                  <SchedCell
                                    cell={cell} courses={courses} trainers={trainers} halls={halls}
                                    spColor={spColor} mainSpec={selSpec}
                                    sched={sched} periods={periods}
                                    conflicts={cellConflicts}
                                    sharedWith={getSharedWith(cell,selSpec,selLv,curGid)}
                                    allGroups={allAvailGroups(cell,selSpec,selLv,curGid)}
                                    onAddShared={(keys)=>addSharedCopies(cell,selSpec,selLv,curGid,keys)}
                                    onRemove={()=>setSched(selSpec,selLv,curGid,p=>p.filter(s=>s.id!==cell.id))}
                                    onUpdate={upd=>handleCellUpdate(selSpec,selLv,curGid,cell.id,upd)}
                                  />
                                </div>
                              )}
                            </td>
                          );
                        });
                        return (
                          <>
                            <tr key={pr.id}>
                              <td style={{background:"#E0FDF9",padding:"4px 6px",border:"1px solid #E2E8F0",verticalAlign:"middle",minWidth:88}}>
                                <div style={{textAlign:"center"}}>
                                  <div style={{color:"#0E7490",fontWeight:700,fontSize:12}}>الحصة {pr.label}</div>
                                  <div style={{color:"#0F766E",fontSize:9,direction:"ltr"}}>{pr.start}</div>
                                  <div style={{color:"#0F766E",fontSize:9,direction:"ltr"}}>{pr.end}</div>
                                </div>
                              </td>
                              {rowCells}
                            </tr>
                            {pr.breakAfter&&(
                              <tr key={`br-${pr.id}`}>
                                <td colSpan={6} style={{background:"#ECFDF5",border:"1px solid #A7F3D0",padding:"4px 14px"}}>
                                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                                    <span style={{color:"#065F46",fontSize:10}}>🕌 {pr.breakAfter.label}</span>
                                    <span style={{color:"#065F46",fontSize:10,direction:"ltr"}}>{pr.breakAfter.from}–{pr.breakAfter.to} ({pr.breakAfter.dur}د)</span>
                                  </div>
                                </td>
                              </tr>
                            )}
                          </>
                        );
                      });
                    })()}
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        )}


        {modal==="periodSettings"&&<PeriodSettingsModal periods={periods} setPeriods={setPeriods} onClose={()=>setModal(null)}/>}
        {modal==="assignCourses"&&(
          <CourseAssignModal courses={courses} trainers={trainers}
            assignments={courseAssign} setAssignments={setCourseAssign}
            onClose={()=>setModal(null)}/>
        )}
      </div>
    );
  };

  // ── STATISTICS TAB ────────────────────────────────────────
  const StatsTab = () => {
    const [filterSpec, setFilterSpec] = useState("all");

    // ── Helpers ──────────────────────────────────────────────
    const allCells = () => Object.entries(scheds).flatMap(([sp,ls])=>
      Object.entries(ls||{}).flatMap(([lv,lvGroups])=>
        Object.entries(lvGroups||{}).flatMap(([gid,cells])=>
          cells.map(c=>({...c,spec:sp,level:+lv,gid}))
        )
      )
    );

    const specCells = (sp) => allCells().filter(c=>sp==="all"||c.spec===sp);

    const getGroups2 = (sp,lv) => (groups[sp]&&groups[sp][lv])||[];

    // ── Course completion per spec/level/group ────────────────
    const courseStats = (sp) => {
      const spCourses = courses.filter(c=>!c.coop&&(sp==="all"||c.spec===sp||c.spec==="gen"));
      const specs = sp==="all"?["net","auto"]:([sp]);
      return specs.flatMap(specId => {
        const lvls = [1,2,3,4,5];
        return lvls.flatMap(lv => {
          const lvCourses = courses.filter(c=>!c.coop&&(c.spec===specId||c.spec==="gen")&&c.level===lv);
          if(!lvCourses.length) return [];
          const gs = getGroups2(specId,lv);
          return gs.map(g => {
            const cells = (scheds[specId]&&scheds[specId][lv]&&scheds[specId][lv][g.id])||[];
            const courseResults = lvCourses.map(c => {
              const sessions = cells.filter(s=>s.courseId===c.id);
              const theoryNeeded = c.L+c.T, practNeeded = c.P;
              const scheduledTh = sessions.filter(s=>{
                const st=s.sessionType||(c.P===0?"theory":"practical");
                return st==="theory"||c.P===0;
              }).reduce((a,s)=>a+(s.sessionHours||1),0);
              const scheduledPr = sessions.filter(s=>{
                const st=s.sessionType||(c.L===0&&c.T===0?"practical":"theory");
                return st==="practical"||(c.L===0&&c.T===0);
              }).reduce((a,s)=>a+(s.sessionHours||1),0);
              const thDone = theoryNeeded===0||scheduledTh===theoryNeeded;
              const prDone = practNeeded===0||scheduledPr===practNeeded;
              const thOver = theoryNeeded>0&&scheduledTh>theoryNeeded;
              const prOver = practNeeded>0&&scheduledPr>practNeeded;
              return {c, scheduledTh, scheduledPr, theoryNeeded, practNeeded,
                done:thDone&&prDone, over:thOver||prOver, partial:sessions.length>0&&!(thDone&&prDone)&&!thOver&&!prOver, empty:sessions.length===0};
            });
            return {specId, lv, g, courseResults};
          });
        });
      });
    };

    // ── Trainer load ──────────────────────────────────────────
    const trainerLoad = trainers.map(t=>{
      const allCellsT = allCells().filter(c=>c.theoryTrainerId===t.id||c.practicalTrainerId===t.id);
      // Only count source cells (not shared copies) for hours
      const srcCells = allCellsT.filter(c=>!c.isSharedCopy);
      const thH  = srcCells.filter(c=>c.theoryTrainerId===t.id).reduce((a,s)=>a+(s.sessionHours||1),0);
      const prH  = srcCells.filter(c=>c.practicalTrainerId===t.id).reduce((a,s)=>a+(s.sessionHours||1),0);
      const total = srcCells.reduce((a,s)=>a+(s.sessionHours||1),0);
      return {t, thH, prH, total, pct:Math.min(100,t.maxH>0?total/t.maxH*100:0), over:total>t.maxH};
    });

    // ── Hall usage ────────────────────────────────────────────
    const hallUsage = halls.map(h=>{
      const cells = allCells().filter(c=>c.hallId===h.id);
      const totalPeriods = periods.length * DAYS.length;
      const usedPeriods  = cells.length;
      return {h, usedPeriods, totalPeriods, pct:Math.round(usedPeriods/totalPeriods*100)};
    });

    // ── Overall completion ────────────────────────────────────
    const stats = courseStats(filterSpec==="all"?"all":filterSpec);
    const allCourseRes = stats.flatMap(s=>s.courseResults);
    const totalCourses = allCourseRes.length;
    const doneCourses  = allCourseRes.filter(x=>x.done).length;
    const overCourses  = allCourseRes.filter(x=>x.over).length;
    const partCourses  = allCourseRes.filter(x=>x.partial).length;
    const emptyCourses = allCourseRes.filter(x=>x.empty).length;
    const completePct  = totalCourses>0?Math.round(doneCourses/totalCourses*100):0;

    // ── Stat card ─────────────────────────────────────────────
    const StatCard = ({icon,label,value,sub,color,onClick,active}) => (
      <div onClick={onClick} style={{background:active?color+"22":"#FFFFFF",border:`2px solid ${active?color:color+"44"}`,
        borderRadius:14,padding:"16px 18px",cursor:onClick?"pointer":"default",transition:"all 0.15s"}}>
        <div style={{fontSize:24,marginBottom:6}}>{icon}</div>
        <div style={{color,fontSize:26,fontWeight:800}}>{value}</div>
        <div style={{color:"#0F766E",fontSize:12,fontWeight:600}}>{label}</div>
        {sub&&<div style={{color:"#0F766E",fontSize:11,marginTop:2}}>{sub}</div>}
      </div>
    );

    // ── Progress bar ──────────────────────────────────────────
    const ProgressBar = ({value,max,color,height=8}) => (
      <div style={{background:"#99F6E4",borderRadius:99,height,overflow:"hidden"}}>
        <div style={{height:"100%",borderRadius:99,width:Math.min(100,max>0?value/max*100:0)+"%",
          background:color,transition:"width 0.4s"}}/>
      </div>
    );

    const StatusBadge = ({done,over,partial,empty}) => {
      if(over)    return <span style={{background:"#FEE2E2",color:"#991B1B",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700}}>🚫 تجاوز</span>;
      if(done)    return <span style={{background:"#D1FAE5",color:"#065F46",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700}}>✅ مكتمل</span>;
      if(partial) return <span style={{background:"#FEF3C7",color:"#92400E",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700}}>⚠️ ناقص</span>;
      return <span style={{background:"#F0FDFA",color:"#0F766E",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700}}>⬜ لم يُوزَّع</span>;
    };

    return (
      <div>
        {/* ── Filter ── */}
        <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18,marginLeft:"auto"}}>📈 الإحصائيات والتقرير النهائي</h2>
          {[{id:"all",name:"الكل",color:"#0F766E"},...SPECIALIZATIONS.filter(s=>!s.shared)].map(s=>(
            <button key={s.id} onClick={()=>setFilterSpec(s.id)}
              style={{padding:"6px 14px",borderRadius:8,border:`1px solid ${s.color}55`,
                background:filterSpec===s.id?s.color:"transparent",
                color:filterSpec===s.id?"#fff":s.color,cursor:"pointer",fontSize:12,fontWeight:600}}>
              {s.name}
            </button>
          ))}
        </div>

        {/* ── Overall KPIs ── */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:12,marginBottom:20}}>
          <StatCard icon="📋" label="إجمالي الجداول" value={stats.length} color="#0EA5E9"
            sub={`${stats.filter(s=>s.courseResults.every(x=>x.done)).length} مكتمل`}/>
          <StatCard icon="✅" label="مقررات مكتملة" value={doneCourses} color="#10B981"
            sub={`${completePct}% من الإجمالي`}/>
          <StatCard icon="⚠️" label="مقررات ناقصة" value={partCourses} color="#F59E0B"
            sub="ساعات غير مكتملة"/>
          <StatCard icon="🚫" label="تجاوز الساعات" value={overCourses} color="#EF4444"
            sub="تحتاج مراجعة"/>
          <StatCard icon="⬜" label="لم تُوزَّع" value={emptyCourses} color="#0F766E"
            sub="لم تُضَف للجدول"/>
        </div>

        {/* ── Overall progress ── */}
        <div style={{background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:14,padding:18,marginBottom:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{color:"#042F2E",fontWeight:700,fontSize:14}}>نسبة اكتمال الجداول</div>
            <div style={{color:completePct===100?"#10B981":completePct>60?"#F59E0B":"#EF4444",
              fontWeight:800,fontSize:22}}>{completePct}%</div>
          </div>
          <ProgressBar value={doneCourses} max={totalCourses}
            color={completePct===100?"#10B981":completePct>60?"#F59E0B":"#EF4444"} height={12}/>
          <div style={{display:"flex",gap:16,marginTop:10,flexWrap:"wrap"}}>
            {[
              {label:"مكتمل",v:doneCourses,color:"#10B981"},
              {label:"ناقص", v:partCourses,color:"#F59E0B"},
              {label:"تجاوز",v:overCourses,color:"#EF4444"},
              {label:"فارغ", v:emptyCourses,color:"#0C4A6E"},
            ].map(x=>(
              <div key={x.label} style={{display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:10,height:10,borderRadius:2,background:x.color}}/>
                <span style={{color:"#0F766E",fontSize:12}}>{x.label}: <strong>{x.v}</strong></span>
              </div>
            ))}
          </div>
        </div>

        {/* ── Per spec/level/group detail ── */}
        <div style={{marginBottom:20}}>
          <h3 style={{color:"#042F2E",fontSize:14,fontWeight:700,marginBottom:12}}>📋 تفاصيل المقررات لكل رزمة</h3>
          {stats.map(({specId,lv,g,courseResults})=>{
            const done=courseResults.filter(x=>x.done).length;
            const total=courseResults.length;
            const pct=total>0?Math.round(done/total*100):0;
            const sc=spColor(specId);
            return (
              <div key={`${specId}-${lv}-${g.id}`}
                style={{background:"#FFFFFF",border:`1px solid ${sc}33`,borderRadius:12,padding:16,marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <Badge color={sc}>{spName(specId)}</Badge>
                    <span style={{color:"#0F766E",fontWeight:700}}>الفصل {lv}</span>
                    <span style={{color:sc,fontWeight:700}}>— {g.name}</span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{color:pct===100?"#10B981":pct>60?"#F59E0B":"#EF4444",fontWeight:800,fontSize:16}}>{pct}%</span>
                    <span style={{color:"#0F766E",fontSize:12}}>{done}/{total}</span>
                  </div>
                </div>
                <ProgressBar value={done} max={total}
                  color={pct===100?"#10B981":pct>60?"#F59E0B":"#EF4444"} height={6}/>
                <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6}}>
                  {courseResults.map(({c,scheduledTh,scheduledPr,theoryNeeded,practNeeded,done,over,partial,empty})=>(
                    <div key={c.id} style={{
                      background:over?"#FEF2F2":done?"#F0FDF4":partial?"#FFFBEB":"#F0FDFA",
                      border:`1px solid ${over?"#EF4444":done?"#10B981":partial?"#F59E0B":"#99F6E4"}`,
                      borderRadius:8,padding:"6px 10px",minWidth:120}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        <span style={{color:spColor(c.spec),fontWeight:700,fontSize:10,fontFamily:"monospace"}}>{c.code}</span>
                        <StatusBadge done={done} over={over} partial={partial} empty={empty}/>
                      </div>
                      <div style={{color:"#0F766E",fontSize:9,marginBottom:4}}>{c.name}</div>
                      <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                        {theoryNeeded>0&&(
                          <span style={{fontSize:9,color:scheduledTh===theoryNeeded?"#065F46":scheduledTh>theoryNeeded?"#991B1B":"#92400E",
                            background:scheduledTh===theoryNeeded?"#D1FAE5":scheduledTh>theoryNeeded?"#FEE2E2":"#FEF3C7",
                            borderRadius:4,padding:"1px 5px",fontWeight:600}}>
                            ن:{scheduledTh}/{theoryNeeded}س
                          </span>
                        )}
                        {practNeeded>0&&(
                          <span style={{fontSize:9,color:scheduledPr===practNeeded?"#065F46":scheduledPr>practNeeded?"#991B1B":"#92400E",
                            background:scheduledPr===practNeeded?"#D1FAE5":scheduledPr>practNeeded?"#FEE2E2":"#FEF3C7",
                            borderRadius:4,padding:"1px 5px",fontWeight:600}}>
                            ع:{scheduledPr}/{practNeeded}س
                          </span>
                        )}
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            );
          })}
          {stats.length===0&&(
            <div style={{textAlign:"center",padding:"30px",color:"#0F766E",border:"1px dashed #E2E8F0",borderRadius:12}}>
              لا توجد رزم بعد — أضف رزمة وابدأ بتوزيع المقررات
            </div>
          )}
        </div>

        {/* ── Trainer Load ── */}
        <div style={{background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:14,padding:18,marginBottom:20}}>
          <h3 style={{color:"#042F2E",fontSize:14,fontWeight:700,marginBottom:14}}>👨‍🏫 حمل المدربين</h3>
          {trainerLoad.length===0
            ?<div style={{color:"#0F766E",fontSize:13}}>لا يوجد مدربون</div>
            :trainerLoad.map(({t,thH,prH,total,pct,over})=>(
              <div key={t.id} style={{marginBottom:14,paddingBottom:14,borderBottom:"1px solid #F1F5F9"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                  <div>
                    <span style={{color:"#042F2E",fontWeight:700,fontSize:13}}>{t.name}</span>
                    <span style={{color:"#0F766E",fontSize:11,marginRight:8}}>
                      ({total}/{t.maxH}س — نظري:{thH}س عملي:{prH}س)
                    </span>
                  </div>
                  <span style={{color:over?"#EF4444":pct>80?"#F59E0B":"#10B981",fontWeight:800,fontSize:14}}>
                    {Math.round(pct)}%
                  </span>
                </div>
                <ProgressBar value={total} max={t.maxH}
                  color={over?"#EF4444":pct>80?"#F59E0B":"#10B981"}/>
                {over&&<div style={{color:"#EF4444",fontSize:11,marginTop:4}}>⚠️ تجاوز الحد الأقصى بـ {total-t.maxH} ساعة</div>}
              </div>
            ))
          }
        </div>

        {/* ── Hall Usage ── */}
        <div style={{background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:14,padding:18,marginBottom:20}}>
          <h3 style={{color:"#042F2E",fontSize:14,fontWeight:700,marginBottom:14}}>🏛 استخدام القاعات</h3>
          {hallUsage.map(({h,usedPeriods,totalPeriods,pct})=>(
            <div key={h.id} style={{marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                <div>
                  <span style={{color:"#042F2E",fontWeight:600}}>{h.name}</span>
                  <Badge color={h.type==="theory"?"#0891B2":"#10B981"} style={{marginRight:6}}>
                    {h.type==="theory"?"نظري":"عملي"}
                  </Badge>
                  <span style={{color:"#0F766E",fontSize:11,marginRight:6}}>سعة: {h.cap} | محجوز: {usedPeriods} حصة</span>
                </div>
                <span style={{color:pct>80?"#EF4444":pct>50?"#F59E0B":"#10B981",fontWeight:700}}>{pct}%</span>
              </div>
              <ProgressBar value={usedPeriods} max={totalPeriods}
                color={pct>80?"#EF4444":pct>50?"#F59E0B":"#0891B2"}/>
            </div>
          ))}
        </div>

        {/* ── Final Report ── */}
        <div style={{background:"linear-gradient(135deg,#1E3A5F,#1D4ED8)",borderRadius:14,padding:20,marginBottom:20}}>
          <h3 style={{color:"#FFFFFF",fontSize:15,fontWeight:700,marginBottom:16}}>📄 التقرير النهائي</h3>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[
              {l:"نسبة اكتمال الجداول",v:`${completePct}%`,ok:completePct===100},
              {l:"مقررات مكتملة",      v:`${doneCourses}/${totalCourses}`,ok:doneCourses===totalCourses},
              {l:"مقررات تجاوزت الساعات",v:overCourses,ok:overCourses===0},
              {l:"مقررات لم تُوزَّع",  v:emptyCourses,ok:emptyCourses===0},
              {l:"مدربون تجاوزوا الحد",v:trainerLoad.filter(x=>x.over).length,ok:trainerLoad.filter(x=>x.over).length===0},
              {l:"قاعات غير مستخدمة", v:hallUsage.filter(x=>x.usedPeriods===0).length,ok:true},
            ].map(item=>(
              <div key={item.l} style={{background:"#ffffff18",borderRadius:10,padding:"10px 14px",
                border:`1px solid ${item.ok?"#10B98155":"#EF444455"}`}}>
                <div style={{color:item.ok?"#6EE7B7":"#FCA5A5",fontSize:10,marginBottom:3}}>{item.l}</div>
                <div style={{color:"#FFFFFF",fontWeight:800,fontSize:18}}>{item.v}</div>
                <div style={{fontSize:14}}>{item.ok?"✅":"⚠️"}</div>
              </div>
            ))}
          </div>
          <div style={{marginTop:16,padding:"10px 14px",background:"#ffffff15",borderRadius:10,color:"#0C4A6E",fontSize:12,lineHeight:2}}>
            {completePct===100&&overCourses===0&&trainerLoad.filter(x=>x.over).length===0
              ? "🎉 ممتاز! جميع الجداول مكتملة وجاهزة للتطبيق."
              : [
                  overCourses>0     && `• يوجد ${overCourses} مقرر تجاوز الساعات المحددة`,
                  emptyCourses>0    && `• يوجد ${emptyCourses} مقرر لم يُوزَّع بعد`,
                  partCourses>0     && `• يوجد ${partCourses} مقرر ساعاته ناقصة`,
                  trainerLoad.filter(x=>x.over).length>0 && `• يوجد ${trainerLoad.filter(x=>x.over).length} مدرب تجاوز الحد الأقصى للساعات`,
                ].filter(Boolean).join(" | ")
            }
          </div>
        </div>
      </div>
    );
  };

  // ── VIEW SCHEDULES ────────────────────────────────────────
  const ViewTab = () => {
    const col=spColor(viewSpec);
    const allC=Object.entries(scheds[viewSpec]||{}).flatMap(([lv,lvGroups])=>
      Object.entries(lvGroups||{}).flatMap(([gid,cells])=>{
        const gName=(groups[viewSpec]&&groups[viewSpec][+lv]||[]).find(g=>g.id===gid)?.name||gid;
        return cells.map(c=>({...c,level:+lv,gid,gName}));
      })
    );

    const GridTable = ({cells,color}) => {
      const coveredSlots = new Set();
      cells.forEach(cell=>{
        const h = cell.sessionHours||1;
        if(h>1){ for(let i=1;i<h;i++) coveredSlots.add(`${cell.day}-${cell.period+i}`); }
      });
      return (
      <div style={{overflowX:"auto",background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:11,padding:9}}>
        <table style={{borderCollapse:"collapse",fontSize:11,width:"100%"}}>
          <thead>
            <tr>
              <th style={{background:"#F0FDFA",color:"#0F766E",padding:"6px 8px",border:"1px solid #E2E8F0",minWidth:88,fontSize:10}}>الحصة / الوقت</th>
              {DAYS.map(d=><th key={d} style={{background:color+"1a",color,padding:"6px 8px",border:"1px solid #E2E8F0",minWidth:150,textAlign:"center"}}>{d}</th>)}
            </tr>
          </thead>
          <tbody>
            {periods.map(pr=>{
              const rowCells = DAYS.map(day=>{
                if(coveredSlots.has(`${day}-${pr.id}`)) return null;
                const cell=cells.find(c=>c.day===day&&c.period===pr.id);
                const crs=cell?courses.find(c=>c.id===cell.courseId):null;
                const tTr=cell?trainers.find(t=>t.id===cell.theoryTrainerId):null;
                const pTr=cell?trainers.find(t=>t.id===cell.practicalTrainerId):null;
                const hl=cell?halls.find(h=>h.id===cell.hallId):null;
                const cc=crs?(crs.spec==="gen"?spColor("gen"):color):color;
                const sessionType=cell?.sessionType||(crs?.P===0?"theory":"practical");
                const isTheory=sessionType==="theory"||(crs&&crs.P===0);
                const sessionH=cell?.sessionHours||1;
                const showTr=isTheory?tTr:pTr;
                return (
                  <td key={day} rowSpan={sessionH}
                    style={{border:`3px solid ${crs?(isTheory?"#A5F3FC":"#6EE7B7"):"#99F6E4"}`,
                      background:crs?(isTheory?"#ECFEFF":"#ECFDF5"):"#F0FDFA",
                      padding:"8px 10px",verticalAlign:"top"}}>
                    {crs&&(
                      <div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                          <span style={{background:isTheory?"#0891B2":"#10B981",color:"#fff",borderRadius:5,padding:"2px 7px",fontSize:9,fontWeight:700}}>
                            {isTheory?"📘 نظري":"🔧 عملي"}
                          </span>
                          <div style={{display:"flex",gap:3}}>
                            {cell.refNumber&&<span style={{background:cc,color:"#fff",borderRadius:4,padding:"1px 5px",fontSize:8,fontWeight:700}}>{cell.refNumber}</span>}
                            <span style={{background:isTheory?"#0E7490":"#065F46",color:"#fff",borderRadius:5,padding:"2px 6px",fontSize:9,fontWeight:700}}>{sessionH}س</span>
                          </div>
                        </div>
                        <div style={{color:cc,fontWeight:800,fontFamily:"monospace",fontSize:10,marginBottom:1}}>{crs.code}</div>
                        <div style={{color:"#134E4A",fontSize:9,lineHeight:1.4,marginBottom:4}}>{crs.name}</div>
                        {showTr&&<div style={{color:isTheory?"#0E7490":"#065F46",fontSize:9,marginBottom:2}}>👨‍🏫 {showTr.name}</div>}
                        {hl&&<div style={{color:"#0F766E",fontSize:9}}>🏛 {hl.name}</div>}
                        {sessionH>1&&<div style={{marginTop:4,color:isTheory?"#0891B2":"#10B981",fontSize:8,fontWeight:600,direction:"ltr"}}>{pr.start} — {periods.find(p=>p.id===pr.id+sessionH-1)?.end||""}</div>}
                      </div>
                    )}
                  </td>
                );
              }).filter(Boolean);
              return (
                <>
                  <tr key={pr.id}>
                    <td style={{background:"#E0FDF9",padding:"4px 6px",border:"1px solid #E2E8F0",textAlign:"center",verticalAlign:"middle"}}>
                      <div style={{color:"#0E7490",fontWeight:700,fontSize:11}}>الحصة {pr.label}</div>
                      <div style={{color:"#0F766E",fontSize:9,direction:"ltr"}}>{pr.start}–{pr.end}</div>
                    </td>
                    {rowCells}
                  </tr>
                  {pr.breakAfter&&(
                    <tr key={"br"+pr.id}>
                      <td colSpan={6} style={{background:"#ECFDF5",border:"1px solid #A7F3D0",padding:"3px 12px",fontSize:9,color:"#065F46"}}>
                        🕌 {pr.breakAfter.label} — {pr.breakAfter.from}–{pr.breakAfter.to}
                      </td>
                    </tr>
                  )}
                </>
              );
            })}
          </tbody>
        </table>
      </div>
      );
    };

    return (
      <div id="print-area">
        <div className="no-print" style={{display:"flex",gap:10,marginBottom:18,alignItems:"center",flexWrap:"wrap"}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>عرض الجداول</h2>
          <div style={{display:"flex",gap:7}}>
            {[{id:"trainees",l:"جدول المتدربين",i:"👥"},{id:"trainers",l:"جدول المدربين",i:"👨‍🏫"},{id:"halls",l:"جدول القاعات",i:"🏛"}].map(m=>(
              <button key={m.id} onClick={()=>setViewMode(m.id)}
                style={{padding:"6px 12px",borderRadius:8,border:"1px solid #CBD5E1",
                  background:viewMode===m.id?"#99F6E4":"transparent",color:viewMode===m.id?"#134E4A":"#0F766E",cursor:"pointer",fontSize:12}}>
                {m.i} {m.l}
              </button>
            ))}
          </div>
          {viewMode!=="halls"&&(
            <div style={{display:"flex",gap:7}}>
              {SPECIALIZATIONS.filter(s=>!s.shared).map(s=>(
                <button key={s.id} onClick={()=>setViewSpec(s.id)}
                  style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${s.color}55`,
                    background:viewSpec===s.id?s.color:"transparent",color:viewSpec===s.id?"#fff":s.color,cursor:"pointer",fontSize:12}}>
                  {s.name}
                </button>
              ))}
            </div>
          )}
          <button onClick={()=>window.print()}
            style={{marginRight:"auto",padding:"8px 18px",borderRadius:10,
              background:"#EF4444",color:"#fff",border:"none",
              cursor:"pointer",fontSize:13,fontWeight:700,boxShadow:"0 2px 8px #EF444433"}}>
            🖨️ طباعة / PDF
          </button>
        </div>

        {/* Print header — shown only when printing */}
        <div id="print-header" style={{display:"none",justifyContent:"space-between",
          alignItems:"center",background:"linear-gradient(135deg,#0C4A6E,#0E7490)",padding:"10px 16px",marginBottom:14,
          color:"white",direction:"rtl"}}>

          {/* Right: Logo + college name */}
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <img src={LOGO_B64} alt="TVTC"
              style={{height:44,width:"auto",objectFit:"contain",filter:"brightness(0) invert(1)"}}/>
            <div style={{borderRight:"1px solid #ffffff44",paddingRight:12}}>
              <div style={{fontSize:"7pt",color:"#0C4A6E",marginBottom:2}}>المؤسسة العامة للتدريب التقني والمهني</div>
              <div style={{fontSize:"12pt",fontWeight:900,color:"#FFFFFF"}}>{collegeName||"الكلية التقنية"}</div>
            </div>
          </div>

          {/* Center: Schedule title */}
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:"13pt",fontWeight:800,color:"#FFFFFF"}}>
              {viewMode==="trainees"?"جدول المتدربين":viewMode==="trainers"?"جدول المدربين":"جدول القاعات"}
            </div>
            <div style={{fontSize:"9pt",color:"#0C4A6E"}}>
              {viewMode!=="halls"?spName(viewSpec):"جميع القاعات"} — العام الدراسي ١٤٤٧هـ
            </div>
          </div>

          {/* Left: Date/time + preparer */}
          <div style={{textAlign:"left",fontSize:"9pt",color:"#0C4A6E"}}>
            <div style={{color:"#FDE68A",fontWeight:700,marginBottom:2}}>
              {new Date().toLocaleDateString("ar-SA",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}
            </div>
            <div>{new Date().toLocaleTimeString("ar-SA",{hour:"2-digit",minute:"2-digit"})}</div>
            <div style={{marginTop:4,color:"#0C4A6E"}}>م. معاذ عبدالله السلوم</div>
            <div style={{color:"#0C4A6E"}}>وكيل شؤون المدربين</div>
          </div>
        </div>

        {viewMode==="trainees"&&[1,2,3,4,5].map(lv=>{
          const lvGroups=scheds[viewSpec]&&scheds[viewSpec][lv]?scheds[viewSpec][lv]:{};
          const gs2=(groups[viewSpec]&&groups[viewSpec][lv])||[];
          const hasAny=Object.values(lvGroups).some(c=>c.length>0);
          if(!hasAny) return null;
          return (
            <div key={lv} style={{marginBottom:24}}>
              <div style={{color:col,fontWeight:700,fontSize:14,marginBottom:10}}>الفصل التدريبي {lv}</div>
              {gs2.map(g=>{
                const cells=(lvGroups[g.id])||[];
                if(!cells.length) return null;
                return (
                  <div key={g.id} style={{marginBottom:14}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                      <div style={{width:3,height:16,background:col,borderRadius:2}}/>
                      <span style={{color:col,fontWeight:700,fontSize:13}}>{g.name}</span>
                      <Badge color={col}>{cells.length} حصة</Badge>
                      {(()=>{
                        const th=cells.filter(c=>c.sessionType==="theory"||(courses.find(x=>x.id===c.courseId)?.P===0)).length;
                        const pr=cells.filter(c=>c.sessionType==="practical"||(courses.find(x=>x.id===c.courseId)?.L===0&&courses.find(x=>x.id===c.courseId)?.T===0)).length;
                        return (<>
                          {th>0&&<span style={{background:"#EFF6FF",color:"#0E7490",borderRadius:5,padding:"1px 8px",fontSize:10,fontWeight:700}}>📘 نظري: {th}</span>}
                          {pr>0&&<span style={{background:"#ECFDF5",color:"#065F46",borderRadius:5,padding:"1px 8px",fontSize:10,fontWeight:700}}>🔧 عملي: {pr}</span>}
                        </>);
                      })()}
                    </div>
                    <GridTable cells={cells} color={col}/>
                  </div>
                );
              })}
            </div>
          );
        })}

        {viewMode==="trainers"&&trainers.map(t=>{
          // Show ALL cells (including shared copies) in the visual schedule
          const cells=allC.filter(c=>c.theoryTrainerId===t.id||c.practicalTrainerId===t.id);
          if(!cells.length) return null;
          // Hours count: only NON-shared-copy cells (same as usedH)
          const srcCells=cells.filter(c=>!c.isSharedCopy);
          const thH=srcCells.filter(c=>c.theoryTrainerId===t.id).reduce((a,s)=>a+(s.sessionHours||1),0);
          const prH=srcCells.filter(c=>c.practicalTrainerId===t.id).reduce((a,s)=>a+(s.sessionHours||1),0);
          return (
            <div key={t.id} style={{background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:13,padding:14,marginBottom:14}}>
              <div style={{marginBottom:10}}>
                <div style={{color:"#042F2E",fontWeight:800,fontSize:14,marginBottom:6}}>{t.name}</div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  <span style={{background:"#EFF6FF",color:"#0E7490",borderRadius:6,padding:"3px 10px",fontSize:10,fontWeight:700}}>📘 نظري: {thH}س</span>
                  <span style={{background:"#ECFDF5",color:"#065F46",borderRadius:6,padding:"3px 10px",fontSize:10,fontWeight:700}}>🔧 عملي: {prH}س</span>
                  <span style={{background:thH+prH>t.maxH?"#FEE2E2":"#FEF3C7",
                    color:thH+prH>t.maxH?"#991B1B":"#92400E",borderRadius:6,padding:"3px 10px",fontSize:10,fontWeight:700}}>
                    إجمالي: {thH+prH}/{t.maxH}س
                  </span>
                  {cells.some(c=>c.isSharedCopy)&&(
                    <span style={{background:"#F5F3FF",color:"#6D28D9",borderRadius:5,
                      padding:"3px 8px",fontSize:10,fontWeight:700}}>
                      🔗 {cells.filter(c=>c.isSharedCopy).length} مشترك (غير محسوب)
                    </span>
                  )}
                </div>
              </div>
              <GridTable cells={cells} color={col}/>
            </div>
          );
        })}

        {viewMode==="halls"&&halls.map(hall=>{
          const hc=Object.entries(scheds).flatMap(([sp,ls])=>
            Object.entries(ls||{}).flatMap(([lv,lvGroups])=>
              Object.entries(lvGroups||{}).flatMap(([gid,cells])=>
                cells.filter(c=>c.hallId===hall.id).map(c=>({...c,spec:sp,level:+lv,gid}))
              )
            )
          );
          if(!hc.length) return null;
          const thCount = hc.filter(c=>c.sessionType==="theory"||(courses.find(x=>x.id===c.courseId)?.P===0)).length;
          const prCount = hc.filter(c=>c.sessionType==="practical").length;
          const isTheoryHall = hall.type==="theory";
          return (
            <div key={hall.id} style={{background:"#FFFFFF",
              border:`2px solid ${isTheoryHall?"#A5F3FC":"#6EE7B7"}`,borderRadius:13,padding:14,marginBottom:14}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                <div style={{color:"#042F2E",fontWeight:800,fontSize:14}}>{hall.name}</div>
                <span style={{background:isTheoryHall?"#EFF6FF":"#ECFDF5",
                  color:isTheoryHall?"#0E7490":"#065F46",borderRadius:6,
                  padding:"3px 10px",fontSize:11,fontWeight:700}}>
                  {isTheoryHall?"📘 نظري":"🔧 عملي"}
                </span>
                <span style={{background:"#FEF3C7",color:"#92400E",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700}}>
                  👥 سعة: {hall.cap} متدرب
                </span>
                <span style={{background:"#F0FDF9",color:"#0C4A6E",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700}}>
                  {hc.length} حصة محجوزة
                </span>
                {thCount>0&&<span style={{background:"#EFF6FF",color:"#0E7490",borderRadius:5,padding:"2px 8px",fontSize:10}}>📘 {thCount} نظري</span>}
                {prCount>0&&<span style={{background:"#ECFDF5",color:"#065F46",borderRadius:5,padding:"2px 8px",fontSize:10}}>🔧 {prCount} عملي</span>}
              </div>
              <GridTable cells={hc} color="#0D9488"/>
            </div>
          );
        })}
      </div>
    );
  };


  // ── SHARING MODAL ─────────────────────────────────────────

  // ── QUOTA TAB ─────────────────────────────────────────────
  const QuotaTab = () => {
    const THEORY_SIZE  = theoryCap; // متدرب / شعبة نظري
    const PRACT_SIZE   = practCap;  // متدرب / شعبة عملي

    // General Studies course codes (shared)
    const GEN_CODES = new Set([
      "ENGL 101","ENGL 102","ENGL 103","ENGL 204",
      "ETHS 201","ENTR 101","ISLM 101","ARAB 101",
      "MATH 101","PHYS 101",
    ]);

    const calcGroups = (count, size) =>
      count > 0 ? Math.ceil(count / size) : 0;

    const setCount = (cid, val) => {
      const n = Math.max(0, parseInt(val)||0);
      setQuotaCounts(p=>({...p,[cid]:n}));
    };

    // Spec sections: net, auto, then gen separately
    const sections = [
      { spec:"net",  label:"تقنية شبكات الحاسب",      color:"#0EA5E9" },
      { spec:"auto", label:"تقنية المحركات والمركبات", color:"#F97316" },
      { spec:"gen",  label:"الدراسات العامة (مشتركة)", color:"#8B5CF6" },
    ];

    // For net/auto: include their own + gen courses; for gen section: only gen
    const getSectionCourses = (spec) => {
      if(spec==="gen") return courses.filter(c=>c.spec==="gen"&&!c.coop);
      return [
        ...courses.filter(c=>c.spec===spec&&!c.coop),
        ...courses.filter(c=>c.spec==="gen"&&!c.coop),
      ];
    };

    // Grand totals for summary
    const allRows = sections.flatMap(s=>
      getSectionCourses(s.spec).map(c=>({
        c, count:quotaCounts[c.id]||0,
        theoryH:c.L+c.T, practH:c.P,
      }))
    );
    const totalTheoryGroups = allRows.reduce((a,r)=>a+(r.theoryH>0?calcGroups(r.count,THEORY_SIZE):0),0);
    const totalPractGroups  = allRows.reduce((a,r)=>a+(r.practH>0?calcGroups(r.count,PRACT_SIZE):0),0);

    const GroupBadge = ({n,color}) => n===0 ? null : (
      <span style={{background:color+"22",color,border:`1px solid ${color}44`,
        borderRadius:5,padding:"1px 8px",fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>
        {n} شعبة
      </span>
    );

    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap",gap:10}}>
          <h2 style={{color:"#042F2E",margin:0,fontSize:18}}>📋 النصاب التدريبي</h2>
          <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
            {/* Import Excel button */}
            <button onClick={()=>setModal("importQuota")}
              style={{padding:"8px 16px",borderRadius:10,border:"1px solid #0E7490",
                background:"#E0FDF9",color:"#0C4A6E",cursor:"pointer",fontSize:12,fontWeight:700,
                display:"flex",alignItems:"center",gap:6}}>
              📊 استيراد من Excel
            </button>
            <div style={{display:"flex",gap:10}}>
              <div style={{background:"#E0FDF9",border:"1px solid #A5F3FC",borderRadius:10,padding:"8px 16px",textAlign:"center"}}>
                <div style={{color:"#0F766E",fontSize:10}}>النظري / شعبة</div>
                <div style={{color:"#0E7490",fontWeight:800,fontSize:18}}>{THEORY_SIZE} متدرب</div>
              </div>
              <div style={{background:"#ECFDF5",border:"1px solid #6EE7B7",borderRadius:10,padding:"8px 16px",textAlign:"center"}}>
                <div style={{color:"#0F766E",fontSize:10}}>العملي / شعبة</div>
                <div style={{color:"#065F46",fontWeight:800,fontSize:18}}>{PRACT_SIZE} متدرباً</div>
              </div>
            </div>
          </div>
        </div>

        {/* ── Import Quota Modal ── */}
        {modal==="importQuota"&&(
          <div style={{position:"fixed",inset:0,background:"#00000099",zIndex:4000,
            display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{background:"#FFFFFF",borderRadius:18,padding:24,width:520,
              maxHeight:"90vh",overflow:"auto",direction:"rtl",boxShadow:"0 20px 60px #00000033"}}>

              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                <h3 style={{color:"#042F2E",margin:0,fontSize:16,fontWeight:800}}>
                  📊 استيراد عدد المتدربين من Excel
                </h3>
                <button onClick={()=>{setModal(null);setImportTxt("");}}
                  style={{background:"none",border:"none",color:"#0C4A6E",cursor:"pointer",fontSize:22}}>×</button>
              </div>

              {/* Instructions */}
              <div style={{background:"#E0FDF9",border:"1px solid #A5F3FC",borderRadius:10,
                padding:"12px 14px",marginBottom:16,fontSize:11,color:"#0C4A6E",lineHeight:1.9}}>
                <strong>خطوات الاستيراد:</strong><br/>
                1. افتح ملف Excel الخاص بك<br/>
                2. حدد الأعمدة التي تريد نسخها (رمز المقرر + عدد المتدربين)<br/>
                3. انسخ الخلايا وألصقها في المربع أدناه<br/>
                4. حدد في أي عمود رمز المقرر وأي عمود عدد المتدربين
              </div>

              {/* Column selector */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                <div>
                  <label style={{display:"block",color:"#0C4A6E",fontSize:12,fontWeight:700,marginBottom:4}}>
                    عمود رمز المقرر (Code)
                  </label>
                  <select id="quota-code-col" defaultValue="0"
                    style={{width:"100%",border:"1px solid #A5F3FC",borderRadius:8,padding:"8px 10px",
                      fontSize:12,color:"#042F2E",background:"#F0FDFA"}}>
                    {["العمود الأول","العمود الثاني","العمود الثالث","العمود الرابع","العمود الخامس"].map((l,i)=>(
                      <option key={i} value={i}>{l}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label style={{display:"block",color:"#0C4A6E",fontSize:12,fontWeight:700,marginBottom:4}}>
                    عمود عدد المتدربين
                  </label>
                  <select id="quota-count-col" defaultValue="1"
                    style={{width:"100%",border:"1px solid #A5F3FC",borderRadius:8,padding:"8px 10px",
                      fontSize:12,color:"#042F2E",background:"#F0FDFA"}}>
                    {["العمود الأول","العمود الثاني","العمود الثالث","العمود الرابع","العمود الخامس"].map((l,i)=>(
                      <option key={i} value={i}>{l}</option>
                    ))}
                  </select>
                </div>
              </div>

              {/* Separator option */}
              <div style={{marginBottom:14}}>
                <label style={{display:"block",color:"#0C4A6E",fontSize:12,fontWeight:700,marginBottom:4}}>
                  فاصل الأعمدة
                </label>
                <div style={{display:"flex",gap:8}}>
                  {[{v:"tab",l:"Tab (افتراضي Excel)"},
                    {v:",",l:"فاصلة ,"},
                    {v:";",l:"فاصلة منقوطة ;"}].map(sep=>(
                    <label key={sep.v} style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer",
                      fontSize:11,color:"#0C4A6E"}}>
                      <input type="radio" name="sep" value={sep.v} defaultChecked={sep.v==="tab"}/>
                      {sep.l}
                    </label>
                  ))}
                </div>
              </div>

              {/* Paste area */}
              <div style={{marginBottom:14}}>
                <label style={{display:"block",color:"#0C4A6E",fontSize:12,fontWeight:700,marginBottom:4}}>
                  الصق البيانات هنا (Ctrl+V):
                </label>
                <textarea
                  id="quota-paste"
                  rows={8}
                  placeholder={"ICMT 121\t45\nINET 111\t38\nENGL 101\t72\n..."}
                  style={{width:"100%",border:"1px solid #A5F3FC",borderRadius:8,padding:10,
                    fontSize:11,fontFamily:"monospace",direction:"ltr",
                    background:"#F0FDFA",boxSizing:"border-box",color:"#042F2E",resize:"vertical"}}
                />
              </div>

              {/* Preview */}
              <div id="quota-preview" style={{marginBottom:14}}/>

              {/* Buttons */}
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{
                  const text = document.getElementById("quota-paste")?.value||"";
                  const codeCol = +document.getElementById("quota-code-col")?.value||0;
                  const countCol = +document.getElementById("quota-count-col")?.value||1;
                  const sepVal = document.querySelector('input[name="sep"]:checked')?.value||"tab";
                  const sep = sepVal==="tab"?"\t":sepVal;

                  const lines2 = text.split("\n").filter(l=>l.trim());
                  let matched=0, notFound=[];
                  const newCounts = {...quotaCounts};

                  lines2.forEach(line=>{
                    const cols = line.split(sep).map(c=>c.trim().replace(/\r/,""));
                    const rawCode = cols[codeCol]||"";
                    const rawCount = cols[countCol]||"";
                    const count = parseInt(rawCount.replace(/[^\d]/g,""))||0;
                    if(!rawCode||!count) return;

                    // Try to match course by code (partial or full match)
                    const matched_course = courses.find(c=>
                      c.code.replace(/\s/g,"").toLowerCase() === rawCode.replace(/\s/g,"").toLowerCase() ||
                      c.code.toLowerCase().includes(rawCode.toLowerCase()) ||
                      rawCode.toLowerCase().includes(c.code.replace(/\s/g,"").toLowerCase())
                    );
                    if(matched_course){
                      newCounts[matched_course.id] = count;
                      matched++;
                    } else {
                      notFound.push(rawCode);
                    }
                  });

                  setQuotaCounts(newCounts);
                  setModal(null);
                  const msg = `✅ تم استيراد ${matched} مقرر بنجاح!`+
                    (notFound.length>0?`\n\n⚠️ لم يُعثر على (${notFound.length}):\n${notFound.slice(0,10).join(", ")}`:"");
                  alert(msg);
                }} style={{flex:1,background:"#0E7490",color:"#fff",border:"none",borderRadius:10,
                  padding:"12px",cursor:"pointer",fontSize:13,fontWeight:700}}>
                  ✅ استيراد
                </button>
                <button onClick={()=>{setModal(null);}}
                  style={{padding:"12px 18px",background:"#F0FDFA",color:"#0C4A6E",
                    border:"1px solid #A5F3FC",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:700}}>
                  إلغاء
                </button>
              </div>
            </div>
          </div>
        )}

        {sections.map(sec=>{
          const secCourses = getSectionCourses(sec.spec);
          const byLevel = [1,2,3,4,5].map(lv=>({
            lv, list:secCourses.filter(c=>c.level===lv)
          })).filter(x=>x.list.length>0);

          return (
            <div key={sec.spec} style={{marginBottom:24}}>
              {/* Section header */}
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                <div style={{width:5,height:22,background:sec.color,borderRadius:3}}/>
                <h3 style={{color:sec.color,margin:0,fontSize:15,fontWeight:800}}>{sec.label}</h3>
              </div>

              {byLevel.map(({lv,list})=>(
                <div key={lv} style={{marginBottom:14}}>
                  <div style={{color:"#0F766E",fontSize:11,fontWeight:700,marginBottom:6,
                    paddingRight:8,borderRight:`2px solid ${sec.color}44`}}>
                    الفصل التدريبي {lv}
                  </div>
                  <div style={{overflowX:"auto"}}>
                    <table style={{borderCollapse:"collapse",width:"100%",fontSize:12}}>
                      <thead>
                        <tr style={{background:"#F0FDFA"}}>
                          {["الرمز","اسم المقرر","مح","عم","س.أ","عدد المتدربين","شعب نظري","شعب عملي","إجمالي الشعب","إجمالي ساعات الاتصال"].map(h=>(
                            <th key={h} style={{color:"#0C4A6E",padding:"8px 10px",
                              border:"1px solid #E2E8F0",textAlign:"center",whiteSpace:"nowrap",fontSize:11,fontWeight:700}}>
                              {h}
                            </th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {list.map(c=>{
                          const count = quotaCounts[c.id]||0;
                          const isGen = GEN_CODES.has(c.code);
                          const theoryH = c.L+c.T;
                          const practH  = c.P;
                          const thGroups = theoryH>0 ? calcGroups(count,THEORY_SIZE) : 0;
                          const prGroups = practH>0  ? calcGroups(count,PRACT_SIZE)  : 0;
                          const total    = thGroups + prGroups;
                          const cColor   = isGen ? "#8B5CF6" : sec.color;
                          return (
                            <tr key={c.id} style={{background:isGen?"#FAFAF9":"#FFFFFF"}}>
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",
                                color:cColor,fontWeight:700,fontFamily:"monospace",fontSize:11}}>
                                {c.code}
                                {isGen&&<span style={{marginRight:4,background:"#EDE9FE",color:"#7C3AED",
                                  fontSize:8,padding:"1px 4px",borderRadius:3,fontWeight:700}}>مشترك</span>}
                              </td>
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",color:"#134E4A"}}>
                                {c.name}
                              </td>
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                {theoryH>0?<Badge color="#0891B2">{theoryH}س</Badge>:<span style={{color:"#0C4A6E"}}>—</span>}
                              </td>
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                {practH>0?<Badge color="#10B981">{practH}س</Badge>:<span style={{color:"#0C4A6E"}}>—</span>}
                              </td>
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                <Badge color="#F59E0B">{c.CTH}س</Badge>
                              </td>
                              {/* Trainee count input */}
                              <td style={{padding:"5px 8px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                <div style={{display:"flex",alignItems:"center",gap:6,justifyContent:"center"}}>
                                  <input
                                    type="number" min={0}
                                    defaultValue={quotaCounts[c.id]||""}
                                    key={c.id}
                                    onBlur={e=>{
                                      const n=parseInt(e.target.value)||0;
                                      setQuotaCounts(p=>({...p,[c.id]:n}));
                                    }}
                                    onKeyDown={e=>{
                                      if(e.key==="Enter"){
                                        const n=parseInt(e.target.value)||0;
                                        setQuotaCounts(p=>({...p,[c.id]:n}));
                                        e.target.blur();
                                      }
                                    }}
                                    placeholder="0"
                                    style={{width:80,background:count>0?"#F0FDF4":"#F0FDFA",
                                      border:`2px solid ${count>0?"#10B981":"#99F6E4"}`,
                                      borderRadius:8,padding:"5px 8px",textAlign:"center",
                                      fontSize:13,fontWeight:700,color:"#134E4A"}}
                                  />
                                  <span style={{color:"#0F766E",fontSize:10}}>متدرب</span>
                                </div>
                              </td>
                              {/* Theory groups */}
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                {theoryH>0&&count>0 ? (
                                  <div>
                                    <GroupBadge n={thGroups} color="#0891B2"/>
                                    <div style={{color:"#0F766E",fontSize:9,marginTop:2}}>
                                      {count} ÷ {THEORY_SIZE} = {thGroups}
                                    </div>
                                  </div>
                                ) : <span style={{color:"#0C4A6E",fontSize:11}}>—</span>}
                              </td>
                              {/* Practical groups */}
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                {practH>0&&count>0 ? (
                                  <div>
                                    <GroupBadge n={prGroups} color="#10B981"/>
                                    <div style={{color:"#0F766E",fontSize:9,marginTop:2}}>
                                      {count} ÷ {PRACT_SIZE} = {prGroups}
                                    </div>
                                  </div>
                                ) : <span style={{color:"#0C4A6E",fontSize:11}}>—</span>}
                              </td>
                              {/* Total groups */}
                              <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                {total>0 ? (
                                  <span style={{background:cColor,color:"#fff",borderRadius:7,
                                    padding:"3px 10px",fontWeight:800,fontSize:13}}>
                                    {total}
                                  </span>
                                ) : <span style={{color:"#0C4A6E"}}>—</span>}
                              </td>
                              {/* Total contact hours = (شعب نظري × مح) + (شعب عملي × عم) */}
                              {(()=>{
                                const totalCTH = (thGroups * theoryH) + (prGroups * practH);
                                return (
                                  <td style={{padding:"7px 10px",border:"1px solid #E2E8F0",textAlign:"center",
                                    background:totalCTH>0?"#FFF7ED":"transparent"}}>
                                    {totalCTH>0 ? (
                                      <div>
                                        <span style={{background:"#F97316",color:"#fff",borderRadius:7,
                                          padding:"3px 10px",fontWeight:800,fontSize:13}}>
                                          {totalCTH}س
                                        </span>
                                        {total>0&&(
                                          <div style={{color:"#0F766E",fontSize:9,marginTop:2}}>
                                            {thGroups>0?`${thGroups}×${theoryH}`:""}{thGroups>0&&prGroups>0?" + ":""}{prGroups>0?`${prGroups}×${practH}`:""}
                                          </div>
                                        )}
                                      </div>
                                    ) : <span style={{color:"#0C4A6E"}}>—</span>}
                                  </td>
                                );
                              })()}
                            </tr>
                          );
                        })}
                        {/* Level subtotal */}
                        {(()=>{
                          const filledList = list.filter(c=>quotaCounts[c.id]>0);
                          if(!filledList.length) return null;
                          const subTh=filledList.reduce((a,c)=>a+(c.L+c.T>0?calcGroups(quotaCounts[c.id]||0,THEORY_SIZE):0),0);
                          const subPr=filledList.reduce((a,c)=>a+(c.P>0?calcGroups(quotaCounts[c.id]||0,PRACT_SIZE):0),0);
                          const subCTH=filledList.reduce((a,c)=>{
                            const th=c.L+c.T>0?calcGroups(quotaCounts[c.id]||0,THEORY_SIZE):0;
                            const pr=c.P>0?calcGroups(quotaCounts[c.id]||0,PRACT_SIZE):0;
                            return a+(th*(c.L+c.T))+(pr*c.P);
                          },0);
                          return (
                            <tr style={{background:"#F0F9FF"}}>
                              <td colSpan={6} style={{padding:"6px 10px",border:"1px solid #E2E8F0",
                                color:"#0E7490",fontWeight:700,fontSize:11}}>
                                مجموع الفصل {lv}
                              </td>
                              <td style={{padding:"6px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                <Badge color="#0891B2">{subTh} شعبة</Badge>
                              </td>
                              <td style={{padding:"6px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                <Badge color="#10B981">{subPr} شعبة</Badge>
                              </td>
                              <td style={{padding:"6px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                <span style={{background:"#0E7490",color:"#fff",borderRadius:7,
                                  padding:"2px 10px",fontWeight:800,fontSize:12}}>{subTh+subPr}</span>
                              </td>
                              <td style={{padding:"6px 10px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                                <span style={{background:"#F97316",color:"#fff",borderRadius:7,
                                  padding:"2px 10px",fontWeight:800,fontSize:12}}>{subCTH}س</span>
                              </td>
                            </tr>
                          );
                        })()}
                      </tbody>
                    </table>
                  </div>
                </div>
              ))}
            </div>
          );
        })}

        {/* Grand total summary */}
        {(totalTheoryGroups+totalPractGroups)>0&&(()=>{
          const totalCTH = sections.flatMap(s=>getSectionCourses(s.spec)).reduce((a,c)=>{
            const count=quotaCounts[c.id]||0;
            if(!count) return a;
            const th=c.L+c.T>0?calcGroups(count,THEORY_SIZE):0;
            const pr=c.P>0?calcGroups(count,PRACT_SIZE):0;
            return a+(th*(c.L+c.T))+(pr*c.P);
          },0);
          return (
          <div style={{background:"linear-gradient(135deg,#1E3A5F,#1D4ED8)",borderRadius:14,padding:20,marginTop:8}}>
            <div style={{color:"#FFFFFF",fontWeight:800,fontSize:15,marginBottom:14}}>
              📊 ملخص النصاب الإجمالي
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
              <div style={{background:"#ffffff18",borderRadius:10,padding:"12px",textAlign:"center"}}>
                <div style={{color:"#0C4A6E",fontSize:10}}>إجمالي شعب النظري</div>
                <div style={{color:"#CFFAFE",fontWeight:900,fontSize:24}}>{totalTheoryGroups}</div>
                <div style={{color:"#0F766E",fontSize:9}}>× {THEORY_SIZE} متدرب/شعبة</div>
              </div>
              <div style={{background:"#ffffff18",borderRadius:10,padding:"12px",textAlign:"center"}}>
                <div style={{color:"#6EE7B7",fontSize:10}}>إجمالي شعب العملي</div>
                <div style={{color:"#D1FAE5",fontWeight:900,fontSize:24}}>{totalPractGroups}</div>
                <div style={{color:"#6EE7B7",fontSize:9}}>× {PRACT_SIZE} متدرباً/شعبة</div>
              </div>
              <div style={{background:"#ffffff28",borderRadius:10,padding:"12px",textAlign:"center",border:"1px solid #ffffff33"}}>
                <div style={{color:"#FDE68A",fontSize:10}}>إجمالي الشعب الكلي</div>
                <div style={{color:"#FFFFFF",fontWeight:900,fontSize:28}}>{totalTheoryGroups+totalPractGroups}</div>
                <div style={{color:"#FDE68A",fontSize:9}}>شعبة تدريبية</div>
              </div>
              <div style={{background:"#ffffff28",borderRadius:10,padding:"12px",textAlign:"center",border:"2px solid #FB923C"}}>
                <div style={{color:"#FED7AA",fontSize:10}}>إجمالي ساعات الاتصال</div>
                <div style={{color:"#FFFFFF",fontWeight:900,fontSize:28}}>{totalCTH}</div>
                <div style={{color:"#FED7AA",fontSize:9}}>ساعة اتصال أسبوعية</div>
              </div>
            </div>
          </div>
          );
        })()}

        {/* ── Trainer Average Hours per Specialization ── */}
        {Object.values(quotaCounts).some(v=>v>0)&&(()=>{
          // For each specialization, calculate total CTH from quota
          // Then divide by number of trainers eligible for that spec
          const specData = ["net","auto"].map(specId=>{
            const sc = spColor(specId);
            // Only specialization courses (NOT gen) — gen is separate
            const specOnlyCourses = courses.filter(c=>c.spec===specId&&!c.coop);
            const genOnlyCourses  = courses.filter(c=>c.spec==="gen"&&!c.coop);

            const calcCTH = (list) => list.reduce((a,c)=>{
              const count=quotaCounts[c.id]||0;
              if(!count) return a;
              const th=c.L+c.T>0?calcGroups(count,THEORY_SIZE):0;
              const pr=c.P>0?calcGroups(count,PRACT_SIZE):0;
              return a+(th*(c.L+c.T))+(pr*c.P);
            },0);

            const specCTH_only = calcCTH(specOnlyCourses); // تخصص فقط
            const genCTH       = calcCTH(genOnlyCourses);  // دراسات عامة فقط
            const specCTH      = specCTH_only + genCTH;    // الإجمالي

            // Theory CTH
            const theoryCTH = [...specOnlyCourses,...genOnlyCourses].reduce((a,c)=>{
              const count=quotaCounts[c.id]||0;
              if(!count||c.L+c.T===0) return a;
              return a+calcGroups(count,THEORY_SIZE)*(c.L+c.T);
            },0);
            // Practical CTH
            const practCTH = [...specOnlyCourses,...genOnlyCourses].reduce((a,c)=>{
              const count=quotaCounts[c.id]||0;
              if(!count||c.P===0) return a;
              return a+calcGroups(count,PRACT_SIZE)*c.P;
            },0);
            // Eligible trainers — spec trainers handle spec+gen, gen-only handle gen
            const specTrainers = trainers.filter(t=>t.specs.includes(specId));
            const genTrainers  = trainers.filter(t=>t.specs.includes("gen")&&!t.specs.includes(specId));
            const allEligible  = trainers.filter(t=>t.specs.includes(specId)||t.specs.includes("gen"));
            // Average if distributed among eligible trainers
            const avgPerTrainer = allEligible.length>0 ? Math.round(specCTH/allEligible.length) : 0;
            const maxAllowed    = allEligible.length>0 ? Math.max(...allEligible.map(t=>t.maxH)) : 0;
            const minNeeded     = maxAllowed>0 ? Math.ceil(specCTH/maxAllowed) : 0;
            return {specId, sc, specCTH, specCTH_only, genCTH, theoryCTH, practCTH,
                    specTrainers, genTrainers, allEligible,
                    avgPerTrainer, maxAllowed, minNeeded};
          });

          return (
            <div style={{marginTop:20}}>
              <h3 style={{color:"#042F2E",fontSize:14,fontWeight:800,marginBottom:14,
                borderBottom:"2px solid #E2E8F0",paddingBottom:8}}>
                👨‍🏫 متوسط الساعات المتوقعة للمدربين حسب التخصص
              </h3>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                {specData.map((item)=>{
                  const {specId,sc,specCTH,specCTH_only,genCTH,theoryCTH,practCTH,
                    specTrainers,genTrainers,allEligible,avgPerTrainer,maxAllowed,minNeeded} = item;
                  if(!specCTH) return null;
                  const overloaded = avgPerTrainer > maxAllowed;
                  return (
                    <div key={specId} style={{background:"#FFFFFF",border:`2px solid ${sc}33`,
                      borderRadius:14,padding:18}}>
                      {/* Spec header */}
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
                        <div style={{width:4,height:20,background:sc,borderRadius:2}}/>
                        <div style={{color:sc,fontWeight:800,fontSize:14}}>{spName(specId)}</div>
                      </div>

                      {/* CTH breakdown */}
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
                        <div style={{background:"#E0FDF9",borderRadius:8,padding:"8px",textAlign:"center"}}>
                          <div style={{color:"#0F766E",fontSize:9}}>ساعات النظري</div>
                          <div style={{color:"#0E7490",fontWeight:800,fontSize:16}}>{theoryCTH}</div>
                          <div style={{color:"#0C4A6E",fontSize:8}}>س/أسبوع</div>
                        </div>
                        <div style={{background:"#ECFDF5",borderRadius:8,padding:"8px",textAlign:"center"}}>
                          <div style={{color:"#0F766E",fontSize:9}}>ساعات العملي</div>
                          <div style={{color:"#065F46",fontWeight:800,fontSize:16}}>{practCTH}</div>
                          <div style={{color:"#0C4A6E",fontSize:8}}>س/أسبوع</div>
                        </div>
                        <div style={{background:`${sc}11`,borderRadius:8,padding:"8px",textAlign:"center",
                          border:`1px solid ${sc}33`}}>
                          <div style={{color:"#0F766E",fontSize:9}}>الإجمالي</div>
                          <div style={{color:sc,fontWeight:900,fontSize:18}}>{specCTH}</div>
                          <div style={{color:"#0C4A6E",fontSize:8}}>س/أسبوع</div>
                        </div>
                      </div>
                      {/* Breakdown: spec vs gen */}
                      {genCTH>0&&(
                        <div style={{background:"#F5F3FF",border:"1px solid #DDD6FE",borderRadius:8,
                          padding:"6px 10px",marginBottom:10,fontSize:10,color:"#5B21B6"}}>
                          📚 منها: <strong>{specCTH_only}س</strong> تخصص +
                          <strong> {genCTH}س</strong> دراسات عامة
                        </div>
                      )}

                      {/* Trainer distribution */}
                      <div style={{marginBottom:10}}>
                        <div style={{color:"#0C4A6E",fontSize:11,fontWeight:700,marginBottom:8}}>
                          توزيع على المدربين المؤهلين:
                        </div>
                        {allEligible.map(t=>{
                          const isSpec = t.specs.includes(specId);
                          const isGenOnly = !isSpec && t.specs.includes("gen");
                          // Spec trainer: handles spec+gen hours
                          // Gen-only trainer: handles gen hours only
                          const trainerCTH = isSpec
                            ? Math.round(specCTH / specTrainers.length +
                                (genTrainers.length===0 ? 0 : 0)) // spec trainers split all
                            : Math.round(genCTH / (genTrainers.length||1)); // gen-only split gen
                          const actualCTH = isSpec
                            ? Math.round(specCTH / allEligible.length)
                            : Math.round(genCTH / Math.max(genTrainers.length,1));
                          const pct = Math.min(100,t.maxH>0?actualCTH/t.maxH*100:0);
                          const over = actualCTH>t.maxH;
                          return (
                            <div key={t.id} style={{marginBottom:8}}>
                              <div style={{display:"flex",justifyContent:"space-between",
                                alignItems:"center",marginBottom:3}}>
                                <div style={{display:"flex",alignItems:"center",gap:6}}>
                                  <span style={{color:"#134E4A",fontSize:11,fontWeight:600}}>{t.name}</span>
                                  <span style={{background:isSpec?sc+"22":"#F3F4F6",
                                    color:isSpec?sc:"#8B5CF6",borderRadius:4,
                                    padding:"1px 6px",fontSize:9,fontWeight:700}}>
                                    {isSpec?spName(specId):"دراسات عامة"}
                                  </span>
                                </div>
                                <div style={{display:"flex",alignItems:"center",gap:6}}>
                                  <span style={{color:over?"#EF4444":"#0F766E",
                                    fontSize:11,fontWeight:700}}>
                                    ~{actualCTH}س
                                  </span>
                                  <span style={{color:"#0C4A6E",fontSize:10}}>/ {t.maxH}س</span>
                                  {over&&<span style={{background:"#FEE2E2",color:"#991B1B",
                                    borderRadius:4,padding:"1px 5px",fontSize:9,fontWeight:700}}>
                                    تجاوز!
                                  </span>}
                                </div>
                              </div>
                              <div style={{background:"#99F6E4",borderRadius:99,height:6,overflow:"hidden"}}>
                                <div style={{height:"100%",borderRadius:99,
                                  width:pct+"%",
                                  background:over?"#EF4444":pct>80?"#F59E0B":"#10B981",
                                  transition:"width 0.3s"}}/>
                              </div>
                            </div>
                          );
                        })}
                      </div>

                      {/* Recommendation */}
                      <div style={{background:overloaded?"#FEF2F2":"#F0FDF4",
                        border:`1px solid ${overloaded?"#EF4444":"#10B981"}`,
                        borderRadius:8,padding:"8px 12px",fontSize:11}}>
                        {overloaded ? (
                          <div style={{color:"#991B1B"}}>
                            ⚠️ الحمل يتجاوز الطاقة — يُنصح بـ
                            <strong> {minNeeded} مدرب </strong>
                            كحد أدنى لتغطية {specCTH} ساعة
                          </div>
                        ) : (
                          <div style={{color:"#065F46"}}>
                            ✅ الحمل مناسب — متوسط {avgPerTrainer} ساعة / مدرب
                            من أصل {allEligible.length} مدرب مؤهل
                          </div>
                        )}
                      </div>
                    </div>
                  );
                })}
              </div>
            </div>
          );
        })()}
      </div>
    );
  };

  // ── RENDER ────────────────────────────────────────────────
  const [menuOpen, setMenuOpen] = useState(false);

  const doBackup = () => {
    const data = {
      version:"1.0",
      savedAt: new Date().toLocaleString("ar-SA"),
      courses, trainers, halls, trainees,
      scheds, groups, courseAssign, periods, quotaCounts, theoryCap, practCap,
    };
    const blob = new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a");
    a.href = url;
    a.download = `جدول-احتياطي-${new Date().toISOString().slice(0,10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const TAB_ICONS = {
    dashboard: { icon: "📊", label: "الرئيسية" },
    courses:   { icon: "📚", label: "المقررات" },
    trainers:  { icon: "👨‍🏫", label: "المدربون" },
    halls:     { icon: "🏛",  label: "القاعات"  },
    trainees:  { icon: "👥",  label: "المتدربون"},
    schedule:  { icon: "📅",  label: "الجداول"  },
    view:      { icon: "🗓",  label: "العرض"    },
    stats:     { icon: "📈",  label: "الإحصائيات"},
    quota:     { icon: "📋",  label: "النصاب التدريبي"},
  };

  return (
    <div style={{minHeight:"100vh",background:"#F0FDFA",fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif",direction:"rtl",paddingBottom:100}}>

      {/* ══ SPLASH / LOGIN SCREEN ══ */}
      {!entered&&(
        <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#0C4A6E 0%,#0E7490 50%,#0891B2 100%)",
          display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
          padding:"20px 20px",direction:"rtl"}}>

          {/* Logo card — white bg, large logo */}
          <div style={{background:"#FFFFFF",borderRadius:20,padding:"24px 32px",
            marginBottom:20,boxShadow:"0 12px 40px #00000044",width:"100%",maxWidth:400,textAlign:"center"}}>
            <img src={LOGO_B64} alt="TVTC"
              style={{width:"100%",maxWidth:360,height:"auto",objectFit:"contain"}}/>
          </div>

          {/* Title bar */}
          <div style={{textAlign:"center",marginBottom:20}}>
            <div style={{color:"#FFFFFF",fontSize:22,fontWeight:900,marginBottom:4,
              textShadow:"0 2px 8px #00000044"}}>
              نظام الجداول التدريبية
            </div>
            <div style={{color:"#BAE6FD",fontSize:13,fontWeight:500}}>
              إعداد وإدارة الجداول — وكالة شؤون المدربين
            </div>
          </div>

          {/* Main card */}
          <div style={{background:"#FFFFFF",borderRadius:20,padding:"24px 20px",
            width:"100%",maxWidth:400,boxShadow:"0 12px 40px #00000044"}}>

            <div style={{marginBottom:16}}>
              <label style={{display:"block",color:"#0C4A6E",fontSize:14,fontWeight:800,marginBottom:8}}>
                🏫 اسم الكلية
              </label>
              <input
                type="text"
                value={collegeName}
                onChange={e=>setCollegeName(e.target.value)}
                onKeyDown={e=>{
                  if(e.key==="Enter"&&collegeName.trim()){
                    logEntry(collegeName.trim());
                    setEntered(true);
                  }
                }}
                placeholder="الكلية التقنية بالبدائع"
                style={{width:"100%",border:`2px solid ${collegeName?"#0E7490":"#0D9488"}`,
                  borderRadius:12,padding:"13px 16px",fontSize:15,color:"#0C4A6E",
                  direction:"rtl",boxSizing:"border-box",outline:"none",fontWeight:600,
                  background:collegeName?"#F0F9FF":"#F0FDFA",transition:"all 0.2s"}}
              />
            </div>

            <button
              onClick={()=>{
                if(collegeName.trim()){
                  logEntry(collegeName.trim());
                  setEntered(true);
                }
              }}
              disabled={!collegeName.trim()}
              style={{width:"100%",padding:"15px",borderRadius:12,border:"none",
                background:collegeName.trim()?"linear-gradient(135deg,#0C4A6E,#0E7490)":"#99F6E4",
                color:collegeName.trim()?"#FFFFFF":"#0D9488",fontSize:16,fontWeight:900,
                cursor:collegeName.trim()?"pointer":"not-allowed",
                boxShadow:collegeName.trim()?"0 6px 20px #0E749066":"none",
                transition:"all 0.2s",letterSpacing:0.5}}>
              دخول النظام &larr;
            </button>

            {/* Stats */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:16}}>
              {[
                {l:"مقررات الشبكات",v:"14",c:"#0EA5E9"},
                {l:"مقررات المركبات",v:"20",c:"#F97316"},
                {l:"مقررات مشتركة",v:"12",c:"#8B5CF6"},
                {l:"فصول دراسية",v:"5",c:"#10B981"},
              ].map(s=>(
                <div key={s.l} style={{background:s.c+"15",border:`1.5px solid ${s.c}55`,
                  borderRadius:10,padding:"10px 6px",textAlign:"center"}}>
                  <div style={{color:s.c,fontWeight:900,fontSize:22}}>{s.v}</div>
                  <div style={{color:"#0C4A6E",fontSize:11,fontWeight:600}}>{s.l}</div>
                </div>
              ))}
            </div>
          </div>

          {/* Footer */}
          <div style={{marginTop:18,textAlign:"center"}}>
            <div style={{color:"#BAE6FD",fontSize:11,fontWeight:600}}>
              تصميم وتطوير: م. معاذ عبدالله السلوم
            </div>
            <div style={{color:"#7DD3FC",fontSize:10,marginTop:2}}>
              وكيل شؤون المدربين — الكلية التقنية بالبدائع
            </div>
            <button
              onClick={()=>{
                const p = prompt("كلمة مرور المشرف:");
                if(p===ADMIN_PASS){ loadUsers(); setAdminView(true); }
                else if(p!==null) alert("كلمة المرور غير صحيحة");
              }}
              style={{marginTop:12,background:"transparent",border:"1px solid #ffffff33",
                color:"#7DD3FC",borderRadius:8,padding:"6px 16px",cursor:"pointer",fontSize:10}}>
              🔐 لوحة المشرف
            </button>
          </div>
        </div>
      )}

      {/* ══ ADMIN DASHBOARD ══ */}
      {adminView&&!entered&&(
        <div style={{minHeight:"100vh",background:"#0C4A6E",direction:"rtl",padding:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
            <h2 style={{color:"#FFFFFF",margin:0,fontSize:18,fontWeight:800}}>🔐 لوحة المشرف</h2>
            <button onClick={()=>setAdminView(false)}
              style={{background:"#ffffff22",border:"none",color:"#A5F3FC",
                borderRadius:8,padding:"8px 16px",cursor:"pointer",fontSize:13}}>
              ← رجوع
            </button>
          </div>

          {/* Stats */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:20}}>
            <div style={{background:"#ffffff15",borderRadius:14,padding:16,textAlign:"center"}}>
              <div style={{color:"#A5F3FC",fontSize:11}}>إجمالي الجلسات</div>
              <div style={{color:"#FFFFFF",fontWeight:900,fontSize:32}}>{users.length}</div>
            </div>
            <div style={{background:"#ffffff15",borderRadius:14,padding:16,textAlign:"center"}}>
              <div style={{color:"#A5F3FC",fontSize:11}}>كليات مختلفة</div>
              <div style={{color:"#FFFFFF",fontWeight:900,fontSize:32}}>
                {new Set(users.map(u=>u.college)).size}
              </div>
            </div>
          </div>

          {/* College breakdown */}
          {(()=>{
            const collegeCounts = users.reduce((acc,u)=>{
              acc[u.college]=(acc[u.college]||0)+1; return acc;
            },{});
            return (
              <div style={{background:"#ffffff10",borderRadius:14,padding:16,marginBottom:16}}>
                <div style={{color:"#A5F3FC",fontWeight:700,marginBottom:12}}>الكليات المستخدِمة</div>
                {Object.entries(collegeCounts).sort((a,b)=>b[1]-a[1]).map(([col,cnt])=>(
                  <div key={col} style={{display:"flex",justifyContent:"space-between",
                    alignItems:"center",marginBottom:8,padding:"8px 12px",
                    background:"#ffffff10",borderRadius:8}}>
                    <span style={{color:"#FFFFFF",fontSize:13}}>{col}</span>
                    <span style={{background:"#0E7490",color:"#fff",borderRadius:20,
                      padding:"2px 10px",fontSize:11,fontWeight:700}}>
                      {cnt} {cnt===1?"جلسة":"جلسات"}
                    </span>
                  </div>
                ))}
                {Object.keys(collegeCounts).length===0&&(
                  <div style={{color:"#7DD3FC",fontSize:12,textAlign:"center"}}>لا توجد جلسات مسجلة بعد</div>
                )}
              </div>
            );
          })()}

          {/* Sessions list */}
          <div style={{background:"#ffffff10",borderRadius:14,padding:16}}>
            <div style={{color:"#A5F3FC",fontWeight:700,marginBottom:12}}>
              آخر الجلسات ({users.length})
            </div>
            <div style={{maxHeight:400,overflowY:"auto"}}>
              {users.slice(0,50).map((u,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",
                  alignItems:"center",padding:"10px 12px",marginBottom:6,
                  background:"#ffffff10",borderRadius:10,
                  borderRight:`3px solid #0E7490`}}>
                  <div>
                    <div style={{color:"#FFFFFF",fontSize:13,fontWeight:600}}>{u.college}</div>
                    <div style={{color:"#7DD3FC",fontSize:10}}>{u.date} — {u.timeStr}</div>
                  </div>
                  <span style={{color:"#A5F3FC",fontSize:10}}>#{i+1}</span>
                </div>
              ))}
              {users.length===0&&(
                <div style={{color:"#7DD3FC",fontSize:12,textAlign:"center",padding:20}}>
                  لا توجد جلسات مسجلة
                </div>
              )}
            </div>
          </div>

          <button onClick={async()=>{
            if(window.confirm("هل تريد حذف جميع السجلات؟")){
              try {
                const r = await window.storage.list("session-",true);
                for(const k of (r?.keys||[])) await window.storage.delete(k,true);
                setUsers([]);
              } catch(e){}
            }
          }} style={{marginTop:16,width:"100%",padding:12,background:"#EF444422",
            color:"#FCA5A5",border:"1px solid #EF444444",borderRadius:10,
            cursor:"pointer",fontSize:13,fontWeight:700}}>
            🗑️ حذف جميع السجلات
          </button>
        </div>
      )}

      {/* ══ MAIN APP (shown after login) ══ */}
      {entered&&(<>
      <style dangerouslySetInnerHTML={{__html:`
        @media print {
          @page { size: A4 landscape; margin: 6mm; }
          body * { visibility: hidden !important; }
          #print-area, #print-area * { visibility: visible !important; }
          #print-area { position: fixed !important; inset: 0; background: white !important; padding: 6mm; }
          #print-header { display: flex !important; border-radius: 8px; margin-bottom: 10px; }
          .no-print { display: none !important; }
          table { font-size: 7pt !important; }
          td, th { padding: 3px 5px !important; }
        }
      `}}/>

      {/* ── Top Header ── */}
      <div style={{background:"linear-gradient(135deg,#0C4A6E,#0E7490)",padding:"10px 16px",
        display:"flex",alignItems:"center",justifyContent:"space-between",
        position:"sticky",top:0,zIndex:400,boxShadow:"0 2px 12px #00000033"}}>

        {/* Logo + title */}
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <img src={LOGO_B64} alt="TVTC"
            style={{height:36,width:"auto",objectFit:"contain",filter:"brightness(0) invert(1)",flexShrink:0}}/>
          <div style={{borderRight:"1px solid #ffffff33",paddingRight:10}}>
            <div style={{color:"#FFFFFF",fontWeight:800,fontSize:13,lineHeight:1.2}}>نظام الجداول التدريبية</div>
            <div style={{color:"#0C4A6E",fontSize:10,lineHeight:1.3,fontWeight:600}}>{collegeName}</div>
            <div style={{color:"#0C4A6E",fontSize:8,direction:"ltr",lineHeight:1.2}}>07:30–14:20 | 8 حصص | 50 دقيقة</div>
          </div>
        </div>

        {/* Backup + Restore + autosave indicator */}
        <div style={{display:"flex",gap:8,flexShrink:0,alignItems:"center"}}>
          <span style={{color:"#A5F3FC",fontSize:9,display:"flex",alignItems:"center",gap:3}}>
            💾 <span>محفوظ تلقائياً</span>
          </span>
          <button onClick={doBackup} title="نسخ احتياطي"
            style={{width:38,height:38,borderRadius:10,border:"1px solid #10b98166",
              background:"#10b98120",color:"#10b981",cursor:"pointer",fontSize:18,
              display:"flex",alignItems:"center",justifyContent:"center"}}>
            💾
          </button>
          <button onClick={()=>setModal("restore")} title="استعادة"
            style={{width:38,height:38,borderRadius:10,border:"1px solid #f59e0b66",
              background:"#f59e0b20",color:"#f59e0b",cursor:"pointer",fontSize:18,
              display:"flex",alignItems:"center",justifyContent:"center"}}>
            📂
          </button>
        </div>
      </div>

      {/* ── Page Title Bar ── */}
      <div style={{background:"#FFFFFF",borderBottom:"2px solid #E2E8F0",
        padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:22}}>{TAB_ICONS[tab]?.icon}</span>
        <span style={{color:"#042F2E",fontWeight:800,fontSize:16}}>{TAB_ICONS[tab]?.label}</span>
      </div>

      {/* Content */}
      <div style={{padding:"14px 12px",maxWidth:1480,margin:"0 auto"}}>
        {tab==="dashboard"&&<Dashboard/>}
        {tab==="courses"  &&<CoursesTab/>}
        {tab==="trainers" &&<TrainersTab/>}
        {tab==="halls"    &&<HallsTab/>}
        {tab==="trainees" &&<TraineesTab/>}
        {tab==="schedule" &&<ScheduleTab/>}
        {tab==="view"     &&<ViewTab/>}
        {tab==="stats"    &&<StatsTab/>}
        {tab==="quota"    &&<QuotaTab/>}
      </div>

      {/* ── Bottom Tab Bar ── */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:400,
        background:"#FFFFFF",borderTop:"1px solid #E2E8F0",
        display:"flex",flexDirection:"column",alignItems:"stretch",
        boxShadow:"0 -4px 20px #00000015"}}>
        <div style={{display:"flex",alignItems:"stretch"}}>
        {TABS.map(t=>{
          const active = tab === t.id;
          return (
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
                justifyContent:"center",padding:"8px 2px 6px",border:"none",cursor:"pointer",
                background:active?"#ECFEFF":"#FFFFFF",
                borderTop:active?"3px solid #0E7490":"3px solid transparent",
                transition:"all 0.15s"}}>
              <span style={{fontSize:20,lineHeight:1,marginBottom:3}}>{TAB_ICONS[t.id]?.icon}</span>
              <span style={{fontSize:9,fontWeight:active?700:500,
                color:active?"#0E7490":"#0F766E",lineHeight:1,whiteSpace:"nowrap"}}>
                {TAB_ICONS[t.id]?.label}
              </span>
            </button>
          );
        })}
        </div>
        {/* Developer credit */}
        <div style={{textAlign:"center",padding:"5px 8px 8px",
          borderTop:"1px solid #E2E8F0",background:"#0C4A6E"}}>
          <span style={{color:"#0C4A6E",fontSize:10}}>
            تصميم وتطوير &nbsp;
            <strong style={{color:"#FFFFFF"}}>م. معاذ عبدالله السلوم</strong>
            &nbsp;—&nbsp; وكيل شؤون المدربين بالكلية التقنية بالبدائع
          </span>
        </div>
      </div>

      {/* Restore modal */}
      {modal==="restore"&&(
        <div style={{position:"fixed",inset:0,background:"#000000bb",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#FFFFFF",border:"1px solid #1e2d45",borderRadius:16,padding:26,width:520,direction:"rtl"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
              <h3 style={{color:"#042F2E",margin:0,fontSize:16}}>📂 استعادة نسخة احتياطية</h3>
              <button onClick={()=>setModal(null)} style={{background:"none",border:"none",color:"#0F766E",cursor:"pointer",fontSize:22}}>×</button>
            </div>
            <div style={{color:"#0F766E",fontSize:12,marginBottom:14,lineHeight:1.8}}>
              الصق محتوى ملف النسخة الاحتياطية (.json) هنا، أو اسحب الملف وأفلته في المربع أدناه.
            </div>
            <textarea
              id="restore-input"
              rows={8}
              placeholder='{ "version": "1.0", "courses": [...], ... }'
              style={{width:"100%",background:"#FFFFFF",border:"1px solid #1e3a5f",borderRadius:8,
                padding:12,color:"#134E4A",fontSize:12,fontFamily:"monospace",boxSizing:"border-box",direction:"ltr",resize:"vertical"}}
              onDragOver={e=>e.preventDefault()}
              onDrop={e=>{
                e.preventDefault();
                const file=e.dataTransfer.files[0];
                if(!file)return;
                const reader=new FileReader();
                reader.onload=ev=>{ document.getElementById("restore-input").value=ev.target.result; };
                reader.readAsText(file);
              }}
            />
            <div style={{display:"flex",gap:10,marginTop:14}}>
              <button onClick={()=>{
                try{
                  const txt=document.getElementById("restore-input").value.trim();
                  const data=JSON.parse(txt);
                  if(data.courses)  setCourses(data.courses);
                  if(data.trainers) setTrainers(data.trainers);
                  if(data.halls)    setHalls(data.halls);
                  if(data.trainees) setTrainees(data.trainees);
                  if(data.scheds)   setScheds(data.scheds);
                  if(data.groups)   setGroups(data.groups);
                  if(data.courseAssign) setCourseAssign(data.courseAssign);
                  if(data.periods)  setPeriods(data.periods);
                  if(data.quotaCounts) setQuotaCounts(data.quotaCounts);
                  if(data.theoryCap)  setTheoryCap(data.theoryCap);
                  if(data.practCap)   setPractCap(data.practCap);
                  setModal(null);
                  alert("✅ تم استعادة النسخة الاحتياطية بنجاح!");
                }catch(e){
                  alert("❌ خطأ في قراءة الملف. تأكد أن الملف صحيح.");
                }
              }} style={{flex:1,background:"#10b981",color:"#fff",border:"none",borderRadius:9,
                padding:"10px",cursor:"pointer",fontSize:13,fontWeight:700}}>
                ✅ استعادة
              </button>
              <button onClick={()=>setModal(null)} style={{flex:1,background:"transparent",color:"#0F766E",
                border:"1px solid #334155",borderRadius:9,padding:"10px",cursor:"pointer",fontSize:13}}>
                إلغاء
              </button>
            </div>
          </div>
        </div>
      )}

      {/* Sharing Modal */}
      </>) /* end entered */}
    </div>
  );
}
